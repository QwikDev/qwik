...
- The Vite plugin leaves Qwik libraries external on the server by default; only libraries that
  import Qwik Router or a v1 package name (`checkExternals` in
  `packages/qwik-vite/src/plugins/vite.ts`) stay bundled, because the router's virtual modules and
  the v1 aliases exist only inside the app bundle. Keep that guard until the router reads its
  config through an API instead of virtual imports.
