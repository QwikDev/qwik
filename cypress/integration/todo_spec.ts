...
describe('todo', () => {
  ...
  it('should add new item', () => {
    ...
    cy.get('todo-item:last-child label').should((todo) => expect(todo).to.have.text('New Item'));
  });

  it('should remove item', () => {
    cy.get('.todo-count > strong').should((strong) => expect(strong).to.have.text('3'));
    cy.get('todo-item:last-child button').invoke('show').click();
    cy.get('.todo-count > strong').should((strong) => expect(strong).to.have.text('2'));
  });

  it('should complete an item', () => {
    cy.get('.todo-count > strong').should((strong) => expect(strong).to.have.text('3'));
    cy.get('todo-item:last-child input').click();
    cy.get('.todo-count > strong').should((strong) => expect(strong).to.have.text('2'));
  });

  it('should edit an item', function () {
    cy.get('todo-item:first-child').dblclick();
    cy.wait(20);
    cy.get('todo-item:first-child input.edit').type('123{enter}');
    cy.get('todo-item:first-child').should((item: any) =>
  ...
  it('should clear completed', () => {
    cy.get('.todo-count > strong').should((strong) => expect(strong).to.have.text('3'));
    cy.get('todo-item:first-child input[type=checkbox]').click();
