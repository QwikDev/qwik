...
```html
<div q:host>
  <button q:obj="1" on:cilck="./chunk-a.js#Counter_button_onClick[0]">0</button>
  <div q:host>...</div>
</div>
```

1. For the HTML `<button>` the resultning HTML is `<button>` along with `on:click` attribute for the event.
...
Notice that TypeScript automatically creates a correct prop with `$` suffix, which generates the parameterized `T` of `QRL<T>`. In our case `T` is `() => void`. This type information makes sure that you correctly pass `QRL` to `on<event>Qrl` suffix and closures to `on<ovent>$` suffix. 
...
Notice that we can pass the `props.onClickQrl` directly to the `onDblclickQrl` as seen on `<button>`. (see attribute `onDblclickQrl={props.onClickQrl}`.) This is because both the inputing prop `onClickQrl` as well as JSX prop `onDblclickQrl` are of type `QRL<?>` (and both have `Qrl` suffix.)
