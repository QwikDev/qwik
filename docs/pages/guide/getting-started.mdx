...
1. Browser wants ES modules, whereas `node.js` runs better with `commonJS`.
...
At this point, it is important to take a tour of the To-do application to understand the main differences with the current generation of frameworks. Understanding the differences will give you better insight into many technical decisions of the framework.
...
To resume a Qwik application, it takes less than 1kB of Javascript, which then executes on the client in about 10 ms. The thing to understand is that this code is in no way specific to the To-do application. The cost described above is fixed no matter the size or complexity of the application. Current generation frameworks must resume the application on the client to make it interactive. This requires downloading the framework and the application. The hydration cost is proportional to the size and complexity of the application. So it may start out small, but as the application grows, so will the hydration cost. With Qwik, the startup cost is 1) significantly smaller and 2) is fixed no matter the complexity of the application.

## Understanding User Interactions

So far, we have shown how to run the Qwik application and how little code is needed to make the application interactive. Now let's look into what happens upon interactivity.
...
NOTE: Opening up network tab will add `client` and `env.mjs` files. These are from Vite and are used for HMR. They will not be present in the production build and are ignored for this discussion.
...
You may want to know how does Qwik know which code to download based on the user interaction. You can do so by examining the DOM in the DevTools. Let's zoom in on the checkmark HTML. You should see something like this.

```htmlembedded=
<input q:obj="9"
       type="checkbox"
       on:click="/src/components/item/h_item_item_onrender_input_onclick.js#Item_onRender_input_onclick[0]"
       class="toggle">
```
...
- `on:click`: is a serialized version of the event handler. It describes:
...
1. Structural and event handler information is encoded in HTML directly in form of attributes or element names. (`<q:slot>` or `<div q:slot="...">`, `<button on:click="./chunk.js#symbol">`)
...
Components are declared using `component$()` function. A component optionally creates a state and return a on-render function (`return $(() => ...)`.

The unique thing to notice is the present of the `$` on `component$()` as well as on-render function `return $(() => ...)`. The presence of `$` has a specific meaning:
...
- **Optimizer**: The Optimizer uses the `$` to refactor the code and pull out the closures into top level importable symbols and place them in related chunks. The Optimizer leaves a pointer, knows as QRL, to the refactored code in that location.
- **Runtime**: The runtime uses the QRL to lazy load the symbol on as-needed basis. Many symbols such as the on-mount and on-render will never download to the client because `<Main>` component is static. No change to `todos` would require a re-render.
...
The on-mount function is responsible for returning an on-render function.
...
The on render function is wrapped in `$`, this means that it can be lazy-loaded independently from on-mount. The on-mount function captures the `state` which is the result of the on-mount function which executed on the server as part of SSR/SSG.

The runtime notices that the render function depends on `state.text`. This tells Qwik that whenever the `state.text` value changes the render function needs to be downloaded and executed. This makes Qwik reactive. The subscription information is automatic because `state` is a proxy.

### Event Listeners

Finally let's look at event listener:
...
The `on$:keyup` tells the Optimizer to automatically extract the event listener into a lazy loadable chunk. This allows the Qwik runtime delay the loading of the `keyup` handler until the user interacts with the page. The `keyup` handler modifies the `store.text` and `props.todos`. Both of these objects are stores and hence proxies. Any modification of the proxies automatically invalidates the subscribed on-render function s as described above. The Qwik knows about the subscriptions because the subscriptions were serialized as part of the server render.
...
Will allow Qwik to start prefetching symbols based on the likelihood that they will be needed in the browser. This removes any concerns over latancy on interaction over slow networks.
