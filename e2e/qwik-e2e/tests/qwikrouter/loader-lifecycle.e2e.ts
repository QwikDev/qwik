...
test.describe('loader lifecycle', () => {
  ...
  test.describe('from the navigation page', () => {
    test.beforeEach(async ({ page }) => {
    ...
    for (const responseType of ['data', 'redirect', 'http redirect']) {
      test(`ignores superseded same-page action ${responseType}`, async ({ page }) => {
        ...
        try {
          ...
          expect(
            requests.filter((url) => url.includes('q-loader-navigation-parent.'))
          ).toHaveLength(1);
    ...
    test('hash change during rendering still commits the loader registry', async ({ page }) => {
      ...
      try {
        ...
        await held.requested;
        await page.evaluate(() => {
          void (window as any).__navigateLoaderTest(
            '/qwikrouter-test/loader-navigation/42/#anchor'
          );
        });
        held.release();
        await expect(page).toHaveTitle('child 42 - Qwik');
    ...
    test('finishes the allowed navigation when a later navigation is prevented', async ({
      page,
    }) => {
      const held = await holdLoader(page, '**/q-loader-navigation-static-child.*');
      try {
    ...
    test('keeps current action results when an earlier loader response arrives', async ({
      page,
    }) => {
      await page.locator('#navigate-dynamic').click();
      await completed(page, '42/');
