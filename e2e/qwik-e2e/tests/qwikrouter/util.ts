...
export async function assertPage(ctx: TestContext, test: AssertPage) {
  ...
  if (typeof test.activeHeaderLink === 'string') {
    await expect(activeLink, `Header link "${test.activeHeaderLink}" not active`).toBeVisible();
    await expect(activeLink).toHaveText(test.activeHeaderLink, {
      useInnerText: true,
    });
  } else if (test.activeHeaderLink === false) {
    await expect(activeLink, `There should not be an active header link`).not.toBeVisible();
  }
}
...
export async function linkNavigate(ctx: TestContext, linkSelector: string, responseStatus = 200) {
  ...
  if (ctx.javaScriptEnabled) {
    ...
    expect(response?.status(), `Expected status ${responseStatus} for ${href}`).toBe(
      responseStatus
    );
  }
}
