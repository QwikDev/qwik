...
test.describe('signals', () => {
  function tests() {
    test('should do its thing', async ({ page }) => {
      ...
      await expect(parentRender).toHaveText('Parent renders: 1');
      // v3: components never rerun, so the child render count never grows
      await expect(childRender).toHaveText('Child renders: 1');
      await expect(text).toHaveText('Text: Message');
      await expect(text).toHaveAttribute('data-set', 'ref');
      await expect(id).toHaveText('Id: 1');
    ...
    test('issue 1681 - signal text node replacement and whitespace', async ({ page }) => {
    ...
    test('issue 2000 - textarea renders with html comment markers', async ({ page }) => {
    ...
    test('issue 2245-b - class reactivity signal store (b)', async ({ page }) => {
    ...
    test('issue 4249 - signals ternary unnecessary rerenders', async ({ page }) => {
    ...
    test('issue 4368 - props spreading useResource reactivity', async ({ page }) => {
