  ...
  try {
    ...
    if ((wire as { ssr?: { needsRootRange?: boolean } }).ssr?.needsRootRange === true) {
      ...
      generator.blockMarkers = [
        {
          open: `${QwikWord.CreateSsrMarkup}('<!d=', ${QwikWord.CreateSsrNodeId}(${rootRange}), '>')`,
          close: '<!/d>',
        },
      ];
    }
...
class JsComponentGenerator {
  ...
  generateProduction(
    ...
    if (contextScope !== null) {
      parts.unshift(`${QwikWord.CreateSsrMarkup}('<!c=', ${contextScope}, '>')`);
  ...
  private innerHtmlProp(
    ...
    if (segmentId === undefined) {
      ...
      this.pushStep(
        step,
        [signal],
        `${QwikWord.RenderSsrAttr}(${idVariable}, ${name}, ${signal})`,
      ...
      this.pushStep(
        step,
        captures,
        `${QwikWord.RenderSsrAttrExpression}(${idVariable}, ${name}, [${captures.join(', ')}], ${this.qrlExpression(meta, false)})`,
        this.claimId(idVariable)
      );
    }
  ...
  private op(
    ...
    switch (operation.kind) {
      ...
      case SsrOpKind.Branch: {
        ...
        parts.push(
          `${QwikWord.CreateSsrMarkup}('<!b=', ${QwikWord.CreateSsrNodeId}(${idVariable}), '>')`
        );
        parts.push(step);
        pushStatic('<!/b>');
        return;
      ...
      case SsrOpKind.Collection: {
        ...
        if (this.asyncSteps.length > 0 && operation.source.kind === 'derived') {
          ...
          this.asyncSteps.push({ name: step, expr: `${step}()` });
          parts.push(
            `${QwikWord.CreateSsrMarkup}('<!f=', ${QwikWord.CreateSsrNodeId}(${idVariable}), '>')`
          );
          parts.push(step);
          pushStatic('<!/f>');
          return;
      ...
      case SsrOpKind.DynamicSlot: {
        ...
        parts.push(
          `${QwikWord.CreateSsrMarkup}('<!d=', ${QwikWord.CreateSsrNodeId}(${idVariable}), '>')`
        );
        parts.push(step);
        pushStatic('<!/d>');
        return;
  ...
  private element(
    ...
    if (operation.propsEffect !== null) {
      ...
      this.pushStep(
        step,
        captures,
        `${QwikWord.RenderSsrProps}(${idVariable}, [${captures.join(', ')}], ${this.qrlExpression(meta, false)}, ${this.names.ctx}.eventAttr${scopeArgs})`,
        this.claimId(idVariable),
    ...
    if (foldable && open.length === 1 && isStringLiteral(open[0])) {
      ...
      this.imports.add(QwikWord.CreateSsrMarkup);
      parts.push(`${QwikWord.CreateSsrMarkup}(${open.join(', ')})`);
    }
      ...
      case 'dynamic': {
        ...
        if (segmentId === undefined) {
          ...
          this.pushStep(
            step,
            [signal],
            `${QwikWord.RenderSsrAttr}(${idVariable}, ${JSON.stringify(item.name)}, ${signal}${attrScopeArgs(scope)})`,
            this.claimId(idVariable)
          );
          open.push(
            `(${step} === null ? '' : ${JSON.stringify(` ${item.name}`)} + (${step} === '' ? '' : '="' + ${QwikWord.EscapeHTML}(${step}) + '"'))`
        ...
        this.pushStep(
          step,
          captures,
          `${QwikWord.RenderSsrAttrExpression}(${idVariable}, ${JSON.stringify(item.name)}, [${captures.join(', ')}], ${this.qrlExpression(meta, false)}${attrScopeArgs(scope)})`,
          this.claimId(idVariable)
        );
        open.push(
          `(${step} === null ? '' : ${JSON.stringify(` ${item.name}`)} + (${step} === '' ? '' : '="' + ${QwikWord.EscapeHTML}(${step}) + '"'))`
      ...
      case 'event': {
        ...
        if (dynamicIndex !== -1) {
          ...
          this.pushStep(
            step,
            captures,
            `${QwikWord.RenderSsrEvent}(${idVariable}, ${JSON.stringify(event.name)}, [${captures.join(', ')}], ${this.qrlExpression(meta, false)}, ${this.names.ctx}.eventAttr, [${before.join(', ')}], [${afterHandlers.join(', ')}])`,
            this.claimId(idVariable)
          );
          open.push(`(${step} ?? '')`);
          return true;
  ...
  private dynamicText(
    ...
    const targetArgs = `${targetIdName}, ${target.kind === 'range' ? target.marker : 'null'}`;
    this.imports.add(QwikWord.EscapeHTML);
    ...
    if (segmentId !== undefined) {
      ...
      this.pushStep(
        step,
        captures,
        `${QwikWord.RenderSsrTextExpression}(${targetArgs}, [${captures.join(', ')}], ${this.qrlExpression(meta, false)})`,
        idPrelude
      );
      ...
      this.pushStep(
        step,
        [signal],
        `${QwikWord.RenderSsrTextNode}(${targetArgs}, ${signal}${operation.stringify ? ', undefined, true' : ''})`,
        idPrelude
      );
    } else {
      markUngeneratable();
