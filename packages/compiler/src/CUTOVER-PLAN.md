...
| `link/render-results.ts`                                                            | stays one file: its core is one strongly connected component (`evaluate` → `readBinding` → `consumerMutationKinds` → `mutationKinds` → `evaluate`) and the earlier phases write into the maps the first one builds, so splitting it would mean a late-bound query object and property calls on the linker's hottest path                                       |
