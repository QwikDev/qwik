 ...
 *
 * Imported as a side-effect from root.tsx by the devtools plugin. Since it does NOT use Qwik's
 * `$()` functions, the Qwik compiler leaves it untouched (no QRL extraction).
 *
 * Exposes `window.__QWIK_DEVTOOLS_HOOK__.getVNodeTree()` which returns a serializable component
 * tree built from the live VNode structure.
 */
const vnodeBridge = `
import {
  _getDomContainer,
