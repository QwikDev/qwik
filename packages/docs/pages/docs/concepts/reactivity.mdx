...
So far, the examples show the store (`createStore()`) was a simple object with primitive values. However, the same behavior works on all properties of the store recursively.
