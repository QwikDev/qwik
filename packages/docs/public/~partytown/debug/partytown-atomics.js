...
(window => {
    ...
    (async receiveMessage => {
        ...
        return (worker, msg) => {
            ...
            })) : 2 === msg[0] ? readMainInterfaces((info => worker.postMessage([ 3, info ]))) : 11 === msgType ? receiveMessage(accessReq, (accessRsp => {
        ...
        if (onMessageHandler) {
            worker = new Worker(libPath + "partytown-ww-atomics.js?v=0.7.0-dev1663948279283", {
                name: "Partytown 🎉"
            });
            worker.onmessage = ev => {
                const msg = ev.data;
