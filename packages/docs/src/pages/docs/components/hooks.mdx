...
See also: `useClientMount$()` and `useServerMount$()` that have same semantics but only run on client or server respectively.  
...
const Cmp = component$(() => {
  ...
  useMount$(async () => {
    // This code will run on the server, then on the client
