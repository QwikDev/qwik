...
This connect with qwik's [core tenant](../think-qwik.mdx) which focus on delaying **the loading** and execution of JavaScript for as long as possible. Qwik needs to break up the application into many lazy loadable chunks to achieve that.
...
Frameworks need to download and execute components to rebuild the render tree on startup. (See [hydration is pure overhead](https://www.builder.io/blog/hydration-is-pure-overhead)) 

This forces eager download and execution of all components in the render tree.
...
Optimizer (described in-depth [here](../advanced/optimizer.mdx)) is a code transformation that extracts functions into top-level importable symbols, which allows the Qwik runtime to lazy-load the JavaScript on a needed basis.
...
following it should be lazy-loaded. (For a detailed discussion see [$ and Optimizer Rules](../advanced/optimizer.mdx).)
