...
1. The APIs contain the `$` to ensure that your code contains them.
2. The Optimizer looks for the `$` and extracts the function wrapped by `$` into a separate lazy-loadable chunk. (See the documentation to better understand how this is done.)
