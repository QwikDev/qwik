...
Next, we need to add `[qwik-react](https://qwik.builder.io/integrations/integration/react/)` so we can wrap our React components with the `qwikify$()` method:
...
In order to get the result we want, we need to use the `host:` attribute, which allows us to [listen to DOM events without hydration](https://qwik.builder.io/integrations/integration/react/#listen-to-dom-events-without-hydration).
...
- [Qwik React docs](https://qwik.builder.io/integrations/integration/react/#listening-to-react-events)
