...
<h2 id="experimentalfeatures-blockssr">blockSSR</h2>
...
<h2 id="experimentalfeatures-catchboundary">catchBoundary</h2>
...
**References:** [InlineEntryStrategy](#inlineentrystrategy), [HoistEntryStrategy](#hoistentrystrategy), [SingleEntryStrategy](#singleentrystrategy), [HookEntryStrategy](#hookentrystrategy), [SegmentEntryStrategy](#segmententrystrategy), [ComponentEntryStrategy](#componententrystrategy), [SmartEntryStrategy](#smartentrystrategy)
...
<h2 id="hoistentrystrategy">HoistEntryStrategy</h2>
...
<h2 id="hookentrystrategy">HookEntryStrategy</h2>
...
_(Optional)_ Use the TypeScript optimizer (`@qwik.dev/ts-optimizer`) instead of the Rust one. The `QWIK_OPTIMIZER` environment variable (`ts` or `rust`) overrides this, so a project can be built or tested on the other optimizer without changing its config.
...
<h2 id="experimentalfeatures-pendingboundary">pendingBoundary</h2>
...
_(Optional)_ Dynamic imports of qrl segments, the subset of `dynamicImports` the preloader follows
...
<h2 id="qwikrolldown">qwikRolldown</h2>
...
<h2 id="qwikrolldownpluginoptions">QwikRolldownPluginOptions</h2>
