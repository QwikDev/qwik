...
Creates a `ServerError` with the given status and data. Its effect depends on whether you `return` or `throw` it from a loader/action:

- `return error(status, data)` — inline failure: surfaces on the loader/action `.error` channel (a typed `ServerError`). `.value` keeps its prior/initial value and reading it does not throw. - `throw error(status, data)` — aborts the request: propagates to middleware / the error page, ending with the given status (e.g. the global 404/500 page).
