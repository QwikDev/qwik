...
```typescript
ServiceWorkerRegister: (props: { nonce?: string }) => import('packages/qwik/public').JSXOutput;
```
...
```typescript
useDocumentHead: <FrontMatter extends Record<string, unknown> = Record<string, any>>() =>
