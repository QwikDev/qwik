...
_(Optional)_ Legacy prefetch control for this \*\*`Link`\*\*.

Setting this value to \*\*`"js"`\*\* will prefetch only javascript bundles required to render this page on the client when the link becomes visible. Setting this value to \*\*`true`\*\* will prefetch both javascript bundles and route data when the link becomes visible. Setting this value to \*\*`false`\*\* will disable prefetching altogether.
...
Defaults to \*\*`"visible"`\*\*. When using the deprecated \*\*`prefetch="js"`\*\* prop, route data prefetching defaults to \*\*`"off"`\*\*.
