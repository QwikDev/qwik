...
>
> Use `return error(status, data)` and read the typed `.error` (`ServerError`) channel instead. `return fail()` still works but the `.value.failed` branch it produces is deprecated.
