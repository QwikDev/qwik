...
_(Optional)_ Server-only. Projects a thrown error into the `Error` an `<ErrorBoundary>` fallback displays during SSR; `onError$` and server logging still receive the original. It never runs on the client, so a re-derived fallback uses the default policy: full fidelity in dev, a generic message plus `digest` in production.
