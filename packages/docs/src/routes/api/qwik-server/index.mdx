...
_(Optional)_ Testing-only: fixes the container `q:instance` hash so output is byte-reproducible. The hash namespaces per-render globals, so it must stay unique per container in a real document.
