...
Inside a component render this throw is deferred: reading `.value` does not throw immediately if the same render also reads `.error`, so you can read both separately. If `.error` is never read, the error is thrown after the component finishes rendering.
