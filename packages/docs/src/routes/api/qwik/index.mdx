...
A deliberately public error: constructing one is consent to serialize and display its `data` through an `<ErrorBoundary>` fallback, unredacted, even in production. Any other thrown value is redacted to a generic message in production.
