...
<h2 id="componentbaseprops-_q_type_">"q:type"</h2>
...
<h2 id="computedsignal-abort">abort</h2>
...
>
> Use `ComputeCtx` instead.

```typescript
export type AsyncCtx<T = unknown> = ComputeCtx<T>;
```

**References:** [ComputeCtx](#computectx)
...
<h2 id="computedsignal-clear">clear</h2>
...
["q:type"?](#componentbaseprops-_q_type_)
...
_(Optional)_ Reported to a parent that reads `props.children`; never rendered.
...
<h2 id="computectx">ComputeCtx</h2>
...
[abort(reason)](#computedsignal-abort)
...
[invalidate(info)](#computedsignal-invalidate)
...
[promise()](#computedsignal-promise)
...
[trigger()](#computedsignal-trigger)
...
Compiler-only fragment marker with a callable JSX type.
...
[Edit this section](https://github.com/QwikDev/qwik/tree/main/packages/qwik/src/core/shared/jsx/compiler-runtime.ts)
...
<h2 id="native_">native$</h2>

Declares native implementations for `impl`. Returns the JS implementation unchanged, so the function behaves identically whether or not a native engine is in play.

```typescript
native$: <TYPE extends Function>(impl: TYPE, targets: NativeTargets) => TYPE;
```
...
[Edit this section](https://github.com/QwikDev/qwik/tree/main/packages/qwik/src/core/shared/native/native.public.ts)

<h2 id="nativecode">nativeCode</h2>

Inline target source. Interpolation is rejected at compile time: the compiler reads the text statically, so the template can only hold literal source.

```typescript
nativeCode: (source: TemplateStringsArray, ...values: unknown[]) =>
  NativeSource;
```
...
<h2 id="nativefrom">nativeFrom</h2>

Target source kept on disk beside the module: a source file, or a directory holding a package for that language (a `Cargo.toml` crate for Rust). Only a package may pull in dependencies.

```typescript
nativeFrom: (path: string) => NativeSource;
```
...
<h2 id="nativesource">NativeSource</h2>
...
_(Optional)_ Path to a source file or a package directory, relative to the module declaring it.
...
_(Optional)_ Inline source text.
...
<h2 id="nativetargets">NativeTargets</h2>
...
**References:** [NativeSource](#nativesource)
...
<h2 id="computedsignal-promise">promise</h2>
...
[Edit this section](https://github.com/QwikDev/qwik/tree/main/packages/qwik/src/core/dom/content/content.ts)
...
<h2 id="computedsignal-trigger">trigger</h2>
...
(ctx: [ComputeCtx](#computectx)&lt;Awaited&lt;T&gt;&gt;) =&gt; T
...
[ComputedOptions](#computedoptions)&lt;Awaited&lt;T&gt;&gt;
...
[ComputedSignal](#computedsignal)&lt;Awaited&lt;T&gt;&gt;
...
A unique id per render environment: the server counts per request, the client per container, and the prefix keeps the two apart after resume.
