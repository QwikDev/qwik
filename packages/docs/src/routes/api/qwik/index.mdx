...
```
interface MyComponentProps &lcub;
  someProp: string;
&rcub;
const MyComponent: Component<MyComponentProps> = component$((props: MyComponentProps) => &lcub;
  return <span>&lcub;props.someProp&rcub;</span>;
&rcub;);
```

```typescript
export type Component<PROPS extends &lcub;&rcub;> = FunctionComponent<PublicProps<PROPS>>;
```
...
```tsx
export interface CounterProps &lcub;
  initialValue?: number;
  step?: number;
&rcub;
export const Counter = component$((props: CounterProps) => &lcub;
  const state = useStore(&lcub; count: props.initialValue || 0 &rcub;);
  return (
    <div>
      <span>&lcub;state.count&rcub;</span>
      <button onClick$=&lcub;() => (state.count += props.step || 1)&rcub;>+</button>
    </div>
  );
&rcub;);
```

- `component$` is how a component gets declared. - `&lcub; value?: number; step?: number &rcub;` declares the public (props) interface of the component. - `&lcub; count: number &rcub;` declares the private (state) interface of the component.

The above can then be used like so:

```tsx
export const OtherComponent = component$(() => &lcub;
  return <Counter initialValue=&lcub;100&rcub; />;
&rcub;);
```
...
```typescript
component$: <PROPS extends &lcub;&rcub;>(onMount: OnRenderFn<PROPS>) => Component<PROPS>
```
...
```typescript
componentQrl: <PROPS extends &lcub;&rcub;>(componentQrl: QRL<OnRenderFn<PROPS>>) => Component<PROPS>
```
...
export const App = component$(() => &lcub;
  useContextProvider(
    TodosContext,
    useStore<TodosStore>(&lcub;
...
export const Items = component$(() => &lcub;
  const todos = useContext(TodosContext);
  return (
    <ul>
      &lcub;todos.items.map((item) => (
        <li>&lcub;item&rcub;</li>
      ))&rcub;
    </ul>
  );
&rcub;);
...
```typescript
Fragment: FunctionComponent<&lcub;
    children?: any;
...
- `component$(() => &lcub;...&rcub;)` is same as `component($(() => &lcub;...&rcub;))`

```tsx
export function myApi(callback: QRL<() => void>): void &lcub;
  // ...
&rcub;
...
```typescript
export const OtherComponent = component$(() => &lcub;
  return $(() => <Counter value=&lcub;100&rcub; />);
&rcub;);
```
...
```typescript
export type PublicProps<PROPS extends &lcub;&rcub;> = TransformProps<PROPS> & ComponentBaseProps & ComponentChildren<PROPS>;
```
...
```typescript
RenderOnce: FunctionComponent<&lcub;
    children?: any;
    key?: string | number | null | undefined;
&rcub;>
```
...
```tsx
const Cmp = component$(() => &lcub;
  const store = useStore(&lcub;
    city: '',
  &rcub;);

  const weatherResource = useResource$<any>(async (&lcub; track, cleanup &rcub;) => &lcub;
    const cityName = track(() => store.city);
    const abortController = new AbortController();
    cleanup(() => abortController.abort('cleanup'));
    const res = await fetch(`http://weatherdata.com?city=$&lcub;cityName&rcub;`, &lcub;
  ...
  return (
    <div>
      <input name="city" onInput$=&lcub;(ev: any) => (store.city = ev.target.value)&rcub; />
      <Resource
        value=&lcub;weatherResource&rcub;
        onResolved=&lcub;(weather) => &lcub;
          return <div>Temperature: &lcub;weather.temp&rcub;</div>;
        &rcub;&rcub;
      />
    </div>
  );
...
```typescript
SSRComment: FunctionComponent<&lcub;
    data: string;
&rcub;>
```
...
```typescript
SSRStreamBlock: FunctionComponent<&lcub;
    children?: any;
&rcub;>
```
...
```tsx
const Cmp = component$(() => &lcub;
  const store = useStore(&lcub; count: 0, doubleCount: 0 &rcub;);
  useTask$((&lcub; track &rcub;) => &lcub;
  ...
  return (
    <div>
      <span>
        &lcub;store.count&rcub; / &lcub;store.doubleCount&rcub;
      </span>
      <button onClick$=&lcub;() => store.count++&rcub;>+</button>
    </div>
  );
&rcub;);
```
...
```tsx
const Stores = component$(() => &lcub;
  const counter = useCounter(1);

  // Reactivity happens even for nested objects and arrays
  const userData = useStore(&lcub;
    name: 'Manu',
    address: &lcub;
      address: '',
      city: '',
  ...
  return (
    <div>
      <div>Counter: &lcub;counter.value&rcub;</div>
      <Child userData=&lcub;userData&rcub; state=&lcub;state&rcub; />
    </div>
  );
&rcub;);

function useCounter(step: number) &lcub;
  // Multiple stores can be created in custom hooks for convenience and composability
  const counterStore = useStore(&lcub;
    value: 0,
  &rcub;);
  useVisibleTask$(() => &lcub;
    // Only runs in the client
    const timer = setInterval(() => &lcub;
...
export const CmpStyles = component$(() => &lcub;
  useStyles$(styles);
...
export const CmpScopedStyles = component$(() => &lcub;
...
const Timer = component$(() => &lcub;
  ...
  return <div>&lcub;store.count&rcub;</div>;
&rcub;);
```
