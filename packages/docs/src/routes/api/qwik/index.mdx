...
`worker$()` is equivalent to calling [workerQrl()](#workerqrl) with the function converted to a `QRL` as the first argument. Use it for CPU-heavy client-side work that would otherwise block the main thread, such as parsing, hashing, indexing, or image processing.

```typescript
worker$: <T extends (...args: any[]) => any>(qrl: T) => QRL<T>;
```
...
Use `workerQrl()` when you already have a `QRL` and want its work to run off the main thread. For most application code, prefer [worker$()](#worker_), which is the ergonomic shorthand.

Arguments and return values must be serializable by Qwik. DOM nodes and browser events should not be passed directly. If a form submit event is provided, it is converted to `FormData` before it is serialized to the worker.

The wrapped function runs in the worker context, so it must not depend on component hooks or direct DOM access.

```typescript
workerQrl: <T extends (...args: any[]) => any>(fnQrl: QRL<T>) => QRL<T>;
```
...
A QRL proxy whose invocation resolves with the worker result.
