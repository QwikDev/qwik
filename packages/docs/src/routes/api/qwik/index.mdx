...
```typescript
export declare function h<TYPE extends string | FunctionComponent<PROPS>, PROPS extends {} = {}>(
  type: TYPE,
  props?: PROPS | null,
  ...children: any[]
): JSXNode<TYPE>;
...
```typescript
implicit$FirstArg: <FIRST, REST extends any[], RET>(fn: (qrl: QRL<FIRST>, ...rest: REST) => RET) =>
  (qrl: FIRST, ...rest: REST) =>
    RET;
```
...
```typescript
useAsyncComputed$: <T>(qrl: AsyncComputedFn<T>, options?: ComputedOptions | undefined) =>
  AsyncComputedReturnType<T>;
```
...
```typescript
useStore: <STATE extends object>(initialState: STATE | (() => STATE), opts?: UseStoreOptions) =>
  STATE;
```
