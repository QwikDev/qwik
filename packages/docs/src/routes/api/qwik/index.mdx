...
[QRL](#qrl-type-alias)&lt;(error: unknown, reset: [QRL](#qrl-type-alias)&lt;() =&gt; void&gt;) =&gt; [JSXOutput](#jsxoutput)&gt;

</td><td>

Rendered when a descendant throws; receives `(error, reset)`. Add a live region for screen-reader announcement. Invoke `reset` wrapped in a handler — `onClick$={() => reset()}`, not `onClick$={reset}` — so it stays wired in a streamed fallback after resume.
...
[QRL](#qrl-type-alias)&lt;(error: unknown, info: [ErrorBoundaryInfo](#errorboundaryinfo)) =&gt; void&gt;

</td><td>

_(Optional)_ Side-effect fired once per caught error; never affects rendering.
