...
Backpatching enables SSR-correct relationships (like `aria-*` attributes) between components whose render order you can't control—common in libraries. It patches attributes after streaming, letting components link to each other without needing to hydrate or enforce a specific order.
