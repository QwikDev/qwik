...
<Note>The worst kind of code magic is the kind that's invisible to the developer.</Note>
...
<LongNote title="Closure Capture Rule">
If a function lexically captures a variable (or parameter), that variable must be:
