...
Only `$()` segments are part of this graph. A regular dynamic `import()` in your code or in a library is never preloaded by Qwik; you (or the library) are responsible for its timing, for example with `<link rel="modulepreload">` or by calling `import()` early. Once the imported code runs, its own `$()` segments join the preloading as usual.
