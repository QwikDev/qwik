...
<Note>The Optimizer is written in Rust (and available as WASM) for instant performance.</Note>
...
## Suppressing optimizer diagnostics

Some optimizer diagnostics can be suppressed for a single line with [`@qwik-disable-next-line`](../disable-next-line/index.mdx).

Use it when a warning is intentional and you want to silence only that specific occurrence.
