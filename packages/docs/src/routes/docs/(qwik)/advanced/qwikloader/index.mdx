...
<html>
  <body q:base="/build/">
    <button q-e:click="./myHandler.js#clickHandler">push me</button>
    <script>
      /* Qwikloader */
    </script>
  </body>
...
3. The Qwikloader retraces the event path and searches for `q-e:click` attribute on the elements.
4. The Qwikloader uses the `q-e:click` and `q:base` attributes along with the `document.baseURI` to build a full URL for fetching the lazy-loaded handler. Assuming the original page was served up from `http://localhost/` the fetch URL becomes `http://localhost/build/myHandler.js`.
...
```html
<div>
  <button q-e:click="./chunk-a.js#Counter_button_onClick[0]">0</button>
</div>
```

The critical thing to notice is that Qwik generated an `q-e:click` attribute, containing the value `./chunk-a.js#Counter_button_onClick[0]`. In the above example the `q-e:click` attribute solves the listener location problem, and the attribute value solves the listener code problem. By serializing the listeners into the HTML Qwik applications do not need to perform hydration on startup.
