...
Components in Qwik are very similar to React components. They are functions that return JSX. However the are some differences.
...
export const MyComponent = component$((props) => {
  ...
  return (
    <>
      <button
        onClick$={() => count.value = count.value + props.step}
      >
