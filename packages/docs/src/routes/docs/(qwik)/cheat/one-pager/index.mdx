...
Components in Qwik are very similar to React components. They are functions that return JSX. However, `component$((props) => {...})` needs to be used, event handlers need to have the `$` suffix, state is created using `useSignal()` , `class` is used instead of `className` and some other differences.
...
export const Button = component$(() => {
  // Project the content of the button into the slot.
  return (
...
Methods that start with `use` are special in Qwik, such as `useSignal()`, `useStore()`, `useOn()`, `useTask$()`, `useLocation()` and so on.
...
Qwik supports out of the box, CSS modules, global CSS imports and lazy loading scoped CSS using `useStyleScoped$()`.
...
```css
/* File: src/components/MyComponent/MyComponent.module.css */
.container {
...
To use global CSS, simply import the CSS file from any typescript file. For example, `src/components/MyComponent/MyComponent.tsx`.
...
It can also import an external CSS file. For that you need to add the `?inline` query parameter to the import of the CSS file, and pass the default export of the CSS file to the `useStyleScoped$()` hook.

```css
/* File: src/components/MyComponent/MyComponent.css */
.container {
  background-color: red;
}
```
...
The `$(...)` function and any function that end with `$` are special in Qwik, such as: `$()`, `useTask$()`, `useVisibleTask$()`... The `$` at the end represents a lazy loading boundary. There are some rules that apply to the first argument of any `$` function.
...
`useSignal()` is the main way to create reactive state. Signals can be shared across components, and any component or task that reads the signal (executing: `signal.value`) will be render when the signal changes.
...
`useStore(initialValue?)` is similar to `useSignal` except that it creates a reactive javascript object, where making every property of the object becomes reactive, just the `value` of a signal. Under the hood, `useStore` is implemented using a `Proxy` object, that intercepts all property access making them reactive.
...
In practive, useSignal and useStore are very similar, but useSignal is prefered most of the times. `useSignal(0) === useStore({ value: 0 })`, some use cases to use `useStore` are:

- When you need reactivity in an array.
- When you want a reactive object that you can easily add properties to.
- When you want to use the `deep` option to make nested properties reactive.
...
`useTask$` is used to create a async task. Tasks are useful to implement side effects, running heavy computation and async code as part of the rendering lifecycle. `useTask$` execute before the first render, and then whenever a tracked signal or store changes, the task is re-executed.
...
export const Counter = component$(() => {
  ...
  useTask$(async (taskContext) => {
    // Since we want to re-run the task whenever the `page` changes, we need to track it.
...
Since Qwik does NOT execute any Javascript on the browser before user interaction. `useVisibleTask$()` is the only API that will run execute eagarly on the client, which is why it's a good place to do things like:
...
Notice that `useVisibleTask$()` should not be used to fetch data, because it will not run on the server. Instead, you should use `useTask$()` to fetch data, and then use `useVisibleTask$()` to do things like start an animation. Abusing `useVisibleTask$()` can lead to a bad performance.


## Routing

Qwik comes with a file-based router, which is similar to Next.js, but with a few differences. The router is based on the file-system, specifically in `src/routes/`. Creating a new `index.tsx` file in `src/routes/` will create a new route. For example, `src/routes/home/index.tsx` will create a route at `/home/`.
...
```tsx
// File: src/routes/user/[userID]/index.tsx
...
export default component$(() => {
  ...
  return (
    <main>
      <h1>User data</h1>
      <div>
        User name: {userData.value.user.name}
      </div>
      <div>
        User email: {userData.value.user.email}
      </div>
    </main>
  );
});
...
Qwik provides the `routeAction$()` API exported from `@builder.io/qwik-city` to handle form requests in the server. `routeAction$()` is very similar to `routeLoader$()`, both are used to create a custom hook that can be used to access data returned from the server. The main difference is that `routeAction$()` is only executed when the form is submitted, while `routeLoader$()` is executed on the server before the route is rendered.
  ...
  return (
    <main>
      ...
      <Form action={userData}>
        <div>
          User name: <input name="name" defaultValue={userData.formData?.get('name')} />
        </div>
        <div>
          User email: <input name="email" defaultValue={userData.formData?.get('email')} />
        </div>
...
## Run code only on the browser

Qwik executes the same code in the server and in the browser. This can be a problem if you need to run code that is only available in the browser, such as accessing the DOM, or using browser APIs. To solve this problem, Qwik provides a few APIs to run code only on the browser.
...
export default component$(() => {
  useVisibleTask$(() => {
    // This code will only run on the browser right after the page is loaded
  });


  useTask$(() => {
    if (isBrowser) {
      // This code will only run on the browser only when the component is first rendered in the browser
    }
  });

  return (
    <button onClick$={() => {
      // This code will only run on the browser when the button is clicked
    }}>Click me</button>
  );
});
```
...
This API will declare a VisibleTask that is ensured to run only at least once on the client. It also executes eagarly on the client as soon as the page is loaded.
...
Sometimes you need to run code only on the server, such as fetching data, or running analytics code. To solve this problem, Qwik provides a few APIs to run code only on the server.
