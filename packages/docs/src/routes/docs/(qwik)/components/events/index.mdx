...
To reuse the same event handler for multiple elements or events, you have to wrap the event handler into the `$()` function exported by `@qwikdev/core`. This transforms it into a [`QRL`](/docs/advanced/qrl/).
