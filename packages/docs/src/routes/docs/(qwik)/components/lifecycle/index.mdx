...
Thanks to [Resumability](/docs/(qwik)/concepts/resumable/index.mdx), components life and its lifecycle extend across server and browser. Sometimes the component will be first rendered in the server, sometimes it could be in the browser, however, in both cases the lifecycle will be the same, only its execution happens in different environments.
...
Additionally, this task can be reactive and re-execute when some **tracked** [signal or store](/docs/(qwik)/components/state/index.mdx) changes, like this:
...
> Note, if you're thinking about load data (like using `fetch()`) inside of `useTask$`, consider using [`useResource$()`](/docs/(qwik)/components/resource/index.mdx) instead. This API is more efficient in terms of leveraging SSR streaming and parallel data fetching.
...
> But as mentioned above - for data fetching, consider using [`useResource$()`](/docs/(qwik)/components/resource/index.mdx) instead.
...
`useVisibleTask$()` registers a hook to be executed when the component becomes visible in the viewport, it will run at least once in the browser, and it can be reactive and re-execute when some **tracked** [signal or store](/docs/(qwik)/components/state/index.mdx) changes, like this:
