...
> The reason for appending `$` to `component$` is to allow the [Optimizer](/docs/advanced/dollar/) to break the components into an application tree. Each separate chunk in the tree can then be independently loaded as needed. Without the `$` the component would always load if the parent component needs to be loaded.
