...
> In the above example, the Parent component passes a changing `name` property to the Child component. The Child component will only re-render when the count signals changes.
