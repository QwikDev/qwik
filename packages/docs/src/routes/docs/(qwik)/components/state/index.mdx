...
The `signal` can be shared with other components which can observe or update it.  When the `.value` property is modified, the signal will notify all components that depend on it.
