...
Reactive state management is an approach to managing the state of an application in response to changes in data or user interactions.

In Qwik:

  - Reactive state is created using the `useSignal()` or `useStore()` functions, (aka hooks).
  - Reactive states can be shared with other components across the application to update them in response to changes.

## `useSignal()`

The `useSignal()` hook accepts an `initial value`, creates, and returns a `reactive state`, also known as a `signal`. This signal is an object with a single property, `.value`.

Multiple components can consume the same signal. When any component with access to the signal modifies the `.value` property, all dependent components will automatically update in response to the change.
...
To create a reactive object, use the `useStore(initialStateObject)` hook. This hook takes an initial object and returns a reactive object.  Alternatively, `userStore(() => initialStateObject)` can be used to create a reactive object lazily.
...
export default component$(() => {
  const store = useStore(
    ...
    { deep: true } // note: no recursion `depth limit`
  );
