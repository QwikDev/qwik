...
A typical use of `useResource$()` is to retrieve data from an external API directly within the component, which can occur either on the server or the client.
