...
The reactive object returned by [`useStore()`](/docs/(qwik)/components/state/index.mdx#usestore) is just like any other object, but it is reactive. If you change any property of the object, any component that depends on that property will be updated.
...
By default, [`useStore()`](/docs/(qwik)/components/state/index.mdx#usestore) only tracks the top level fields in your store, which means that for any updates to be registered, you have to update values at the top level field.
