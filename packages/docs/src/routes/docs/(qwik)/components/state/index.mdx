...
The `useSignal()` hook takes an initial value and generates a reactive state or signal. The signal is an object that consists of a sole property called `.value`. 

The `signal` can be shared with other components which can observe and/or update it.  When the `.value` property is modified, the signal will notify all components that depend on it.
