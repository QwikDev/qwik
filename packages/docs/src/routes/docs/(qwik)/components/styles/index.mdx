...
To use scoped CSS, you can use the `useStylesScoped$()` hook exported from `@qwikdev/core`.
...
```tsx
import { useStylesScoped$, component$ } from '@qwikdev/core';
...
```tsx
import { useStyles$, component$ } from '@qwikdev/core';
