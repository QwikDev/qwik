...
- They are [reactive](../../concepts/reactivity/index.mdx) and render independently of other components on the page. See [rendering](../rendering/index.mdx).
...
<Note>The `component$` function, marked by the trailing `$`, enables the Optimizer to split components into separate chunks. This allows each chunk to be loaded independently as needed, rather than loading all components whenever the parent component is loaded. index.tsx, layout.tsx in routes folder, root.tsx and all entry files need **export default**. For other components you can use export const and export function.</Note>
...
<CodeSandbox src="/src/routes/demo/component/child/index.tsx">
```tsx {7}
import { component$ } from '@qwik.dev/core';
...
<Note>Qwik components are already lazy loaded thanks to the `$` sign. That means that you don't need to dynamically import the child component manually, Qwik will do it for you.</Note>
...
<CodeSandbox src="/src/routes/demo/state/counter/index.tsx">
```tsx
...
<Note>In the examples above, we are using `component$<ItemProps>` to provide an explicit type for the props. This is optional, but it allows the TypeScript compiler to check that the props are used correctly.</Note>
...
<Note>Getting a reference to DOM elements can be useful to calculate the element size (`getBoundingClientRect`), computed styles, initializing a WebGL canvas, or even wire-up some third-party library that interacts with DOM elements directly.</Note>
...
<Note>For simplicity, not all of the transformations are shown; all resulting symbols are kept in the same file for succinctness.</Note>
...
- [`useSignal(initialState)`](../state/index.mdx#usesignal) - creates a signal with a value you can set
- [`useStore(initialStateObject)`](../state/index.mdx#usestore) - creates an object for grouped state; Qwik tracks its property reads and writes
- [`useComputed$()`](../state/index.mdx#usecomputed) - creates a signal with a value calculated (synchronously or asynchronously) from other values
...
- [`<Pending>`](/docs/labs/pending/index.mdx) - shows fallback UI while part of the page waits
- [`<Catch>`](/api/qwik/#catch) - renders a fallback when a descendant throws
