...
> `useAsync$` has built-in support for polling with `pollMs`, which allows you to re-run the async function at a specified interval, as long as it is used somewhere.
...
function useAsync$<T>(
  ...
  options?: { initial?: T; pollMs?: number; concurrency?: number; timeout?: number }
): AsyncSignal<T>;
```
...
- `pollMs`: if provided, the async function will be re-run every `pollMs` milliseconds, as long as the async signal is used somewhere. This is useful to keep data fresh.
