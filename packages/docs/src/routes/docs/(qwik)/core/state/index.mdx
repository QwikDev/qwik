...
> **NOTE** Signal reads in `useComputed$()` are tracked automatically. In an async computation on Qwik v1, only reads before the first `await` are tracked, so read reactive inputs before awaiting.

### `useResource$()`

Use `useResource$()` to create a value from an external async source when you need the resource state (loading, resolved, rejected) in addition to the value.
