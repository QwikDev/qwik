...
While the async work is running, reading `.value` makes that part of the UI wait, and a [`<Pending>`](/docs/labs/pending/index.mdx) boundary can show fallback UI. If the work fails, reading `.value` re-throws the error, and a [`<Catch>`](/docs/labs/catch/index.mdx) can render a fallback for it.
...
Below, `useComputed$()` fetches programming jokes for the current query. Reading `query.value` reruns the function when the query changes, and `abortSignal` cancels any previous request. `JokeList` reads `jokes.value` inside `<Pending>` and `<Catch>`.

<CodeSandbox src="/src/routes/demo/state/async-joke/index.tsx" style={{ height: '20em' }}>
```tsx /useComputed$/ /query.value/ /abortSignal/ /Pending/ /Catch/
import {
  $,
  component$,
  Catch,
...
`<Pending>` shows the loading fallback until the first result arrives, and `<Catch>` renders the error if the request fails. During SSR, Qwik waits for the async work before rendering the result.

<Note title="Async computed values and Pending">
