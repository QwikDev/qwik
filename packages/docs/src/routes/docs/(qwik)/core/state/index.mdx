...
`useComputed$()` creates a signal for those values. The calculation can be synchronous, such as formatting a name, or asynchronous, such as fetched data or Web Worker output. Qwik automatically tracks every signal and store the function reads and re-runs it when they change.
...
Auto-tracking keeps working across `await`: read `signal.value` or `store.prop` directly, before or after awaiting, and Qwik re-runs the function when they change. You do not need `track()`.
...
`useAsync$()`, `createAsync$()`, and the `AsyncSignal` type are deprecated. Use `useComputed$()`, `createComputed$()`, and `ComputedSignal` instead. The only difference was the manual `track()` call, which auto-tracking now replaces.
</Note>
...
The function reads reactive state directly (`signal.value`, `store.prop`); Qwik tracks those reads automatically, including after `await`. Context:
