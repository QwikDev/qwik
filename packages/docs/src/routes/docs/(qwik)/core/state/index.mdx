...
While the async work is running, reading `.value` makes that part of the UI wait, and a [`<Suspense>`](/docs/labs/suspense/index.mdx) boundary can show fallback UI. If the work fails, reading `.value` re-throws the error, and an [`<ErrorBoundary>`](/docs/labs/error-boundary/index.mdx) can render a fallback for it.
...
Below, `useComputed$()` fetches programming jokes for the current query. Reading `query.value` reruns the function when the query changes, and `abortSignal` cancels any previous request. `JokeList` reads `jokes.value` inside `<Suspense>` and `<ErrorBoundary>`.

<CodeSandbox src="/src/routes/demo/state/async-joke/index.tsx" style={{ height: '20em' }}>
```tsx /useComputed$/ /query.value/ /abortSignal/ /Suspense/ /ErrorBoundary/
import {
  $,
  component$,
  ErrorBoundary,
...
`<Suspense>` shows the loading fallback until the first result arrives, and `<ErrorBoundary>` renders the error if the request fails. During SSR, Qwik waits for the async work before rendering the result.

<Note title="Async computed values and Suspense">
When the query changes, `jokes.value` keeps returning the previous jokes until the new ones arrive, so the fallback only shows on the first load. Call `jokes.clear()` when readers should see the fallback again while the new value computes.
</Note>
...
Most apps only need `.value`. The full API also includes options for cleanup, cancellation, timeouts, and manual invalidation.
