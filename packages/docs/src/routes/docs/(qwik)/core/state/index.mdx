...
`useComputed$()` creates a signal for those values. The calculation can be synchronous, such as formatting a name, or asynchronous, such as fetched data or Web Worker output. Qwik automatically tracks every signal and store the function reads and re-runs it when they change (in async functions, reads after the first `await` need [`track()`](#async-computed-values)).
...
Create a signal for a value that is calculated from other state.

Qwik keeps the result and runs the function again only when one of the signals it reads changes. The function can be synchronous (shown here) or asynchronous (see [Async computed values](#async-computed-values) below).
...
The function passed to `useComputed$()` can be `async` (or return a Promise), for values that come from fetching data, calling `server$()`, reading from a Web Worker, or running a long calculation.

Auto-tracking only covers reads before the first `await`: read `signal.value` or `store.prop` at the top of the function and Qwik re-runs it when they change. After the first `await` the tracking context is gone, so reads there are not tracked. To track those, use the `track()` function provided on the context argument:

```tsx
const user = useComputed$(async ({ track }) => {
  // Tracked automatically: read before the first await.
  const id = userId.value;
  const res = await fetch(`/api/users/${id}`);
  // Tracked explicitly: after an await, use track().
  const locale = track(localeSignal);
  return formatUser(await res.json(), locale);
});
```

While the async work is running, the signal is pending. Read `.pending` for the state, and `.value` or `.error` for the result. If you read `.value` before it is ready, that part of the UI can wait and a [`<Suspense>`](/docs/labs/suspense/index.mdx) boundary can show fallback UI.

The async work can run during SSR and in the browser. For values that need to stay up to date, poll the signal with `usePoll` from `@qwik.dev/utils`.

<Note title="Migrating from useAsync$">
`useAsync$()`, `createAsync$()`, and the `AsyncSignal` type are deprecated. Use `useComputed$()`, `createComputed$()`, and `ComputedSignal` instead. Synchronous reads before the first `await` are now tracked automatically; `track()` is still provided on the context argument for reads after an `await`.
</Note>

#### Example

Below, `useComputed$()` fetches programming jokes for the current query. Reading `query.value` reruns the function when the query changes, and `abortSignal` cancels any previous request.

<CodeSandbox src="/src/routes/demo/state/async-joke/index.tsx" style={{ height: '20em' }}>
```tsx /useComputed$/ /query.value/ /abortSignal/
import { component$, useComputed$, useSignal } from '@qwik.dev/core';
...
The code handles pending and error states inline with `.pending` and `.error`. During SSR, Qwik waits for the async work before rendering the result.

<Note title="Async computed values and Suspense">
The jokes example handles several UI states inline: loading, error, no results, and results.

That is useful when each state needs its own markup. If you only need fallback UI while `.value` is not ready, wrap that part of the UI in [`<Suspense>`](/docs/labs/suspense/index.mdx).
</Note>

<LongNote title="Advanced: channel example">
`useComputed$()` can also keep its own value updated after the first result. For example, a Server-Sent Events connection can write new messages into the signal.

```tsx
import { component$, useComputed$ } from '@qwik.dev/core';

export default component$(() => {
  const state = useComputed$<string>(
    ({ abortSignal }) => {
...
`eagerCleanup` closes the connection as soon as nothing is reading the signal.
</LongNote>

<LongNote title="API reference">
Most apps only need `.value`, `.pending`, and `.error`. The full API also includes options for cleanup, cancellation, timeouts, and manual invalidation.
...
The function reads reactive state directly (`signal.value`, `store.prop`); Qwik tracks those reads automatically until the first `await`. Context:

- `track`: tracks reads explicitly. Required for reads after the first `await`, where automatic tracking stops.
...
- `info`: the value passed to `signal.invalidate(info)`.
...
- `pending`: whether the async function is running.
...
- `invalidate()`: marks the signal to run again; readers keep seeing the previous value while it recomputes.
- `clear()`: like `invalidate()`, but drops the value so readers suspend until the new value is ready.

To re-run the signal on an interval, use `usePoll(signal, ms)` from `@qwik.dev/utils`: it starts polling once the document is idle and resets the timer whenever a new result lands.
</LongNote>
