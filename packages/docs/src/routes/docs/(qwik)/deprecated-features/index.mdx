---
title: Deprecated Features | Guides
contributors:
  - nsdonato
  - mrhoodz
  - manucorporat
updated_at: '2023-09-26T19:56:40Z'
created_at: '2023-06-14T10:39:06Z'
---

# Deprecated features

These features were deprecated in the preparation of Qwik for its stable release.

If you happened to come across one of them and don't know what its replacement is, we quickly mention them in this table with the link to their new documentation ✨.

## Qwik

### Deprecated functions and their replacements

----

| Deprecated   | Replacements     |
| :-------- | :-------
| `useWatch$` | [useTask$](https://qwik.builder.io/docs/components/tasks/#usetask) |
| `useMount$` | [useTask$](https://qwik.builder.io/docs/components/tasks/#usetask) |
| `useServerMount` | [useTask$](https://qwik.builder.io/docs/components/tasks/#usetask) + [isServer](https://qwik.builder.io/docs/guides/qwik-nutshell/#isserver-conditional)
| `useClientMount` | [useTask$](https://qwik.builder.io/docs/components/tasks/#usetask) + [isBrowser](https://qwik.builder.io/docs/guides/qwik-nutshell/#isbrowser-conditional)
| `useClientEffect` - `useClientEffectQrl` | [useVisibleTask$](https://qwik.builder.io/docs/components/tasks/#usevisibletask) |
| `useBrowserVisibleTask` - `useBrowserVisibleTaskQrl` | [useVisibleTask$](https://qwik.builder.io/docs/components/tasks/#usevisibletask) |
| `useEnvData` | [useServerData](https://qwik.builder.io/api/qwik/#useserverdata) |
| `useRef` | [useSignal](https://qwik.builder.io/docs/components/state/#usesignal) |
| `createContext` | [createContextId](https://qwik.builder.io/docs/components/context/#createcontextid) |

## Qwik City

### Deprecated functions and their replacements
----

| Deprecated   | Replacements
| :-------- | :-------
| `useEndpoint ` | [routeLoader$](https://qwik.builder.io/docs/route-loader/#routeloader)
| `loader$` - `loaderQrl` | [routeLoader$](https://qwik.builder.io/docs/route-loader/#routeloader)
| `action$` - `actionQrl`  | [routeLoader$](https://qwik.builder.io/docs/route-loader/#routeloader)
