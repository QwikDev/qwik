...
Javascript streaming would theoretically be achievable with some sort of on-demand hydration, but that would require re-running all side effects on the client, which would force shipping a lot more code to the client and downloading it eagerly on startup, therefore slowing down TTI.

If you’re already familiar with Qwik, this would be the equivalent of doing all your side effects solely in ‘useVisibleTask$’ hooks and downloading eagerly all those chunks on startup, but worse since the bundles would most likely include the rest of the component’s code.
...
<Note>In short, Qwik uses vDOM, but significantly less than React in comparable situations.</Note>
