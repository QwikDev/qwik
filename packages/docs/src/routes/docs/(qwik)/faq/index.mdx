...
- React uses vDOM for everything all the time, and is part of the reason why it is slow.

- Qwik uses vDOM only sometimes, and other times it does what Solid does.

The way to think about it is as following:

1. If the change in state does not have a structural change then Qwik will most likely not use vDOM. For example: 
...
export const NoStructuralChange = component$(() => {
  ...
  return (
    <>
    {/* This will not cause vDOM to activate. (No DOM structure change, only update value) */}
...
2. Qwik will use vDOM when the change is structural. In this following example the DOM structure needs to be updated and so vDOM will be used for rendering:
...
3. The big thing to understand (and the reason vDOM is not a performance problem in Qwik) is that in React when you invalidate a root component the vDOM for the whole tree gets created. In Qwik the decision is made on per component basis.  And only for components which have structural change AND are actualy changing their structure. If a component is structural (vDOM) but no change in structure is detected, then Qwik does nothing.
...
Because vDOM has attractive DX properties, especially when you need dynamic components. For example:
