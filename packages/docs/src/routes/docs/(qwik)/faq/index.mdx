...
You can easily deploy a Qwik App in any [serverless environment thanks to our adapters](/deployments/index.mdx), we also support a [vanilla-node adaptor](/deployments/node/index.mdx) for Node.js-based servers, such as Express.

You can deploy your Qwik app as a static site, if there is no need for SSR, thanks to our [SSG (Static Site Generation) adaptor](/docs/(qwikcity)/guides/static-site-generation/index.mdx).
