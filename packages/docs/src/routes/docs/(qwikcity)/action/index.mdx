...
Qwik comes with built-in support for [Zod](https://zod.dev/) and [Valibot](https://valibot.dev): TypeScript-first schema validations that can be used directly with actions, using the `zod$()` and `valibot$()` functions.

Actions + [Zod](https://zod.dev/) or [Valibot](https://valibot.dev) allow you to create type-safe forms, where data is validated server-side before actions are executed.

```tsx {16-20} /firstName/#a /lastName/#b title="src/routes/index.tsx"
import { component$ } from '@builder.io/qwik';
import { routeAction$, zod$, z, valibot$, v, Form } from '@builder.io/qwik-city';
...
The ```valibot$``` constructor will take function too. It works the same as its zod counterpart. 
...
```tsx /fail/
import { routeAction$, zod$, z, valibot$, v } from '@builder.io/qwik-city';
...
Failures are stored in the `action.value` property, just like the success value. However, the `action.value.failed` property is set to `true` when the action fails. Futhermore, failure messages can be found in the `fieldErrors` object (when using Zod) or the `nested` object when using Valibot according to the properties defined in your Zod schema.
...
export default component$(() => {
  ...
  return (
    <Form action={action}>
      <input name="name" />
      <button type="submit">Add user</button>
      {action.value?.failed && <p>{action.value.fieldErrors.name}</p>} // or action.value.nested.name when using Valibot
...
import { component$ } from '@builder.io/qwik';
import { routeAction$, Form, zod$, z, valibot$, v } from '@builder.io/qwik-city';
