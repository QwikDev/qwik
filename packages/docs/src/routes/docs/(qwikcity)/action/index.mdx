...
Actions can be declared using `routeAction$()` or `globalAction$()` exported from `@qwikdev/city`.

```tsx {4,16} /useAddUser/ /firstName/#a /lastName/#b /email/#c title="src/routes/layout.tsx"
import { component$ } from '@qwikdev/core';
import { routeAction$, Form } from '@qwikdev/city';
...
The best way to call an action is using the `<Form/>` component exported in `@qwikdev/city`.
...
import { component$ } from '@qwikdev/core';
import { routeAction$ } from '@qwikdev/city';
...
import { component$, type Signal, useSignal } from '@qwikdev/core';
...
import { component$ } from '@qwikdev/core';
import { routeAction$, zod$, z, Form } from '@qwikdev/city';
...
```tsx /fail/
import { routeAction$, zod$, z } from '@qwikdev/city';
...
Actions can be declared using the `routeAction$()` or `globalAction$()` exported from `@qwikdev/city`, the only difference between the two is that `routeAction$()` is scoped to a route, while `globalAction$()` is globally available across the whole app.
...
import { globalAction$ } from '@qwikdev/city';
