...
Actions can be declared using `routeAction$()` or `globalAction$()` exported from `@qwik.dev/city`.

```tsx {4,16} /useAddUser/ /firstName/#a /lastName/#b /email/#c title="src/routes/layout.tsx"
import { component$ } from '@qwik.dev/core';
import { routeAction$, Form } from '@qwik.dev/city';
...
The best way to call an action is using the `<Form/>` component exported in `@qwik.dev/city`.
...
import { component$ } from '@qwik.dev/core';
import { routeAction$, zod$, z, Form } from '@qwik.dev/city';
...
```tsx /fail/
import { routeAction$, zod$, z } from '@qwik.dev/city';
...
Actions can be declared using the `routeAction$()` or `globalAction$()` exported from `@qwik.dev/city`, the only difference between the two is that `routeAction$()` is scoped to a route, while `globalAction$()` is globally available across the whole app.
...
import { globalAction$ } from '@qwik.dev/city';
