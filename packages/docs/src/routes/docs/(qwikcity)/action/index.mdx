...
When submitting data to an action, the data is validated against the Zod schema. If the data is invalid, the action will put the validation error in the `action.value.fieldErrors` property. The `fieldErrors` object will have properties defined according to the Zod schema, in this case it will have the `name` property.
