...
One gotcha is that during development, and using Vite's preview mode, is that the service worker is disabled which also disables speculative module fetching. During development we want to always ensure the latest development code is always being used, rather than previous code. 
