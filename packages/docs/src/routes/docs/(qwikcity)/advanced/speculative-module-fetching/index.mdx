...
The goal of Qwik is to have already cached only what's possible _at that time_ and not pre-populate the cache with the entire application. When the Qwik optimizer breaks apart the application, it figures out what user interactions might hapapen next. Just as importantly, it recognizes what's not possible, and avoids loading those bundles.
...
Each page load will pre-populate the cache with bundles that _could_ be executed on the page by the user at that moment. For example, let's say that the page has a click listener on a button. When the page loads, the service worker's first task is ensure the bundle for that click listener is already in the [cache](#cache-api). When the user clicks the button and Qwik makes a request to the event listener's bundle, the goal is for the bundle to already be wiating in the [browser's cache](https://developer.mozilla.org/en-US/docs/Web/API/Cache) ready to execute.

### Pre-populating the Per Interaction

You can think of the page load as the first user interaction, which pre-populates the cache with what the next user interaction _could_ be. When a follow-up interaction happens, such as opening a modal or menu, Qwik will emit another event with additional bundles that could be used since the last interaction. Pre-populating the cache happens continuously as users interact with the application, not just on page load.
