...
Endpoints are low level API that give developers full control over the HTTP response. The are only recommended when you need to create an API to be consumed by an external entity such as a mobile app or a third-party service.
...
export const onGet2: RequestHandler = async (requestEvent) => {
