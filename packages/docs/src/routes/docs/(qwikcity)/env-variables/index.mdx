...
export default component$(() => {
  // `import.meta.env.VITE_*` variables can be read anywhere, including browser
  return <div>PUBLIC_API_URL: {import.meta.env.PUBLIC_API_URL}</div>
})
```

> `import.meta.url.PUBLIC_*` variables can be read anywhere, but do not put any sensitive information in them, since they will be visible to the client.
