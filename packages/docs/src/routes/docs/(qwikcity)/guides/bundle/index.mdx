---
title: Qwik City - Bundle Optimization
contributors:
  - mhevery
---
...
- The client can't run until any symbol until the whole chunk is loaded.
...
`qsymbol` event is fired every time a Qwik needs to resolve a symbol. Listening to this event can give you insight into when different symbols are loaded by your application. The information can then be used to better optimize your bundles by putting symbols that are needed together in the same bundle.
