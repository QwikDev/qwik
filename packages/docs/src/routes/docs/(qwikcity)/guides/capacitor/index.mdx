...
created_at: '2024-12-28T19:56:54Z'
---
...
By using Capacitor, your static Qwik app becomes a self-contained mobile app that can interact with device hardware and APIs as if it were built natively.
...
export default component$(() => {
  ...
  // Handler to apply or download and apply an update
  const handleUpdate = $(async () => {
    try {
      updateStatus.value = `Checking for existing bundle...`;

      // Fetch the list of downloaded bundle IDs
  ...
  useVisibleTask$(() => {
    try {
      ...
      console.error("Error notifying app readiness:", error);
    }
  });
