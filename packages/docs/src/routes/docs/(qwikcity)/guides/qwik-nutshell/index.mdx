...
Qwik executes the same code in the server and in the browser.

Sometimes it's desired to execute code only in browser, because you need the DOM, or other browser API such as: `window`, `document`, `localStorage`, `sessionStorage`, `indexedDB`, `navigator`, `fetch`, `requestAnimationFrame`, `setInterval`, `setTimeout`, `WebSockets`, `WebGL`.
...
export default component$(() => {
  ...
  // useVisibleTask$ will only run on the browser, so you don't need to check for `isBrowser` before accessing the DOM
  useVisibleTask$(() => {
    ref.value?.focus();
  ...
  return (
    <button
      ref={ref}
      onClick$={() => {
        // All event handlers are only executed on the browser, so it's safe to access the DOM
        ref.value?.focus();
        document.title = 'Hello world';
      }}
    >
