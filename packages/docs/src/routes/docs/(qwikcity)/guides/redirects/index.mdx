...
export const onGet: RequestHandler = async ({ url, redirect }) => {
  ...
  > {
    ...
    return [
      ...
      // Add more rules as needed. These properties above can be data grabbed from a CMS or API.
    ];
  }

  const redirectData = fetchRules().then(
    (results) =>
      results
        .filter((content) => {
          return !!(content.source && content.destination);
        })
        // Create the "redirect" object Qwik expects
        .map((data) => ({
          source: data.source,
    ...
    (error) => {
      // Logs issues with redirects without failing the deployment.
      // Remove this function to stop deployment on redirect issues.
      console.error("Error setting up redirects", error);
      return [];
    },
  );

  const redirectRules = await redirectData;
