...
Middleware consists of a set of functions that are called in a specific order defined by the route.
...
A middleware function that returns a response is called an endpoint. Endpoints are useful for returning data such as  RESTful API, GraphQL API, JSON, XML, reverse proxy or any other time of API where you need to control all aspects of the response. Returning a response (using `json` for example) will stop the execution of subsequent middleware functions past this point in the middleware chain. 
...
If any additional middleware functions are defined past this endpoint they will not be executed because this middleware has returned a response and is therefore an endpoint.
...
`routeLoader$` and `routeAction$` are also considered part of the middleware and they execute after the `on*` functions and before the default exported component.
