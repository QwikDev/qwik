...
You can also export a function that returns a `DocumentHead` object. This allows you to dynamically set the title of the page based on the data that is retrieved from `routeLoader$()` or `routeAction$()` that executed before the page component is rendered.
