...
- **📄 index. files:** Page/middleware.
- **🖼️ layout. files:** Nested layout/middleware.

## Directory-based routing

Only the directory names are used to match the incoming requests to pages/middleware.
...
- **`index.tsx|mdx` files** are the pages/middleware.
...
Inside the `src/routes` directory, all files named `index` are considered pages/middleware, Qwik supports the following extensions: `.ts`, `.tsx`, `.md` and `.mdx`.

Pages/middleware are the leaf nodes of the routing tree, ie, **the modules that will handle the request and return an HTTP response**.
