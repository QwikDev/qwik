...
The Link component's `reload` prop can be used to refresh the current page.
...
> The current implementation will prefetch on hover and will significantly improve the UX on desktop. We are working on improving this feature so that it can also properly prefetch on mobile when Link components appear on the viewport ([see github issue](https://github.com/BuilderIO/qwik/issues/5240)).
