...
Instead of repeating the same code in each route, we can use layouts to automatically reuse common parts.  Layouts support adding middleware to the route also.
...
Layouts also **provide a way to add common UI to the rendered page**. For example, if we want to add a common header to all of the routes, we can add a `Header` component to the root layout.
