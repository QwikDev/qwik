...
> ** **Important Gotcha** **
> When defining and calling `server$()` inside an `onClick$`, be aware that this can lead to potential errors. To avoid them, make sure the functions have $ wrapped around them.
