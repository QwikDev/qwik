...
`schema$()` validates an action's input on the server before the action runs, and infers the action's data type from the schema. It accepts any [Standard Schema](https://standardschema.dev/) validator, including [Zod](https://zod.dev/), [Valibot](https://valibot.dev/), [ArkType](https://arktype.io/), and [Effect Schema](https://effect.website/). The examples below use Zod.

<Note>`zod$()` and `valibot$()` still work but are deprecated aliases for `schema$()`. Effect Schema is not a Standard Schema by itself; wrap it with `Schema.standardSchemaV1(schema)` before passing it in.</Note>

```tsx {16-20} /firstName/#a /lastName/#b title="src/routes/index.tsx"
import { component$ } from '@qwik.dev/core';
import { routeAction$, schema$, Form } from '@qwik.dev/router';
...
export const useAddUser = routeAction$(
  ...
  // validate that the FormData includes "firstName" and "lastName"
  schema$(
    z.object({
      firstName: z.string(),
      lastName: z.string(),
...
When submitting data to a `routeAction()`, the data is validated against the schema. If the data is invalid, the action puts the validation error in the `routeAction.value` property.

### Event-based validation

`schema$()` also accepts a function that receives the `RequestEvent`, so you can build the schema from request data such as query parameters or headers.
...
export const useAddUser = routeAction$(
  ...
  schema$((ev) => {
    // firstName is optional when the url has the query parameter "firstname=optional"
    const firstName =
...
The `action.formData` is useful for retaining user-filled form data after a page refresh, even with JS disabled.
