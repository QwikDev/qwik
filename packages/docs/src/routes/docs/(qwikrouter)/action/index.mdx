  ...
  return (
    <>
      <Form action={action}>
        ...
        {action.error?.fieldErrors && <p>{action.error.fieldErrors.firstName}</p>}
        <button type="submit">Add user</button>
      </Form>
...
When submitting data to a `routeAction()`, the data is validated against the Zod schema. If the data is invalid, the action will put the validation error in the `action.error` property.
...
In order to return non-success values, the action can `return error(...)`.
...
Failures are stored in the `action.error` property. For `zod$`/`valibot$` validation failures, the per-field messages are typed under `action.error.fieldErrors` according to the properties defined in your schema.

The `fieldErrors` becomes a dot notation object. See [Complex forms](/docs/advanced/complex-forms) for more information.
