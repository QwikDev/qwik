...
In order to return non-success values, the action must return the `error()` method.
...
Failures are stored in the `action.error` property. Futhermore, failure messages can be found in the `action.error.fieldErrors` object according to properties defined in your Zod schema.
