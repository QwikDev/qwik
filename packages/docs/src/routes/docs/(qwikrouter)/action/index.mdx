...
export default component$(() => {
  ...
  return (
    <Form action={action}>
      <input name="name" />
      <button type="submit">Add user</button>
      {/* fieldErrors comes from a zod$ failure, message from the fail() payload */}
      {action.error && <p>{action.error.fieldErrors?.name ?? action.error.message}</p>}
...
A thrown `error()` never lands on `action.error`: the action records no state, a programmatic `action.submit()` rejects with the error, and a `<Form>` fires `submitcompleted` with `detail.aborted` set. Plain (non-`error()`) throws behave like unexpected errors and result in a sanitized 500.
