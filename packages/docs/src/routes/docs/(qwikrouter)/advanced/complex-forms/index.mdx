...
Complex forms can be validated using schema$ with routeAction$ and globalAction$. Continuing with the previous examples it would look like this:
...
<Note>If you completely forget to assign a value to an array type, lets say you completely forget person in the form. The error is then in `fieldErrors["person"]`, keyed by the field itself since the failure is the missing value rather than a bad element.</Note>
