...
The source code for `src/routes/service-worker.ts` can be modified which includes opting-in, or opting-out, of setting up Qwik Router's service worker.

Notice that the `setupServiceWorker()` function is imported from `@qwik.dev/router/service-worker` and executed at the top of the source file. Developers have the flexibility to modify when and where this function is called according to their needs. For example, if a developer prefers to handle fetch requests first, they can add their fetch listener above the  `setupServiceWorker()`. Or if they didn't want to use Qwik Router's service worker at all, they would just remove `setupServiceWorker()` from the file.
...
Another important note is that Qwik Router's request intercepting is _only_ for Qwik bundles, it does not attempt to handle any requests which are not a part of its build.

So while Qwik Router does provide a way to help prefetch and cache bundles, it does not take full control of the app's service worker. This still allows developers to add their service worker logic without conflicting with Qwik.
