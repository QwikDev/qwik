...
<Note title="Warning">Route data prefetching fetches one `q-loader-*.json` file per `routeLoader$` on the target route. If you use `visible` route data prefetching for many visible links, the browser may start many of these requests at once.</Note>
