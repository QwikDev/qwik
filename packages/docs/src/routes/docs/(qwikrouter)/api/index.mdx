...
Use `prefetchBundle` to control when Qwik prefetches the javascript bundles required for the target
route. Use `prefetchData` to control when Qwik prefetches and caches route data for the target route.

By default, `prefetchBundle` uses `visible`, while `prefetchData` uses `intent`. This means route
