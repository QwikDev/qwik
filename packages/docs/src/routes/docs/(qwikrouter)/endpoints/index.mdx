...
<Note>Sending a response is what differentiates an endpoint from a middleware. Sending a response (using `json()` for example) will therefore implicitly halt the middleware chain.</Note>
...
<CodeSandbox src="/src/routes/demo/qwikrouter/middleware/proxy/index.tsx">
```tsx
import type { RequestHandler } from '@qwik.dev/router';
...
The [`server$`](/docs/(qwikrouter)/server$/index.mdx) functions are usually a better option when you need to run some code on the server and return a response back to the app. The `server$` is strongly typed and provides a more convenient API for returning data.
