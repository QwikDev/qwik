...
See [Speculative Module Fetching](/docs/(qwikrouter)/advanced/speculative-module-fetching/index.mdx).

<Note>Service Worker feature is available only in secure contexts (HTTPS), in some or all supporting browsers. See the [`serviceWorker` property API specs](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/serviceWorker).</Note>
