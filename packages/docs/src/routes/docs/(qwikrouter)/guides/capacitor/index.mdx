...
export const DeviceInfoComponent = component$(() => {
  ...
  return (
    <div style={{ paddingTop: '40px' }}>
      ...
      {deviceInfo.value ? <p>{deviceInfo.value}</p> : <p>Loading device information...</p>}
    </div>
  );
});
```
    ...
    <div style={{ padding: '40px' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <label>
          ...
          <input
            value={version.value}
            onInput$={(event) => (version.value = (event.target as HTMLInputElement).value)}
            style={{
              marginLeft: '10px',
              padding: '8px',
              fontSize: '14px',
      ...
      {updateStatus.value && <p style={{ marginTop: '20px' }}>{updateStatus.value}</p>}
    </div>
  );
});
