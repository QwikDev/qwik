...
<Note title="IMPORTANT">DO NOT use `build-time` environment variables, (ie. variables that start with `PUBLIC_`) to store any sensitive information, such as API keys, secrets, passwords, etc. These variables are accessible in the browser — **ONLY PUBLIC DATA**.</Note>
...
<Note title="IMPORTANT">`process.env` is a Node.js only API, and should be avoided at all costs.</Note>
...
<Note>`import.meta.env.PUBLIC_*` variables can be read anywhere, but do not put any sensitive information in them, since they will be visible to the client.</Note>
...
<LongNote title="Special Considerations for Docker and Fastify">
...
<Note title="IMPORTANT">Make sure you never commit `.env.local` files to git.</Note>
...
<LongNote title="Additional Resources">
