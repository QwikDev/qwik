---
title: Project Structure | Qwik Router
contributors:
  - manucorporat
  - steve8708
  - mhevery
...
<PackageManagerTabs>
  <span q:slot="pnpm">```shell pnpm run qwik new Button ```</span>
  <span q:slot="npm">```shell npm run qwik new Button ```</span>
  <span q:slot="yarn">```shell yarn run qwik new Button ```</span>
  <span q:slot="bun">```shell bun run qwik new Button ```</span>
</PackageManagerTabs>

Maybe you want to create a new route for the /contact page. To do that, you could use the command:

<PackageManagerTabs>
  <span q:slot="pnpm">```shell pnpm run qwik new /contact ```</span>
  <span q:slot="npm">```shell npm run qwik new /contact ```</span>
  <span q:slot="yarn">```shell yarn run qwik new /contact ```</span>
  <span q:slot="bun">```shell bun run qwik new /contact ```</span>
</PackageManagerTabs>
...
<PackageManagerTabs>
  <span q:slot="pnpm">```shell pnpm run qwik new --barrel Button ```</span>
  <span q:slot="npm">```shell npm run qwik new --barrel Button ```</span>
  <span q:slot="yarn">```shell yarn run qwik new --barrel Button ```</span>
  <span q:slot="bun">```shell bun run qwik new --barrel Button ```</span>
</PackageManagerTabs>
