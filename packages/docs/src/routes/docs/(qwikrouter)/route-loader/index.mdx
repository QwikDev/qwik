...
Because `.value` never throws, an ignored `error()` won't crash the page — so Qwik logs a dev-only warning if a loader/action errors and its `.error` is never read. Enable it in production with `globalThis.qWarnUnhandledErrors = true`.
