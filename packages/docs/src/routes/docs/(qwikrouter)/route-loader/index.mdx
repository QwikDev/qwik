...
Hooks return a `ComputedSignal<T>`. Read `.value` to get the data. Wrap the component that reads it in [`<Pending>`](/docs/labs/pending/index.mdx) to show a placeholder while it loads, and in [`<Catch>`](/docs/labs/catch/index.mdx) to render a fallback if the loader throws:

```tsx /Pending/ /Catch/ /product.value/#a title="src/routes/product/[productId]/index.tsx"
...
- **`throw requestEvent.error(status, data)`** — throw a `ServerError` directly. Reading `signal.value` re-throws the `ServerError {status, data}`, so wrap the reader in a `<Catch>`. This is the recommended way to signal failure from a loader.
...
When using lazy-loaded data, wrap the reader in `<Pending>` to show a placeholder until it arrives — see [Reading the result in a component](#reading-the-result-in-a-component).
