...
import { component$ } from '@qwik.dev/core';
import { routeLoader$, isServerError } from '@qwik.dev/router';
...
export default component$(() => {
  ...
  if (product.error) {
    // ServerError (from fail()) or plain Error (client network failure)
    return isServerError(product.error) ? (
      <div>{product.error.status}: {product.error.message}</div>
    ) : (
...
On the client, `signal.error` can also hold a plain `Error` when the loader's JSON fetch itself failed (a network problem). Narrow with the `isServerError()` guard exported from `@qwik.dev/router`:

```tsx
import { isServerError } from '@qwik.dev/router';

if (product.error) {
  if (isServerError(product.error)) {
    // typed: .status, .data, and your fail() payload fields
...
<Note>Prefer `isServerError()` over `instanceof ServerError` — after crossing the serialization boundary, the client deserializes the error as a plain `Error` with the same own properties, so `instanceof` is `false` there while the guard still matches.</Note>
