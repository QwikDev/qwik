...
> If you want to manage common reusable `routeLoader$()`s it is essential that this function is re-exported from within 'layout.tsx' or 'index.tsx file of the existing route otherwise it will not run or throw exception. For more information [check this section](</docs/(qwikrouter)/re-exporting-loaders/index.mdx>).
...
export default component$(() => {
  ...
  return (
    <>
      ...
      <button onClick$={() => (condition.value = !condition.value)}>Toggle</button>
