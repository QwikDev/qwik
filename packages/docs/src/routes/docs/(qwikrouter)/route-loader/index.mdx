...
- the app navigates (every SPA navigation re-fetches every loader; the browser HTTP cache decides whether that fetch hits the network — see [Cache & freshness](#cache--freshness)),
...
<Note>**`cacheControl: 'immutable'` is special.** It means the data for a URL never changes until the application is rebuilt (the manifest hash busts the cache). For SSG, immutable loaders are pre-generated as static `.json` files on disk, so route loaders work even on fully static sites.</Note>
