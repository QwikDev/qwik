...
<Note>**`cacheControl: 'immutable'` is special.** It means the data for a URL never changes until the application is rebuilt (the manifest hash busts the cache). Immutable loaders are skipped by the per-navigation re-fetch, and for SSG they are pre-generated as static `.json` files on disk, so route loaders work even on fully static sites.</Note>
