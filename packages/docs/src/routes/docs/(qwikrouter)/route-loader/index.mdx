...
Because `.value` never throws, an ignored `error()` won't crash the page — so Qwik logs a dev-only warning if a loader/action errors and its `.error` is never read. Control this with the `qwikVite` `unhandledErrorWarning` option (`'dev'` default, `'all'` to warn in production too, or `'off'`); it can also be toggled at runtime with `globalThis.qUnhandledErrorWarning`.
