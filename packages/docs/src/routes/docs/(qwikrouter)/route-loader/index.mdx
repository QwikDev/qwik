...
- **Each loader has its own JSON endpoint.** The client fetches `q-loader-{id}.{hash}.json` for navigation, polling, and post-action invalidation. ETags and the `expires` option apply to those requests; without `expires` the response is uncached, so the browser HTTP cache never hides a fresh value.
...
Loader responses come from per-loader JSON endpoints, so ordinary HTTP caching applies. **Caching is opt-in:** by default a loader response carries no `Cache-Control` header, so data written by an action is visible to the very next loader fetch. The options below turn caching on and tune it.

### `expires`

Time in milliseconds before the data is considered stale. Sets `Cache-Control` on the JSON response and the signal's expiry. When the data expires, the next read triggers a refetch and components update reactively when the new data arrives.
...
<Note>**`expires: 0` (the default) means "no expiry".** The signal never goes stale on its own — the loader refetches on navigation, on invalidation after an action, or when the application is rebuilt — and the JSON response is sent without `Cache-Control`, so the browser never serves it from its HTTP cache. For SSG, loaders with `expires: 0` are pre-generated as static `.json` files on disk, so route loaders work even on fully static sites.</Note>

<Note>**Only cache what is safe to cache.** With `expires` set, the browser may serve the loader's JSON straight from its HTTP cache for that long, so a mutation followed by a client-side navigation can render pre-mutation data. Leave it unset for anything a user can change.</Note>
...
| ----------------------- | ------------ | ----------------------------------------------------------------------------------------------------- |
| `expires`               | `0` (no caching) | Time-to-live in ms. Opt in to HTTP caching: sets `Cache-Control` and the signal expiry. `0` sends no `Cache-Control` and never expires the signal. |
