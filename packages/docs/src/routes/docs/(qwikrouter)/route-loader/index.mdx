...
Loaders signal failure three ways:

- **`return requestEvent.error(status, data)`** — surfaces an inline `ServerError` on `signal.error` (carrying `.status` and `.data`). The page still renders; read `signal.error` alongside `signal.value`.
- **`return requestEvent.fail(status, data)`** _(deprecated)_ — surfaces `{ failed: true, ...data }` on `signal.value` (read via `signal.value.failed`). Prefer `return error()`.
- **`throw requestEvent.error(status, data)`** — aborts the request and renders the global error page (404/500). Use it when the route cannot render at all.
