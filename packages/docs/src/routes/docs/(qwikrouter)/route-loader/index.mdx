...
export default component$(() => {
  // ComputedSignal<Product> — read .value
...
- **Result is a `ComputedSignal`.** Components read `.value`. While the data is loading, reading `.value` suspends; if the loader threw, it re-throws the error.
- **Each loader has its own JSON endpoint.** The client fetches `q-loader-{id}.{hash}.json` for navigation, polling, and post-action invalidation. The browser HTTP cache, ETags, and the `cacheControl` option all apply.
...
Hooks return a `ComputedSignal<T>`. Read `.value` to get the data. Wrap the component that reads it in [`<Suspense>`](/docs/labs/suspense/index.mdx) to show a placeholder while it loads, and in [`<ErrorBoundary>`](/docs/labs/error-boundary/index.mdx) to render a fallback if the loader throws:

```tsx /Suspense/ /ErrorBoundary/ /product.value/#a title="src/routes/product/[productId]/index.tsx"
...
const ProductDetails = component$(() => {
  ...
  return product.value.failed ? (
    <div>{product.value.message}</div>
  ) : (
    <div>Product name: {product.value.name}</div>
  );
});
...
<Note>While the signal is in error state, accessing `.value` re-throws the error.</Note>

### Signaling failure from a loader

Loaders signal failure two ways:

- **`throw requestEvent.error(status, data)`** — throw a `ServerError` directly. Reading `signal.value` re-throws the `ServerError {status, data}`, so wrap the reader in an `<ErrorBoundary>`. This is the recommended way to signal failure from a loader.
...
- the app navigates (every SPA navigation re-fetches every loader except `cacheControl: 'immutable'` ones, which only re-fetch when their URL inputs change; the browser HTTP cache decides whether a fetch hits the network — see [Cache & freshness](#cache--freshness)),
- it's invalidated by an action (see [`routeAction$`](/docs/(qwikrouter)/action/index.mdx) and the action's `invalidate` option),
- you invalidate its signal manually, for example from `usePoll` (see [Polling](#polling)).

A re-fetch that returns unchanged data doesn't rerender anything: the response text is compared to the previous one and the signal keeps the same object.
...
`search: []` means the loader doesn't depend on any search params — pure path-keyed cache, best reuse across visits. When `search` is omitted, all search params are forwarded (subject to `strictLoaders`, below).
...
Loader responses come from per-loader JSON endpoints, and the browser HTTP cache is the single freshness authority: every navigation re-fetches every loader, and the loader's `Cache-Control` header decides whether that fetch is served from cache, revalidated, or hits the server.

### `cacheControl`

Sets the `Cache-Control` header on the loader's JSON responses. It accepts everything the [`cacheControl()`](/docs/(qwikrouter)/caching/index.mdx) request method accepts — `'immutable'`, a max-age number in seconds, an options object — or a function of the request event (return `null` to skip the header). A `Cache-Control` header set inside the loader function wins over the option.

The default is `no-cache`: the browser always revalidates, which pairs well with `eTag` for cheap 304 responses.
...
export const useProductDetails = routeLoader$(
  ...
  { cacheControl: 60 } // fresh for 1 minute, no requests during that time
);
```

<Note>**`cacheControl: 'immutable'` is special.** It means the data for a URL never changes until the application is rebuilt (the manifest hash busts the cache). Immutable loaders are fully lazy on the client: they are skipped by the per-navigation re-fetch, the eager navigation fetch, and link data prefetching, so their JSON downloads only when a component actually reads the signal — and is then browser-cached for the life of the deploy. For SSG they are pre-generated as static `.json` files on disk, so route loaders work even on fully static sites.</Note>

### Polling

To keep loader data fresh while the user stays on the page, use `usePoll` from `@qwik.dev/utils`. It invalidates the signal on an interval (starting once the document is idle, resetting whenever new data lands), and each refetch goes through the browser cache like any other:

```tsx
import { usePoll } from '@qwik.dev/utils';

export default component$(() => {
  const stock = useStockLevel();
  usePoll(stock, 30_000); // refetch every 30s
  return <p>{stock.value} in stock</p>;
});
```

You can also drive this manually: `signal.invalidate()` re-fetches while keeping the current value readable, and `signal.clear()` additionally drops the value so readers suspend until fresh data arrives.
...
By default the HTML response does **not** embed loader data. When the client resumes, the loader signal is empty until the browser fetches the JSON endpoint — meaning fast first paint, but a brief pending state for code that reads `.value` immediately.
...
When using lazy-loaded data, wrap the reader in `<Suspense>` to show a placeholder until it arrives — see [Reading the result in a component](#reading-the-result-in-a-component).
...
`blockSSR: false` is experimental — to use it, you must add `experimental: ['blockSSR']` to your `qwikVite` plugin options.
...
| ----------------------- | ------------ | ----------------------------------------------------------------------------------------------------- |
| `cacheControl`          | `no-cache`   | `Cache-Control` for the JSON responses; the browser cache decides freshness. `'immutable'` enables SSG loader files. |
| `serializationStrategy` | `'never'`    | `'never'` / `'always'` / `'auto'` — embed loader data in the SSR HTML?                                |
| `blockSSR`              | `true`       | Await the loader before SSR (can redirect/error the response). `false` runs it in the background (experimental). |
