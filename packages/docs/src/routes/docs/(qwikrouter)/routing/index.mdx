...
Use `prefetchBundle` and `prefetchData` to control what a link prefetches and when it starts.

`prefetchBundle` controls the javascript bundles needed to render the target route on the client.
`prefetchData` controls route data requests, including `routeLoader$`, `onGet`, and similar server
work for the target route.
...
```tsx
<Link href="/about" prefetchBundle="visible">
  About
</Link>
```

For pages that wait on route data, prefetching data before activation can make SPA navigation feel
instant when the user follows the link.

```tsx
<Link href="/pricing" prefetchData="intent">
  Pricing
</Link>
```
...
You can configure bundle and data prefetching independently:

```tsx
<Link href="/dashboard" prefetchBundle="visible" prefetchData="commit">
  Dashboard
</Link>
```
