...
contributors:
  ...
  - harishkrishnan24
  - maiieul
updated_at: '2026-06-14T18:19:48Z'
...
When submitting a request to a `routeAction()`, the request event and data undergo validation against the defined validator. If the validation fails, the action places the typed validation error on the `action.error` property:
...
export default component$(() => {
  ...
  return (
    <Form action={action}>
      <input name="search" />
      {action.error?.fieldErrors?.search && <p>{action.error.fieldErrors.search}</p>}
