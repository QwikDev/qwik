...
Use `schema$()` to validate actions with any schema library that implements [Standard Schema](https://standardschema.dev/), such as Zod, Valibot, or ArkType. The validator infers the submitted input type, passes the validated output type to the action callback, and returns failed field errors as arrays of messages.

```tsx
import { component$ } from '@builder.io/qwik';
import { Form, routeAction$, schema$ } from '@builder.io/qwik-city';
import { z } from 'zod';
...
export default component$(() => {
  ...
  return (
    <Form action={action}>
      ...
      {action.value?.failed && action.value.fieldErrors['contacts.0.email']?.[0]}
...
Standard Schema issue paths use numeric dot notation for arrays, for example `contacts.0.email`. Root-level issues are returned in `formErrors`.
