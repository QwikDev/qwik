...
Qwik breaks your application into small chunks; these pieces are smaller than the component itself, this process is performed by the [Qwik Optimizer](../optimizer/index.mdx).

The `$` prefix is used to signal both the optimizer and the developer when this transformation occurs.

## For the optimizer

Optimizer runs as a Vite plugin during bundling. The purpose of the Optimizer is to break up the application into many small lazy-loadable chunks. The Optimizer moves expressions (usually functions) into new files and leaves behind a reference pointing to where the expression was moved.

The `$` tells the optimizer which functions to break up and which ones to leave untouched. Since the optimizer does not need to keep an internal list of magic functions, but instead it can solely rely on the `$` prefix to know which functions to transform, the system is extendable and developers can create their own `$` functions, like `myCustomFunction$()`.
...
We show before that the `$` is used by the optimizer, but it's important for the developers to also known when the optimizer magic happens, because it comes with some constrains and rules.

> The worst kind of code magic is the kind it's invisible for the developer.

### Allowed expresions

The first argument of any function that end with `$` have certain restrictions:

### Literals that contains no local identifiers

**Invalid**
```tsx
const foo = 'foo';
foo$({ value: foo }) // it contains a local identifier "bar"
```
...
```tsx
const count = 0;
foo$(count + 1) // it contains a local identifier "bar"
```
...
For closures, the rules are a bit relaxed, and local identifier can be referenced and captured.
