...
We know now that the `$` is used by the optimizer, but it's important for the developers to also known when the optimizer magic happens, because it comes with some constrains and rules.
