...
## Don't register events eagerly with `useBrowserVisibleTask$()`

> ⚠ Use `useBrowserVisibleTask$()` with caution ⚠
...
It also requires you, the developer, to stay more aware of the server/client relationship (there's a reason you reached for `use >BrowserVisible< Task` after all).
