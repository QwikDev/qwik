...
It may also be desirable for a component to set `class`, `style` and `attributes` on the host element associated with the component itself. To do that, one can use `<div>` tag.
