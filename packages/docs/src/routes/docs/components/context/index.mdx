  ...
  return (
    <>
      <h1 class={themeStore.theme}>
        {themeStore.theme === 'dark' ? 'Welcome to dark side' : 'This component is light themed'}
      </h1>
      <button
        onClick$={() => {
...
This method is used to create a new `Context ID`.
...
- This method is used to provide a value to `Context` after using [`createContextId()`](#createContextId).
...
- `data`: you can provide any data type like Qwik's useStore, Array, Objects.
