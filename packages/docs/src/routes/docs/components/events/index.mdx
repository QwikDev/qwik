...
So far, we have discussed how to listen to events that originate at elements. There are events (for example, `scroll` and `mousemove`) that require that we listen to them on `window` or `document`. For this reason, Qwik allows for the `document:on` and `window:on` prefixes when listening for events.
...
The purpose of the `window:on`/`document:` is to register the event at a current DOM location of the component but have it receive events from the `window`/`document`. There are two advantages to it:
...
Components are functions, they are not elements. So DOM events does not exist naturaly, however it's possible to define custom events as props.
...
As far as Qwik is concerned, passing events to a component is equivalent to passing props. In our example, we declare all props in `CmpButtonProps` interface. Specifically, notice `onClick$: PropFunction<() => void>` declaration.

On the usage side, when referring to the `<CmpButton>`:

```tsx
<CmpButton onClick$={async () => store.cmpCount++}>{store.cmpCount}</CmpButton>
```
