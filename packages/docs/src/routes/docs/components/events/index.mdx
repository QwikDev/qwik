...
However, it is not possible to pass `props.onClick$` to `onClick$` because the types don't match. (This would result in the type error: `onClick$={props.onClick$}`) Instead, the `$` is reserved for inlined closures. In our example, we would like to `console.log("clicked")` after we process the `props.onClick` callback. We can do so with the `props.onClick$()` method. This will lazy-loads the code, then restores the closure state, and finally invokes the closure. The operation is asynchronous and therefore returns a promise, which we can resolve using the `await` statement.
