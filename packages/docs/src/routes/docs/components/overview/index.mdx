...
- [`useServerMount$()`](../lifecycle/index.mdx) - defines a callback that will be executed on mount time in the server only
- [`useWatch$()`](../lifecycle/index.mdx) - defines a callback that will be called before render and/or when a watched store changes
