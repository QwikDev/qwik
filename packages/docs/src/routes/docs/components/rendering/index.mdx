...
Qwik offers an attribute on HTML Elements called `dangerouslySetInnerHTML` as replacement for calling `innerHTML` on the DOM. Due to cross-site-scripting (XSS) possibilities by rendering not trustworthy content, you must use this attribute to remind yourself that this opperation might be dangerous.
...
```
import { component$, useStore, RenderOnce } from '@builder.io/qwik';
