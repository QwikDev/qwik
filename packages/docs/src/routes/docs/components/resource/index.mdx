...
The callback passed to `useResource$()` runs right after the `useMount()` and `useWatch()` callbacks complete, please refer to the [Lifecycle](/guide/lifecycle) section for more details.
