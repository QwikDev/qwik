...
The callback passed to [`useResource$()`](/docs/components/resource/index.mdx) runs right after the `useMount$()` and [`useTask$()`](/docs/components/lifecycle/index.mdx#usetask) callbacks complete. Please refer to the [Lifecycle](../lifecycle/index.mdx) section for more details.
