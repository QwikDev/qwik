...
Optimizer (described in-depth [here](../../advanced/optimizer/index.mdx)) is a code transformation that extracts functions into top-level importable symbols, which allows the Qwik runtime to lazy-load the JavaScript on a needed basis.
