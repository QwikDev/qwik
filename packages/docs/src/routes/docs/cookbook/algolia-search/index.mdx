...
To start using it you need a personal Algolia user and retrieve your API Keys which will be used to interact with Algolia.
...
> This implementation uses `server$` to execute the Algolia API call on the server side, but it is possible to execute it on the client side with a simple [fetch](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch).
