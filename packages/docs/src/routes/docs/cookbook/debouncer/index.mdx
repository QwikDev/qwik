...
## BONUS: `useDebounce$` - improved developer experience

To save our developers the extra "wrapping with a $()" action.

We can leverage Qwik's `implicit$FirstArg` function to create a `useDebounce$` function that automatically wraps the provided function with `$()`.
