...
> **Security note:** Using `'unsafe-inline'` reduces CSP's protection against XSS attacks. For production sites with sensitive data, consider Option 3 (script hashes) instead. However, for static portfolio sites or blogs, this trade-off is often acceptable.
...
Here's a complete `root.tsx` setup for a Qwik site using SSG with Cloudflare Pages. This uses the `ThemeToggle` component from Step 2 above:
  ...
  return (
    <QwikCityProvider>
      ...
      <body lang="en" class="bg-white dark:bg-gray-900 text-gray-900 dark:text-white">
        {/* ThemeToggle is placed outside RouterOutlet so it persists across route changes */}
        <ThemeToggle />
        <RouterOutlet />
      </body>
    </QwikCityProvider>
