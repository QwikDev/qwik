...
Qwik Router Azure Static Web Apps middleware allows you to connect Qwik Router to [Azure Static Web Apps](https://learn.microsoft.com/azure/static-web-apps/).
...
<PackageManagerTabs>
  <span q:slot="pnpm">```shell pnpm run qwik add azure-swa ```</span>
  <span q:slot="npm">```shell npm run qwik add azure-swa ```</span>
  <span q:slot="yarn">```shell yarn run qwik add azure-swa ```</span>
  <span q:slot="bun">```shell bun run qwik add azure-swa ```</span>
</PackageManagerTabs>
...
1. Deploy with [Static Web Apps CLI](https://azure.github.io/static-web-apps-cli/)
   ...
   <PackageManagerTabs>
     <span q:slot="pnpm">```shell pnpm dlx swa deploy ```</span>
     <span q:slot="npm">```shell npx swa deploy ```</span>
     <span q:slot="yarn">```shell yarn dlx swa deploy ```</span>
     <span q:slot="bun">```shell bunx swa deploy ```</span>
   </PackageManagerTabs>
