...
Qwik Router Cloud Run middleware allows you to run a Qwik Router app on [Google Cloud Run](https://cloud.google.com/run)
...
<PackageManagerTabs>
  <span q:slot="pnpm">```shell pnpm run qwik add cloud-run ```</span>
  <span q:slot="npm">```shell npm run qwik add cloud-run ```</span>
  <span q:slot="yarn">```shell yarn run qwik add cloud-run ```</span>
  <span q:slot="bun">```shell bun run qwik add cloud-run ```</span>
</PackageManagerTabs>
