...
For middleware, you'll notice that each one calls the common `@qwik.dev/city/middleware/request-handler` package. The job of each middleware is to translate the request and response to the standardized format that Qwik City request handler package uses.
