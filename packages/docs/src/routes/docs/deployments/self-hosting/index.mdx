...
If you're running behind an SSL offload proxy (like Nginx, CloudFront, etc.), you should use the `lax-proto` setting to properly handle the `X-Forwarded-Proto` header:
...
const { router, notFound } = createQwikCity({
  ...
  checkOrigin: {
    strict: false,
    strictHost: false,
    https: 'lax-proto' // Important for SSL offload proxies
  }
});
```
