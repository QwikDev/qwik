...
# Asset caching for immutable, versioned files (e.g., /build/ and /assets/)
location /build/ {
    root /path/to/your/app/dist;
    expires 1y;
...
Set to `true` (default) for strict origin checking, `false` to disable (not recommended), or `'lax-proto'` for SSL offload proxy compatibility.
...
const { router, notFound } = createQwikCity({
  ...
  qwikCityPlan,
  manifest
  // Uses ORIGIN environment variable by default for CSRF protection
});
```


#### SSL Offload Proxy Setup

If you're running behind an SSL offload proxy (like Nginx, CloudFront, etc.), set `checkOrigin: 'lax-proto'` to properly handle the `X-Forwarded-Proto` header:
...
const { router, notFound } = createQwikCity({
  ...
  qwikCityPlan,
  manifest,
  checkOrigin: 'lax-proto' // boolean | 'lax-proto'
});
```
...
If you need to disable CSRF protection (not recommended for production), set `checkOrigin: false`:
...
const { router, notFound } = createQwikCity({
  ...
  qwikCityPlan,
  manifest,
  checkOrigin: false // boolean | 'lax-proto' (not recommended)
});
```
...
3. Consider adding application monitoring with:
   ...
   - Lightweight monitoring with Netdata
     ...
     ```
     Access metrics dashboard at `http://localhost:19999`
