...
Qwik Router Vercel Edge adapter allows you to connect Qwik Router to [Vercel Edge Functions](https://vercel.com/docs/concepts/functions/edge-functions).
...
<PackageManagerTabs>
  <span q:slot="pnpm">```shell pnpm run qwik add vercel-edge ```</span>
  <span q:slot="npm">```shell npm run qwik add vercel-edge ```</span>
  <span q:slot="yarn">```shell yarn run qwik add vercel-edge ```</span>
  <span q:slot="bun">```shell bun run qwik add vercel-edge ```</span>
</PackageManagerTabs>
...
```tsx
└── The Edge Function "_qwik-router" is referencing unsupported modules
