...
**track() is not available when autoTrack is enabled**

You called `track()` inside a `useTask$`/`useVisibleTask$` created with `{ autoTrack: true }`. In that mode every signal and store read is tracked automatically, so remove the `track()` call and read the value directly.
