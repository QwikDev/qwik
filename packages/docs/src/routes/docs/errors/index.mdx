...
The function inside `useComputed$` must be synchronous. If you need an async signal with a value that may resolve later, use `useAsync$` instead. If the UI reads the value before it is ready, wrap that UI in `<Suspense>` or handle `.loading` and `.error` inline.
