...
_This code is no longer used: `useComputed$` now supports async functions. See [Async computed values](/docs/(qwik)/core/state/index.mdx#async-computed-values)._

## Q30

**Qwik version already imported**

Two different versions or builds of Qwik were loaded on the server. Copies of the same version and build share their runtime state, but different ones cannot, which leads to subtle bugs. Make sure every Qwik library resolves to the same `@qwik.dev/core` version, for example with package manager overrides, and that a development build of your app does not load a library's production copy of Qwik.
...
You accessed `.value` on an async signal with `clientOnly: true` during server-side rendering. Since `clientOnly` signals skip computation on the server, there is no value available. Provide an `initial` value in the options.

## Q36

**Invalid element name for SSR**

An element name contains characters that cannot be emitted safely during server-side rendering. Use a valid HTML, SVG, or custom element name.

## Q37

**Invalid serialized Promise dependency**

A serialized Promise tried to resolve with another serialized Promise. Regenerate the state using Qwik's serializer instead of constructing the payload manually.

## Q38

**Invalid serialized Uint8Array payload**

A serialized Uint8Array contained malformed base64 data. Regenerate the state using Qwik's serializer instead of constructing the payload manually.

## Q39

**Computed signal was disposed**

A computation tried to track a dependency or resume execution after its signal was disposed. Check `abortSignal` after asynchronous work before tracking more dependencies.
