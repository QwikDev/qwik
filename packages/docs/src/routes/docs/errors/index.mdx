...
Two different versions of Qwik were loaded on the server. Copies of the same version share their runtime state, but different versions cannot, which leads to subtle bugs. Make sure every Qwik library resolves to the same `@qwik.dev/core` version, for example with package manager overrides.
