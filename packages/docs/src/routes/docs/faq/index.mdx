...
# Does Qwik have any downside?

Yes. Every framework, possessing its own strengths and weaknesses, involves a trade off.

1. As a relative newcomer to the JS world, Qwik has yet to develop a community of libraries and best practices.
2. Qwik’s magic lies in its serialization, but this also brings about some considerations developers wouldn’t otherwise have to be mindful of.
3. Where Qwik outperforms most frameworks is on initial page load. If your use case is an SPA, and any level of server side or static rendering is not a need or interest of yours, Qwik may not offer a huge benefit to you

Despite these drawbacks, we are sure Qwik’s usability will improve with time thanks to our active community. Stay tuned! 
