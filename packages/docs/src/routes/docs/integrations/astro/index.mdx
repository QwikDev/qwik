...
This integration leverages the power of [Resumability](https://qwik.dev/docs/concepts/resumable/) inside of Astro, using Qwik components.
...
Instead, upon page load, a tiny 1kb minified piece of JavaScript, known as the [Qwikloader](https://qwik.dev/docs/advanced/qwikloader/), downloads the rest of the application as needed.
...
As of `@qwikdev/astro` v0.4, we have added support for [Speculative Module Fetching](https://qwik.dev/docs/advanced/speculative-module-fetching/) in Astro.
...
> You should be able to use [Qwik insights](https://qwik.dev/docs/labs/insights/) out of the box!

## Containers vs. Islands

While Astro generally adopts an islands architecture with other frameworks, Qwik uses a different strategy known as [Qwik containers](https://qwik.dev/docs/advanced/containers/). Despite the differences in approach, both share similar limitations.
...
You can also use the `client:*` prefix, but only in tsx files. You can find a list of directives in [Adding Interactivity](https://qwik.dev/docs/integrations/react/#adding-interactivity) section of the Qwik docs.
...
> If you're using [@astrojs/react](https://www.npmjs.com/package/@astrojs/react), you can use [qwik-react](https://qwik.dev/docs/integrations/react/#qwik-react-%EF%B8%8F) instead. The proper configuration will be supported out of the box.
