...
Example using `useSignIn` with the [&lt;Form&gt;](/docs/action/#using-actions-with-form) component and optional `providerId` and `options.callbackUrl`:
...
Example using `useSignIn` programmatically with optional `providerId` and `options.callbackUrl`:
...
Example using `useSignOut` with the [&lt;Form&gt;](/docs/action/#using-actions-with-form) component and optional `callbackUrl`:
...
Example using `useSignOut` programmatically with optional `callbackUrl`:
