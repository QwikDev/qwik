...
[Auth.js](https://authjs.dev/) allows developers to set up authentication flow to your web app. It supports wide range of authentication [providers](https://authjs.dev/reference/core/providers) and other [options](https://next-auth.js.org/configuration/options#options). Be aware that the Auth.js library is still in a pre-1.0 stage and could have bugs.
