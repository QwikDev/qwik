...
<LongNote title="Manual Deployments with Node">
...
<Note>You can also set the `authorizationParams` through the provider.authorizationParams configuration</Note>
...
<Note title="IMPORTANT">Make sure to keep the `onRequest` export as it is used to handle the OAuth flow redirects. Once the user finished OAuth flow, GitHub (or any other provider) will redirect the user back to the application to `/auth/callback/github` (or `/auth/callback/[otherProvider]`). The `onRequest` middleware function will handle requests to this endpoint and finish the OAuth flow.</Note>
...
<Note title="IMPORTANT">Please read the Qwik documentation about [Environment Variables](/docs/guides/env-variables/) to ensure you are using them safely. Many provider secrets should be kept secure and not exposed to the client/browser.</Note>
...
<LongNote title="Warning: Credentials Authentication">
This functionality is discouraged by Auth.js.
...
<Note>If placed in a layout.tsx ensure that the redirect destination does not share the same layout.tsx or a redirection loop could occur.</Note>
