...
<PackageManagerTabs>
  <span q:slot="pnpm">```shell pnpm run qwik add auth ```</span>
  <span q:slot="npm">```shell npm run qwik add auth ```</span>
  <span q:slot="yarn">```shell yarn run qwik add auth ```</span>
  <span q:slot="bun">```shell bun run qwik add auth ```</span>
</PackageManagerTabs>
  ...
  return (
    <button
      onClick$={() =>
        signIn.submit({
          providerId: 'github',
          options: { redirectTo: 'http://qwik-auth-example.com/dashboard' },
        })
      }
    >
      Sign In
...
> _IMPORTANT_: Make sure to keep the `onRequest` export as it is used to handle the OAuth flow redirects. Once the user finished OAuth flow, GitHub (or any other provider) will redirect the user back to the application to `/auth/callback/github` (or `/auth/callback/[otherProvider]`). The `onRequest` middleware function will handle requests to this endpoint and finish the OAuth flow.
...
> _IMPORTANT_: Please read the Qwik documentation about [Environment Variables](/docs/guides/env-variables/) to ensure you are using them safely. Many provider secrets should be kept secure and not exposed to the client/browser.
