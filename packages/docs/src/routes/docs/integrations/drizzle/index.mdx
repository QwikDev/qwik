...
keywords: 'drizzle, orm, database, data, sql, postgres, sqlite'
...
The easiest way to add Drizzle to Qwik is using the `npm qwik add drizzle` command. This will install the required dependencies and create a `db` folder with the drizzle schema.
