...
<div class="docs-long-list">
  - `heic` - `heif` - `avif` - `jpeg` - `jpg` - `jpe` - `tile` - `dz` - `png` - `raw` - `tiff` -
  `tif` - `webp` - `gif` - `jp2` - `jpx` - `j2k` - `j2c` - `jxl`
</div>

Supported file formats to be converted are:

<div class="docs-long-list">- `jpg` - `jpeg` - `png` - `webp` - `gif` - `tiff` - `avif`</div>
...
<abbr title="Scalable Vector Graphics">SVGs</abbr> can be added directly to your JSX code, but you
can also use the image optimization approach to optimize them:
...
<PackageManagerTabs>
  <span q:slot="pnpm">```shell pnpm install qwik-image ```</span>
  <span q:slot="npm">```shell npm install qwik-image ```</span>
  <span q:slot="yarn">```shell yarn add qwik-image ```</span>
  <span q:slot="bun">```shell bun install qwik-image ```</span>
</PackageManagerTabs>
