...
<PackageManagerTabs>
  <span q:slot="pnpm">```shell pnpm add -D @modular-forms/qwik ```</span>
  <span q:slot="npm">```shell npm install -D @modular-forms/qwik ```</span>
  <span q:slot="yarn">```shell yarn add -D @modular-forms/qwik ```</span>
  <span q:slot="bun">```shell bun install -D @modular-forms/qwik ```</span>
</PackageManagerTabs>
...
> Do you like Modular Forms so far? It would be a great honor for us to get a star from you on [GitHub](https://github.com/fabian-hiller/modular-forms)!import PackageManagerTabs from '~/components/package-manager-tabs/index.tsx';
> import CodeSandbox from '../../../../components/code-sandbox/index.tsx';
