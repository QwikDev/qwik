...
<Note title="Warning">Make sure to install @modular-forms/qwik as a devDependency, since it only runs at build/SSR time. Putting it in your regular dependencies will trigger Vite plugin errors.</Note>
...
<Note>`valiForm$` is an adapter that converts Valibot's error messages to the format expected by Modular Forms. For Zod use `zodForm$` instead.</Note>
...
<Note>Do you like Modular Forms so far? It would be a great honor for us to get a star from you on [GitHub](https://github.com/fabian-hiller/modular-forms)!</Note>
