...
<PackageManagerTabs>
  <span q:slot="pnpm">```shell pnpm run qwik add playwright ```</span>
  <span q:slot="npm">```shell npm run qwik add playwright ```</span>
  <span q:slot="yarn">```shell yarn run qwik add playwright ```</span>
  <span q:slot="bun">```shell bun run qwik add playwright ```</span>
</PackageManagerTabs>
