...
Using `supabase` with Qwik is pretty striaghtforward. The Supabase JS library plays nicely with the server-side APIs from Qwik such as `routeLoader$`, `routeAction$` or `server$`.
