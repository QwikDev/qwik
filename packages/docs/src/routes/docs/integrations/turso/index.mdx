...
<PackageManagerTabs>
  <span q:slot="pnpm">```shell pnpm run qwik add turso ```</span>
  <span q:slot="npm">```shell npm run qwik add turso ```</span>
  <span q:slot="yarn">```shell yarn run qwik add turso ```</span>
  <span q:slot="bun">```shell bun run qwik add turso ```</span>
</PackageManagerTabs>
...
> **_Note_**: A database token is not needed when working with file databases and `createClient` must be imported from `@libsql/client` since importing from `@libsql/client/web` [does not support local file URLs](https://docs.turso.tech/sdk/ts/reference#local-development).
...
[create a Turso database]: https://docs.turso.tech/quickstart
