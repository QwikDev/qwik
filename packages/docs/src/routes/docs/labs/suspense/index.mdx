...
If the computation fails, reading `user.value` re-throws the error. Wrap the section in [`<ErrorBoundary>`](/docs/labs/error-boundary/index.mdx) to render a fallback instead.
...
For async work inside a component, `useComputed$()` with an async function is usually the starting point. Add `<Suspense>` when one fallback can cover the loading state for a section. Add `<ErrorBoundary>` to render a fallback when the async work fails.
