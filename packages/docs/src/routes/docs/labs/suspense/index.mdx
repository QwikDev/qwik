...
During SSR, Qwik includes the fallback in the stream when the boundary has a fallback. If the
Suspense content finishes quickly, the resolved content can be shown before the user notices the
fallback.
