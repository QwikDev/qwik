---
title: "\U0001F9EA usePreventNavigate | Experimental"
...
You can return a Promise, and qwik-router will wait until the promise resolves before navigating.

However, in some cases the browser will navigate without calling qwik-router, such as when the user reloads the tab or navigates using `<a/>` instead of `<Link />`. When this happens, the answer must be synchronous, and user interaction is not allowed.

You can tell the difference between qwik-router and browser navigation by looking at the provided URL. If the URL is `undefined`, the browser is navigating away, and you must respond synchronously.
...
export default component$(() => {
  ...
  usePreventNavigate$((url) => {
    if (!okToNavigate.value) {
      ...
      return confirmDialog(`Do you want to lose changes and go to ${url}?`).then(
        (answer) => !answer
      );
  ...
  return (
    <div>
      <button onClick$={() => (okToNavigate.value = !okToNavigate.value)}>toggle user state</button>
      application content
    </div>
  );
});
...
export default component$(() => {
  ...
  return (
    <div>
      ...
      {showConfirm.value && (
        <div>
          <div>Do you want to lose changes and go to {String(navSig.value)}?</div>
          <button
            onClick$={() => {
