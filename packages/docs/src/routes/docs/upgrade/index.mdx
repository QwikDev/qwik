...
| `@qwik-city-plan` | `getRouterConfig()` from `@qwik.dev/router` (`@qwik-router-config` still works) |
...
Qwik libraries no longer need to be bundled into the server build. The Vite plugin finds them in your dependencies, excludes them from dependency optimization and leaves them external in the server build, where they share Qwik's runtime state with your app, Qwik Router included. The dev server still transforms them, so it can serve their lazily loaded code.

Add a library to `ssr.noExternal` and `optimizeDeps.exclude` yourself only when the plugin cannot detect it, for example when it is not listed in your `package.json`:
...
<Note>A library that works without configuration was built for Qwik v2. Libraries built for v1 still need `ssr.noExternal` and `optimizeDeps.exclude`, together with the [package manager overrides](#package-manager-overrides).</Note>
...
In v1 `useComputed$` rejected async functions. In v2 it accepts them: pass an `async` function (or return a Promise) and `.value` holds the resolved result. Qwik auto-tracks the signals and stores the function reads before the first `await`; reads after an `await` must use the `track()` provided on the context argument.
...
`.value` throws while unresolved, and re-throws the error if the computation failed. Read it inside [`<Suspense>`](/docs/labs/suspense/index.mdx) and [`<ErrorBoundary>`](/docs/labs/error-boundary/index.mdx), or provide an `initial` value.
</Note>

```tsx
const DataView = component$((props: { data: Signal<unknown> }) => {
  return <p>{JSON.stringify(props.data.value)}</p>;
});

// In the component that owns `data`:
return (
...
| Rendering | `<Resource onResolved={} />` | read `.value` inside `<Suspense>` / `<ErrorBoundary>` |
...
```tsx
import { $, ErrorBoundary, Suspense, useComputed$, type Signal } from '@qwik.dev/core';

const Temperature = component$((props: { weather: Signal<{ temp: number }> }) => {
  return <div>Temperature: {props.weather.value.temp}</div>;
});
...
- `<Resource onPending onRejected onResolved />` → `<Suspense>` and `<ErrorBoundary>`
...
Two different Qwik versions loaded on the server (error Q30). Add [package manager overrides](#package-manager-overrides) so every library resolves to the same `@qwik.dev/core`.
...
The library imports a v1 package name, so it must be bundled: add it to both `ssr.noExternal` and `optimizeDeps.exclude`. See [ssr.noExternal](#ssrnoexternal).
