...
In v1 `useComputed$` rejected async functions. In v2 it accepts them: pass an `async` function (or return a Promise) and the signal exposes `.pending`, `.error`, and `.value`. Qwik auto-tracks the signals and stores the function reads, including after `await`, so you no longer need `track()`.
...
| Track deps | `ctx.track(() => sig.value)` | read `sig.value` directly (auto-tracked, including after `await`) |
...
- `track(() => signal.value)` → read `signal.value` directly (auto-tracked, including after `await`)
