...
Qwik libraries no longer need to be bundled into the server build. The Vite plugin finds them in your dependencies, excludes them from dependency optimization and leaves them external on the server, where they share Qwik's runtime state with your app. Libraries that use Qwik Router are the exception: the plugin bundles those automatically, because the router's virtual modules only exist inside the server bundle.
...
The library uses Qwik Router, so it must be bundled: add it to both `ssr.noExternal` and `optimizeDeps.exclude`. See [ssr.noExternal](#ssrnoexternal).
