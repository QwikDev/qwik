...
- **`<Pending>`** — Shows fallback UI while part of the page waits
...
`.value` throws while unresolved, and re-throws the error if the computation failed. Read it inside [`<Pending>`](/docs/labs/pending/index.mdx) and [`<Catch>`](/docs/labs/catch/index.mdx), or provide an `initial` value.
</Note>
...
return (
  <Catch fallback$={$((error) => <p>Error: {error.message}</p>)}>
...
| Rendering | `<Resource onResolved={} />` | read `.value` inside `<Pending>` / `<Catch>` |
...
```tsx
import { $, Catch, Pending, useComputed$, type Signal } from '@qwik.dev/core';
...
`useComputed$()` owns the work that creates the value. [`<Pending>`](/docs/labs/pending/index.mdx) owns the fallback UI if you want to read `.value` directly while the value may still be pending.
...
- `<Resource onPending onRejected onResolved />` → `<Pending>` and `<Catch>`
