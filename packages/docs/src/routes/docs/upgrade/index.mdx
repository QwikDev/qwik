...
- On loaders, `.error` is typed `ServerError<ERROR> | Error | undefined` — a plain `Error` means the client-side fetch itself failed. Narrow with the `isServerError()` guard exported from `@qwik.dev/router` (`instanceof ServerError` is `false` on the client after deserialization).
...
`useAsync$()` owns the work that creates the value. [`<Suspense>`](/docs/labs/suspense/index.mdx) owns the fallback UI if you want to read `.value` directly while the value may still be pending.
