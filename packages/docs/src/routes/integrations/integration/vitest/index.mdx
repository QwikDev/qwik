...
[Vitest](https://vitest.dev/) is a blazing fast unit test framework powered by Vite. Because QwikCity uses [Vite](https://vitejs.dev/) by default, Vitest is our defacto unit test framework.
...
After running the command, vitest will be installed and a new component will be added to your project. The component will be added to the `src/components/example` directory as long as a new unit test named `example.spec.ts`.
