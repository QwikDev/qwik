...
Qwik is able to load a page and become interactive extremely fast is due to its ability to startup _without_ JavaScript. In addition to this, Speculative Module Fetching is a powerful feature that allows Qwik to pre-populate the browser's caching in a background thread.
...
- [Pre-populating the Cache with a Service Worker](#pre-populating-the-cache-with-a-service-sorker)
...
Each page load will pre-populate the cache with bundles that _could_ be executed by the user, at that moment, on the page. For example, let's say that the page has a click listener on a button. When the page loads, the very first thing the service worker does is ensure the bundle for that click listener is already waiting in the [cache](#cache-api). When the user clicks the button, and Qwik makes a request to the event listener's bundle, but the goal is that the bundle is already sitting in the [browser's cache](https://developer.mozilla.org/en-US/docs/Web/API/Cache) ready to execute.
...
The recommended strategy is to use a [service worker](#pre-populating-the-cache-with-a-service-sorker) to populate the [browser's cache](https://developer.mozilla.org/en-US/docs/Web/API/Cache). The Qwik framework itself should be use the [prefetchEvent](../../../docs/advanced/prefetching/index.mdx#prefetching-implementation) implementation, which is already the default.

## Pre-populating the Cache with a Service Worker

Traditionally, a service worker is used to cache most, or all, of the bundles an application uses. [Service workers](https://developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API/Using_Service_Workers) are commonly seen only as a way to make an application work offline.
