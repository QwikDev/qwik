...
Qwik's is able to load an entire page and become interactive extremely fast is due to its ability to startup _without_ JavaScript. 

This is a powerful feature that allows Qwik to load and execute JavaScript bundles in parallel, which is a key part of Qwik's performance.

The goal of Qwik is not to prefetch the entire application, but to have already prefetched and cached what's possible at that time. When the Qwik optimizer breaks apart the application, it's able to understand possible user interactions. And from this, it's just as important that it's able to understand what's not possible from user interaction.
