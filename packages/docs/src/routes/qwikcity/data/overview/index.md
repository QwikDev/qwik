...
Each route has the ability to add HTTP request and response handers, allowing for developers to [retrieve](../retrieve/index.mdx) and [modify](../modify/index.mdx) data. The handlers can also be used by [endpoints](../endpoints/index.mdx), which only respond with data rather than a page's HTML.
