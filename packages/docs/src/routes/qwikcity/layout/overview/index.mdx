...
The developer could extract all of these into `<Header>`, `<Footer>`, and `<Menu>` components and manually build up each page's wrapping components, but that is repetitive and error-prone. Instead, we can use layouts to automatically reuse common parts.
