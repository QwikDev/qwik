...
In this example, when Module A is first requested, the browser has no idea that it should also start requesting Module B and C. It actually doesn't even know it needs to start requesting Module B, until AFTER Module A has finished downloading. It's a common problem in that the browser doesn't know ahead of time what it should start to request, until after each module has finished downloading.

However, because our service worker contains a module graph generated from manifest, we actually do know all of the modules which _will_ be requested next. So when either a user interaction, or a prefetch for a bundle happens, the browser is able to initiate the request for all of the bundles that _will_ be requested. This allows us to drastically reduce the time it takes to request all bundles.
