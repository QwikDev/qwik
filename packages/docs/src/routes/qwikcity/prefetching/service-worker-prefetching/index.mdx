...
The service work that is installed by Qwik City is still controlled entirely by the developer. An app can include its custom logic all it wants, and can easily opt-in or opt-out of Qwik City's service worker (which helps with prefetching and caching). Below is an example of a `src/routes/service-worker.ts` source file:
...
Notice that the `setupServiceWorker()` function is imported from `@builder.io/qwik-city/service-worker` and executed at the top of the source file. If, and where, this function is called can be modified in case the developer wanted to change the order of listeners. For example, the `setupServiceWorker()` adds a `fetch` event listener, but if the developer wanted to place its own fetch listener first, the developer could place its own event listener above Qwik City's.
...
Another important note is that Qwik City's request intercepting is _only_ for Qwik bundles, it does not attempt to intercept any requests which are not a part of its build.
