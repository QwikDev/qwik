---
title: Qwik City - Pathless Layouts Routes
---

# Qwik City - Pathless Layouts
...
For example, an app places all accounts routes together in a directory, however, `/account/` could be dropped from the URL for cleaner, shorter URLs. In the example below, notice that the paths are within `src/routes/__accounts/` directory. However, the URL paths exclude `__accounts/`.
