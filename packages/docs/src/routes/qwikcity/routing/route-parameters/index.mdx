...
## Catch All Route
It is also possible to create a catch-all route by adding a directory like `[...slug]`. This works like the above described example but will also consider subsegments of the URI. It can also be applied on the `routes` root level.
