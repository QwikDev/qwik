...
- [`useContext()`](/docs/components/context/index.mdx#usecontext) to retrieve the context and use it in any component.

In this example, we would like to pass the `TodosStore` to the `<Items>` component. Update the code to use [`useContext()`](/docs/components/context/index.mdx#usecontext) to retrieve the value.
