...
<html>
  <body>
    <button q-e:click="chunk-abc.js#App_onClick">
      Click Me
    </button>
...
3. As the server renders the page, the JSX is executed and notices that there is a click listener. The click listener is serialized into the `<button>` element as `q-e:click` attribute. Qwik then knows how to hook this event back up on the client.
...
5. When a user clicks on the button, the Qwikloader intercepts the event and looks for an element with `q-e:click` attribute.
6. The `q-e:click` attribute contains:
