...
All three `useOn*()` hooks also accept an optional third argument for listener modifiers. For example:

- `{ passive: true }` for passive listeners.
- `{ capture: true }` to run in the capture phase.
- `{ preventdefault: true }` to call `event.preventDefault()` before the handler.
- `{ stoppropagation: true }` to call `event.stopPropagation()` before the handler.
