...
A component may need to add additional attributes or listeners to the host element. This can be done through the `<div>` element. Any attribute that can be placed on the DOM element can be placed on the `<div>`.
...
We need to place a click listener on the button, for this, we use `<div>` element to refer to the host element.
