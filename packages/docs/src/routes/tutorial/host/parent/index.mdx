...
In the previous example, we talked about how a component can be bound to its host element using `<div>` element. It is also possible for the parent container to bind to the child components' host element. This can be done with the `host:` prefix.
