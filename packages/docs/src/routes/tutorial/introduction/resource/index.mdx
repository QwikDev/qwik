...
- pending: the data is being fetched from the server => Render a `loading...` indicator with `<Suspense>`. Reading `.value` makes rendering wait while data is being fetched.
...
- errored: the data could not be fetched from the server due to an error => Render the error with `<ErrorBoundary>`. Reading `.value` re-throws the error.
...
Read `.value` in a child component, and wrap it in `<Suspense>` for the `pending` state and `<ErrorBoundary>` for the `rejected` state.
...
```jsx
<ErrorBoundary fallback$={$((error, reset) => <div>Error: {error.message} <button onClick$={() => reset()}>Retry</button></div>)}>
