...
Use [`useStore()`](/docs/components/state/index.mdx#usestore) to store the state of the component. The store's purpose is to:
...
Stores are declared inside the component's function using the [`useStore()`](/docs/components/state/index.mdx#usestore) function. Wrap the `github` assignment in the `useStore(...)` function call to create a store.
