...
Open the `Symbols` tab and notice how Optimizer turned the `onClick$` function into an entry point. Specifically, notice that the `onClick$` entry point does not import the `@qwikdev/core` module.

Now change the `onClick$` callback to `store.count++`.

Open the `Symbols` tab again and notice that this time the Optimizer imported `@qwikdev/core` and inserted `useLexicalScope()` call to restore the captured state of the event handler. Restoring the captured state of the function is what makes Qwik resumable.
