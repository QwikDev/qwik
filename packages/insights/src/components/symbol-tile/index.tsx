...
export const SymbolPopup = component$<{ symbolHash: string }>(
  ({ symbolHash }) => {
    return (
      <div
        class={css({
...
export const SymbolSource = component$<{ symbolHash: string }>(
  ({ symbolHash }) => {
    ...
    const source = useResource$(async ({ track }) => {
      ...
      if (state.symbolHash) {
        state.fullName = "...";
        state.origin = "...";
        const data = await serverGetSourceSnippet(
          location.params.publicApiKey,
          state.symbolHash,
        );
    ...
    return (
      <div class={css({})}>
        ...
        >
          ...
          >
            {state.originUrl ? (
              <a href={state.originUrl}>{state.origin}</a>
            ) : (
              state.origin
            )}
          </code>
  ...
  return (
    <code
      ...
      onPopup$={(e: PopupEvent) =>
        e.detail.show(SymbolPopup, { symbolHash: symbol })
      }
    >
      <SymbolIcon
        class={css({
...
const serverGetSourceSnippet = server$(async function (
  ...
  if (publicApiKey == "221smyuj5gl") {
    const rawGithub =
      "https://raw.githubusercontent.com/BuilderIO/qwik/main/packages/docs/src/";
    const github =
