...
/// `_capturesObj._[N]`, or the legacy `_captures[N]` of libraries built before it.
fn is_capture_read(member: &ast::MemberExpr) -> bool {
	match &member.obj {
		box ast::Expr::Member(obj) => {
			matches!(&obj.prop, ast::MemberProp::Ident(prop) if prop.sym.as_ref() == "_")
		}
		box ast::Expr::Ident(id) => id.sym == *_CAPTURES,
		_ => false,
	}
}
