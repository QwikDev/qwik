...
import ssgRenderSrc from './ssg-render?raw';

/**
 * Virtual module that provides SSG rendering functionality. This is auto-generated by the vite
 * adapter to render pathnames without requiring entry.ssr.
 */
const SSG_ENTRY_VIRTUAL_ID = 'virtual:qwik-router-ssg-entry.ts';
const SSG_ENTRY_OUTPUT = 'qwik-router-ssg-entry.js';
...
export function viteAdapter(opts: ViteAdapterPluginOptions) {
  ...
  let ssgEntryModulePath: string | null = null;
  ...
  const plugin: Plugin<never> = {
    ...
    buildStart() {
      if (isSsrBuild && opts.ssg !== null) {
        ...
        if (viteCommand === 'build' && serverOutDir && srcDir) {
          // Emit an auto-generated SSG entry that can render pathnames
          this.emitFile({
            id: SSG_ENTRY_VIRTUAL_ID,
            type: 'chunk',
            fileName: SSG_ENTRY_OUTPUT,
          });
        }
      }
    },
    resolveId(id) {
      if (id === SSG_ENTRY_VIRTUAL_ID) {
        return id;
      }
      return null;
    },
    load(id) {
      if (id === SSG_ENTRY_VIRTUAL_ID) {
        const { srcDir } = qwikVitePlugin!.api!.getOptions()!;
        // Generate the SSG entry code that wraps render and requestHandler
        return ssgRenderSrc.replace('__RENDER_MODULE__', `${srcDir}/entry.ssr`);
      }
      return null;
    },
      ...
      if (isSsrBuild) {
        ...
        for (const fileName in bundles) {
          ...
          if (chunk.type === 'chunk' && chunk.isEntry) {
            ...
            if (chunk.name === SSG_ENTRY_OUTPUT.replace('.js', '')) {
              ssgEntryModulePath = join(serverOutDir!, fileName);
            }
          }
        }

        // Also check for the SSG entry by filename
        if (!ssgEntryModulePath && SSG_ENTRY_OUTPUT in bundles) {
          ssgEntryModulePath = join(serverOutDir!, SSG_ENTRY_OUTPUT);
        }
      }
      if (!ssgEntryModulePath && isSsrBuild && opts.ssg !== null) {
        console.log('Bundles:', bundles);
        throw new Error(
          `Failed to find SSG entry module in build output. Expected "${SSG_ENTRY_OUTPUT}".`
        );
      }
    },
        ...
        if (isSsrBuild && serverOutDir && qwikRouterPlugin?.api && qwikVitePlugin?.api) {
          ...
          if (opts.ssg !== null && clientOutDir && clientPublicOutDir) {
            ...
            const generateOpts: StaticGenerateOptions = {
              ...
              ...opts.ssg,
              origin: ssgOrigin,
              ssgEntryModulePath: ssgEntryModulePath ?? undefined,
            };
