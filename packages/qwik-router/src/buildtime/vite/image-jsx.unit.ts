...
import {
  INTERNAL_IMAGE_JSX_IMPORT_FILTER,
...
test('creates virtual image jsx ids', () => {
  ...
  assert.equal(parsed.pathId, '/assets/hero.png');
  assert.equal(parsed.extension, '.png');
  assert.isTrue(parsed.params.has('jsx'));
  assert.equal(parsed.params.get('w'), '400');
  assert.equal(parsed.params.get('format'), 'avif');
});

test('creates internal image metadata ids without re-virtualizing them', () => {
  const id = createImageJsxImportId('/assets/hero.png', new URLSearchParams('jsx&w=400'));
  const parsed = parseVirtualImageJsxId(id);
...
test('only transforms internal jsx image imports, not passthrough imports', () => {
  const jsxImportId = createImageJsxImportId('/assets/hero.avif', new URLSearchParams('jsx&w=400'));
  assert.isTrue(INTERNAL_IMAGE_JSX_IMPORT_FILTER.test(jsxImportId));

  assert.isFalse(INTERNAL_IMAGE_JSX_IMPORT_FILTER.test('/assets/hero.avif'));
  assert.isFalse(INTERNAL_IMAGE_JSX_IMPORT_FILTER.test('/assets/hero.avif?url'));
  assert.isFalse(INTERNAL_IMAGE_JSX_IMPORT_FILTER.test('/assets/hero.avif?raw'));
  assert.isFalse(INTERNAL_IMAGE_JSX_IMPORT_FILTER.test('/assets/hero.avif?inline'));
  assert.isFalse(INTERNAL_IMAGE_JSX_IMPORT_FILTER.test('/assets/hero.avif?jsx'));
});

test('wires the internal jsx filter into imagetools so it does not use its default include', async () => {
  const imagetoolsPlugin = (await imagePlugin()[0]) as any;
  assert.equal(imagetoolsPlugin?.load?.filter?.id?.include, INTERNAL_IMAGE_JSX_IMPORT_FILTER);
});
...
test('resolves virtual module ids correctly', async () => {
  const plugin = imagePlugin()[1] as any;
  const resolveId = plugin.resolveId.handler;
