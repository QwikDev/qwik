...
describe('bun staticFile base paths', () => {
  let staticRoot: string;
  let openFile: ReturnType<typeof vi.fn>;
  const configuredPaths = new Set<string>();

  beforeEach(async () => {
    staticRoot = await mkdtemp(join(tmpdir(), 'qwik-bun-static-base-'));
    await Promise.all(
    ...
    openFile = vi.fn((filePath: string) => ({
      exists: () =>
        stat(filePath).then(
          (file) => file.isFile(),
          () => false
        ),
      stream: async () => new Blob([await readFile(filePath)]).stream(),
    }));
    vi.stubGlobal('Bun', { env: {}, fileURLToPath, file: openFile });
  });
