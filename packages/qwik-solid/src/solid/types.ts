...
export interface QwikifyBase {
  ...
  /**
   * Adds a `click` event listener to the host element, this event will be dispatched even if the solid component is not hydrated.
   */
  'host:onClick$'?: PropFunction<(ev: Event) => void>;

  /**
   * Adds a `blur` event listener to the host element, this event will be dispatched even if the solid component is not hydrated.
   */
  'host:onBlur$'?: PropFunction<(ev: Event) => void>;

  /**
   * Adds a `focus` event listener to the host element, this event will be dispatched even if the solid component is not hydrated.
   */
  'host:onFocus$'?: PropFunction<(ev: Event) => void>;

  /**
   * Adds a `mouseover` event listener to the host element, this event will be dispatched even if the solid component is not hydrated.
   */
