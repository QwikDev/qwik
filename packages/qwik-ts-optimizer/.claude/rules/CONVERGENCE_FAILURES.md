...
Snapshot of the 29 failing `pnpm vitest convergence` tests as of 2026-05-12, grouped by root cause for feature-sized work breakdown. Re-run `pnpm vitest convergence --run` to refresh; the categorization itself ages but is still a useful map.

Investigation 2026-05-06 corrected the original groupings for F3, F4, F8 after tracing diffs to actual root causes in code. Original groupings were based on diff symptoms; corrected groupings reflect the underlying transformation gap. Convergence count is now **29/212** — F1 closed 2026-05-07 (`component_level_self_referential_qrl`); F4 closed 2026-05-09 ([OSS-359](https://linear.app/kunai/issue/OSS-359) / PR #42, `example_invalid_references`); F1b closed 2026-05-11 ([OSS-360](https://linear.app/kunai/issue/OSS-360) / PR #46, `example_self_referential_component_migration`); F8a closed 2026-05-12 ([OSS-361](https://linear.app/kunai/issue/OSS-361) / PR #48, `example_getter_generation`); F8b closed 2026-05-12 ([OSS-362](https://linear.app/kunai/issue/OSS-362) / PR #50, `should_transform_three_nested_loops_handler_captures_outer_only`). Date stamp updated: 2026-05-12.
...
## Feature 8: Diverse semantic bugs sharing a "verbatim body" symptom (3 remaining tests; 2 closed)
...
| 3 | F8 individual fixes | 3 (2 of 5 closed) | F8a closed via PR #48 (`simplify.ts`); F8b closed via PR #50 (loop-cross-capture wiring). 3 remaining: `_fnSignal` dedup, migration, hoist depth. Tackle case-by-case. |
