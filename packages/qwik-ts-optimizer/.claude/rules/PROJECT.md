...
**Project-specific extra step** (METHODOLOGIES checklist step 2, not in the portable skill's 8 steps): when the merged PR touched `src/`, rebuild the dist and re-sync it into the `file:`-dep consumer's pnpm content-addressed copy (currently `qwik-design-system`, whose `docs` depends on `qwik-ts-optimizer: file:../../TS-Optimizer`; qwik-bundler is pulled in transitively and symlinks to the same store copy) before moving on — pnpm `file:` deps are snapshots, not symlinks, so the consumer silently runs stale optimizer code otherwise. Commands + verification in METHODOLOGIES.md "After a PR merges" step 2.
