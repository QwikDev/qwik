...
**Next — final multi-agent QDS compatibility pass ([OSS-499](https://linear.app/kunai/issue/OSS-499)):** broad TS↔SWC head-to-head over all qds docs routes (render + interaction), confirming the TS optimizer matches SWC. **Exclude known shared issues** — defects that reproduce on SWC too, so not ours: OSS-532's `readOnly`/`disabled`/`onChange$` playground-toggle inertness (qds resumption bug), the `modal` `[generateJsx]` docs-tooling error, the `/components/navbar/` shared 404, and the prod-preview `_run` Q14. **Any TS-only divergence is a new optimizer bug to file.** A fresh dist build + rsync into the qds store copy already matches `main` (531/535, no parked 532); clear `.vite` after any resync.
