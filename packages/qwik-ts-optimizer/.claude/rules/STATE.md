...
Last updated: 2026-06-10 ([OSS-488](https://linear.app/kunai/issue/OSS-488) scope-bindings walk fusion **Done** — PR #271: `transformAllJsx`'s standalone `collectScopeAwareBindings` pre-walk became a canonical-gather-walk projection on the parent path (per-node logic shared via `createScopeBindingsCollector`; standalone walk retained as differential oracle, parity compared on the consumer contract — `classify()` at every identifier position). Census: standalone bindings walks 1,994 → 1,404 (−590, one per parent JSX module); wall within variance on both shapes as the ticket predicted. Segment-side memo investigated and dropped (no duplicate collection exists); act-walk fusion confirmed out of scope. **Track D perf backlog: the remaining deferred census items on OSS-486 (`flattenDestructureUseCalls`, emitted-text walkers) — unfiled, file per workstream.** **Next pickup: per "What to do next"** — Candidate A (OSS-447 + OSS-448 parity) or Candidate B (OSS-450 bundler integration).) Code-hygiene track: Tracks A, B, C, E closed; track D (perf) **paused at a good stopping point**. **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite **1040**. Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) per the Branches/What-next sections below.
...
| Last verified | 2026-06-10 on `main` (post PR #271; no new test IDs — baseline steady at 1040) |
...
|---|---|---|---|---|
| `main` | `527b507` (PR #271) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router **interactive e2e in dev** (counter + `server$` RPC) under `tsOptimizer`, at parity with SWC; OSS-470 + OSS-472 merged. Backlog: OSS-471 (app migration MOVE — parity, not Q14), OSS-473 (spa_init dev-path), OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3); OSS-450 Sub-D. Prod-preview `_run` Q14 at parity with SWC (bundler/runtime). Code-hygiene track: OSS-474/475/476/477/478/479/480/482 (PRs #232/#245/#234/#236/#238/#240/#242/#251) merged — Tracks A, B, C, E fully closed; D paused (OSS-486 + OSS-487 Done; deferred census items remain). OSS-481 createOptimizer NAPI parity merged (PR #244). OSS-483 src reorg merged (PR #253) — phase/domain layout, `utils/` gone. |
...
- Perf (hygiene track D, **paused — OSS-486 Done**): cumulative BENCH-01 3.06× → 2.71×, BENCH-02 4.56× → ~4.3× (caps 1.15× / 1.5× still far off); CPU profile + per-step results in `BENCHMARKS.md` "Update 2026-06-10". Remaining: census items deferred with rationale on OSS-486 — `flattenDestructureUseCalls` + emitted-text walkers ([OSS-487](https://linear.app/kunai/issue/OSS-487) micros Done via PR #269; [OSS-488](https://linear.app/kunai/issue/OSS-488) `transformAllJsx` gather half Done via PR #271) (`flattenDestructureUseCalls`, `transformAllJsx`, emitted-text walkers — the canonical gather walk is the attachment point). File a Linear ticket per workstream; append `BENCHMARKS.md` row before/after.
