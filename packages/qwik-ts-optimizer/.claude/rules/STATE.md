...
Last updated: 2026-06-08 (router-lib `_captures` fix on branch `fix/router-lib-captures-import`, stacked on the OSS-464 branch). **🎉 Milestone: `vite-qwik-router` now RENDERS end-to-end under the TS optimizer (dev SSR HTTP 200, full content) — both prior walls cleared.** Two fixes this session: OSS-464 (pre-transformed `_jsxDEV` handler extraction) + the router-lib `_captures` import-strip bug. TS now matches SWC behaviour on this fixture: both render in dev SSR, both build green (client + server), and both hit the **same** pre-existing prod-preview Q14 (in `writeAttrs`) — an environmental fixture/runtime issue at parity, not an optimizer bug. RCA at `RCA-vite-qwik-router.md`; systemic follow-ups under OSS-465. Convergence 203/212 unchanged (both fixes are functional, not convergence flips).
...
|---|---|---|---|---|
| `fix/router-lib-captures-import` | (this branch, stacked on OSS-464) | ⏳ PR pending | 203 conv / 996 full + 3 new | Router-lib `_captures` import-strip fix (`rewrite/index.ts`). With OSS-464, `vite-qwik-router` renders end-to-end (dev SSR 200). OSS-469 (sub of OSS-456). |
| `fix/oss-464-jsxdev-handler-extraction` | `5e9b83c` | ✅ PR #225 | 203 conv / 996 full + 2 new | OSS-464 — pre-transformed `_jsxDEV` handler extraction. Route-level `testServer$` crash fixed; client+server build green. Touches `extract.ts`, `rewrite/inline-body.ts`, `transform/segment-generation.ts` + OPTIMIZER.md note. |
| `main` | `4c6cd74` (PR #223) | ✅ | baseline | Active workstream: OSS-456 (vite-qwik-router renders e2e once #225 + the OSS-469 PR land). Open backlog: OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3 multi-session); OSS-450 Sub-D in qwik-bundler PR #12. |
