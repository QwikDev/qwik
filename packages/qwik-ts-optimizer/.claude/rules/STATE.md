...
Last updated: 2026-07-16 (**OSS-537 code-hygiene sweep COMPLETE** — [OSS-538](https://linear.app/kunai/issue/OSS-538)/[OSS-539](https://linear.app/kunai/issue/OSS-539) (PRs #357/#358) + **[OSS-540](https://linear.app/kunai/issue/OSS-540) (PR #359)** all merged → `main` (`37382c5`), all **Done**; branches deleted, qds dist rebuilt + re-synced (OSS-540 code incl. the #4 fix verified in the store copy). **OSS-540** landed the six deferred behavior-*changing* audit consolidations, each with dedicated tests — five behavior-preserving (`plainQrlName` extract; `isHtmlElement` unify; `createTransformSession` ×4 body-reparse sites; `ensureCoreImports`; shared `resolveSameFileImportName` same-file import ladder) + **#4, a real behavior change on the pre-transformed `_jsxDEV` path**: `classifyProp` gains the raw-JSX path's component-param `_fnSignal` const-eligibility arm, moving a param-dependent prop on a *component* element var→const bag (byte-confirmed pre/post). **#4 is NOT a convergence flip** — convergence fixtures are all raw JSX, so the `_jsxDEV` path (what qds/router bundler builds feed) is never exercised there. **Definitive validation = qds head-to-head (OSS-499 loop), NOT yet run — recommended follow-up.** Convergence **203/9 unchanged**, full **1188 baseline all pass** (+20 OSS-540 tests auto-absorbed on merge), 0 regressions; OPTIMIZER.md audit → no update (helper extractions + one localized fix, drift below threshold). Umbrella workstream [OSS-499](https://linear.app/kunai/issue/OSS-499) qds validation: OSS-536 fixed+merged (PR #355), full 29-route head-to-head re-sweep CLEAN (0 TS-only divergences) — see progress log + [[project_qds_validation_oss499]]. **NEXT: OSS-456 parity, OSS-450 qwik-bundler integration, or resume OSS-499 with the #4 qds head-to-head.**
...
| Full suite passing (baseline) | **1188** all green post-#359; +20 OSS-540 tests auto-absorbed on merge (mapped 1213 total from #358) |
| Last verified | 2026-07-16 — post PR #359 merge (`37382c5`): 203/212 convergence + 1188 baseline all pass, 0 regressions |
