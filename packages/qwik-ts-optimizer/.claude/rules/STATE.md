...
Last updated: 2026-07-09 (PR #328 — [OSS-518](https://linear.app/kunai/issue/OSS-518) **Done**). Active workstream [OSS-499](https://linear.app/kunai/issue/OSS-499): validate **qwik-design-system** under qwik-bundler + the TS optimizer. qds docs build GREEN + dev-SSR renders homepage / `/motion/` / `/code-playground/`; a `/components/*` dev-SSR sweep cleared the component-page over-subscription (OSS-516) and the carousel `currentIdx is not defined` crash — const-propagation was dropping a bare-identifier alias declaration (`const start = currentIdx`) while orphaning its reference (`collectReferencesInInitializer` now walks the init node itself; OSS-518, PR #328). Convergence 203/9; full 1141 → 1143 (+2). **Next: keep sweeping `/components/*` for the next codegen layer; the residual `/components/select/` SSR 500 remains [OSS-517](https://linear.app/kunai/issue/OSS-517) (Backlog) — a qwik-core runtime reactivity-scheduling divergence at SWC parity, not codegen.** OSS-456 parity + OSS-450 integration open in parallel.
...
| Last verified | 2026-07-09 on `main` (post PR #328 `9b28a3c`; OSS-518 const-propagation bare-identifier alias fix, full +2) |
...
|---|---|---|---|---|
| `main` | `9b28a3c` (post PR #328) | ✅ | baseline | Active workstream: [OSS-499](https://linear.app/kunai/issue/OSS-499) — qwik-design-system validation under qwik-bundler + tsOptimizer. SWC baseline green; **bug classes A–O fixed + carousel const-alias (OSS-518)**; docs build GREEN and dev-SSR renders (homepage / `/motion/` / `/code-playground/` 200); a `/components/*` sweep cleared the OSS-516 over-subscription and the carousel `currentIdx is not defined` crash (const-propagation dropped a bare-identifier alias decl; [OSS-518](https://linear.app/kunai/issue/OSS-518), PR #328). **Next: keep sweeping `/components/*`; residual `/components/select/` 500 = [OSS-517](https://linear.app/kunai/issue/OSS-517) (qwik-core runtime scheduling at SWC parity, not codegen).** **+** OSS-456 vite-qwik-router interactive e2e in dev at parity. OSS-498 rename → `qwik-ts-optimizer` In Progress until `npm publish`. Backlog: [OSS-512](https://linear.app/kunai/issue/OSS-512) (comment cleanup), OSS-448 (scope-aware import DCE), OSS-471, OSS-473, OSS-465 RCA follow-ups, OSS-439 (F3), OSS-450 Sub-D. Code-hygiene track D paused (OSS-489 stage-2). |
