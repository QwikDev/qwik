...
Last updated: 2026-06-10 ([OSS-486](https://linear.app/kunai/issue/OSS-486) walk/parse consolidation **In Progress** — steps 1+2+3 merged (PRs #261, #263, #265): group-1 walk fusion + group-2 session threading (parses −47%) + **group-3 canonical gather walk** (`analysis/module-gather-walk.ts`, 7 per-module walks → 2, monorepo walks −14.7%); **second Track D wall win** — BENCH-01 2.96× → **2.71×** (TS −6.8%, outside same-session variance). **Next step: OSS-486 close-out** — repair `profile-deep.test.ts` (last open acceptance item); remaining census items deferred with rationale on the ticket (`flattenDestructureUseCalls`, `transformAllJsx`, emitted-text walkers). OSS-487 (micros, Backlog): remaining = `computeSegmentUsage` quadratic + regex hoists.) Code-hygiene track: Tracks A, B, C, E closed; track D (perf) **active**. **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite **1037**. Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) per the Branches/What-next sections below.
...
| Full suite passing (baseline) | **1037** |
| Last verified | 2026-06-10 on `main` (post PR #265; baseline absorbing +2 differential tests) |
...
|---|---|---|---|---|
| `main` | `ad08ad6` (PR #265) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router **interactive e2e in dev** (counter + `server$` RPC) under `tsOptimizer`, at parity with SWC; OSS-470 + OSS-472 merged. Backlog: OSS-471 (app migration MOVE — parity, not Q14), OSS-473 (spa_init dev-path), OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3); OSS-450 Sub-D. Prod-preview `_run` Q14 at parity with SWC (bundler/runtime). Code-hygiene track: OSS-474/475/476/477/478/479/480/482 (PRs #232/#245/#234/#236/#238/#240/#242/#251) merged — Tracks A, B, C, E fully closed; D open and unblocked (OSS-485 BENCH-01 crash fixed, PR #257). OSS-481 createOptimizer NAPI parity merged (PR #244). OSS-483 src reorg merged (PR #253) — phase/domain layout, `utils/` gone. |
...
- Perf (hygiene track D, **active**): baseline established 2026-06-10 at `6204de8` — BENCH-01 3.06× (cap 1.15×), BENCH-02 4.56× (cap 1.5×); CPU profile in `BENCHMARKS.md` "Update 2026-06-10" says walks (40–48% on-stack) + parses (13–17%) dominate and the catalogued D1/D2/D3 micros are <8% combined. File a Linear ticket per workstream; append `BENCHMARKS.md` row before/after.
