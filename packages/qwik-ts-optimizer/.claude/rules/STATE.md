...
Last updated: 2026-06-25 (PR #307 / [OSS-504](https://linear.app/kunai/issue/OSS-504) **merged** — Bug E sync$/_qrlSync serialization fix). Active workstream [OSS-499](https://linear.app/kunai/issue/OSS-499): validate **qwik-design-system** under qwik-bundler + the TS optimizer (`experimental: ['tsOptimizer']`); SWC baseline green. **qds docs-build progression: 44 → 39 (A+C) → 3 (B) → segment-parse (D) → segment-codegen (E) → `_rawProps` double-decl (Bug F).** Fixed: A ([OSS-500](https://linear.app/kunai/issue/OSS-500), PR #297), C ([OSS-502](https://linear.app/kunai/issue/OSS-502), PR #299), B ([OSS-501](https://linear.app/kunai/issue/OSS-501), PR #302), D ([OSS-503](https://linear.app/kunai/issue/OSS-503), PR #305 — flatten-shorthand), E ([OSS-504](https://linear.app/kunai/issue/OSS-504), PR #307 — `buildSyncTransform` double-quote-wrapped the serialized `sync$` body unescaped, so a body with `"`-literals broke it; fix picks the quote by content via a shared `quoteAsStringLiteral` helper, matching SWC + the existing `_hf*_str` logic). **NEW Bug F (not yet filed)**: `Identifier _rawProps has already been declared` (×2) — raw-props consolidation double-declaration in a segment. Convergence **203/9** unchanged; full **1090 → 1092** (+2). Next: dig into Bug F. OSS-456 parity continuation + OSS-450 integration umbrella open in parallel.
...
| Full suite passing (baseline) | **1092** |
| Last verified | 2026-06-25 on `main` (post PR #307; OSS-504 sync$ serialization fix, +2 tests) |
...
|---|---|---|---|---|
| `main` | `d73e567` (post PR #307) | ✅ | baseline | Active workstreams: [OSS-499](https://linear.app/kunai/issue/OSS-499) — qwik-design-system validation under qwik-bundler + tsOptimizer: SWC baseline green; **5 optimizer bug classes fixed (A/B/C/D/E)** — Bug A ([OSS-500](https://linear.app/kunai/issue/OSS-500), PR #297), Bug C ([OSS-502](https://linear.app/kunai/issue/OSS-502), PR #299), Bug B ([OSS-501](https://linear.app/kunai/issue/OSS-501), PR #302 — capture-position `inlinedQrl`), Bug D ([OSS-503](https://linear.app/kunai/issue/OSS-503), PR #305 — flatten-destructures object-shorthand), Bug E ([OSS-504](https://linear.app/kunai/issue/OSS-504), PR #307 — sync$ → _qrlSync quote-by-content). qds docs build 44 → 39 → 3 → segment-parse → segment-codegen → `_rawProps` double-decl. **NEW Bug F (not filed)**: `Identifier _rawProps has already been declared` (×2) — raw-props consolidation double-declaration; next up. **+** OSS-456 — vite-qwik-router **interactive e2e in dev** (counter + `server$` RPC) under `tsOptimizer`, at parity with SWC; OSS-470 + OSS-472 merged. OSS-447 (migration QRL ownership + MIG-06) merged (PR #274); OSS-497 tsdown ESM bundle merged (PR #292 — OSS-450 Sub-F); OSS-498 package rename → `qwik-ts-optimizer` + package README merged (PR #294) — **In Progress until the maintainer runs `npm publish` (the ticket's remaining deliverable)**. Backlog: OSS-448 (scope-aware import DCE — still required for `example_qwik_router_client` flip, alongside the key-prefix/`_wrapProp`/`_fnSignal`/`_rawProps`/transitive-dep families mapped on the OSS-447 comment); OSS-471 (app migration MOVE — parity, not Q14), OSS-473 (spa_init dev-path), OSS-465 RCA follow-ups; OSS-439 (F3); OSS-450 Sub-D. Prod-preview `_run` Q14 at parity with SWC (bundler/runtime). Code-hygiene track: Tracks A, B, C, E fully closed; D paused (OSS-486..491 + the OSS-492 fusion arc OSS-493/494/495/496 all Done; backlog: OSS-489 stage-2 only). |
