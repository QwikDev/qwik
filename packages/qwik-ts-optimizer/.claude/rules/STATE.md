...
Last updated: 2026-07-09 (PR #330 — [OSS-519](https://linear.app/kunai/issue/OSS-519) **Done**). Active workstream [OSS-499](https://linear.app/kunai/issue/OSS-499): validate **qwik-design-system** under qwik-bundler + the TS optimizer. A `/components/*` dev-SSR sweep now renders ~19 pages at 200; residual crashes (`/components/select/` + checkbox/collapsible/radio-group) are the [OSS-517](https://linear.app/kunai/issue/OSS-517) qwik-core runtime sparse-array class at SWC parity. **Manual interaction surfaced a bigger, GET-sweep-invisible blocker: clicking any router `<Link>` breaks** — a 3-layer chain in router-lib QRL handling (SWC navigates cleanly). **Layer 1 fixed ([OSS-519](https://linear.app/kunai/issue/OSS-519), PR #330): `buildDevFilePath` prepended `srcDir` to already-absolute paths, doubling them into malformed QRL chunk URLs that 404;** layers 2-3 (module-scope helpers not moved into `inlinedQrl` lib segments) specced as the transitive-migration feature [OSS-520](https://linear.app/kunai/issue/OSS-520) (Subs OSS-521/522/523). Convergence 203/9; full 1143. OSS-456 parity + OSS-450 integration open in parallel.
...
| Full suite passing (baseline) | **1143** |
| Last verified | 2026-07-09 on `main` (post PR #330 `49ff236`; OSS-519 absolute dev-file-path fix, no test delta) |
...
|---|---|---|---|---|
| `main` | `49ff236` (post PR #330) | ✅ | baseline | Active workstream: [OSS-499](https://linear.app/kunai/issue/OSS-499) — qwik-design-system validation under qwik-bundler + tsOptimizer. SWC baseline green; **bug classes A–O + carousel const-alias (OSS-518) + absolute dev-file-path (OSS-519) fixed**; a `/components/*` sweep renders ~19 pages 200 (residual `/components/select/` + checkbox/collapsible/radio-group = [OSS-517](https://linear.app/kunai/issue/OSS-517), qwik-core runtime class). **Router `<Link>` client-side nav is the current blocker — 3-layer chain; layer 1 fixed (OSS-519, buildDevFilePath absolute-path doubling), layers 2-3 = transitive-migration feature [OSS-520](https://linear.app/kunai/issue/OSS-520) (Subs OSS-521/522/523).** **+** OSS-456 vite-qwik-router e2e at parity. OSS-498 rename → `qwik-ts-optimizer` In Progress until `npm publish`. Backlog: [OSS-512](https://linear.app/kunai/issue/OSS-512) (comment cleanup), OSS-448 (scope-aware import DCE), OSS-471, OSS-473, OSS-465 RCA follow-ups, OSS-439 (F3), OSS-450 Sub-D. Code-hygiene track D paused (OSS-489 stage-2). |
