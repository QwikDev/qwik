...
Last updated: 2026-06-11 ([OSS-493](https://linear.app/kunai/issue/OSS-493) walk-fusion census + `ScopeQueryTracker` spike — PR #281: **GO recorded** for the [OSS-492](https://linear.app/kunai/issue/OSS-492) fusion arc (fuse Phase-1 extraction + ScopeTracker build into the canonical gather walk, 3→1 program walks per module; the formerly-unfiled candidate, now filed with subs OSS-493..496). Census baseline: 2,477 program walks per monorepo pass (trigger 478×3, jsx-only 130×2, passthrough 783×1); full fusion projects −43.8%. Kill criterion clear: unconditional tracker bookkeeping on non-trigger modules ~16.3ms/pass vs ~28ms projected savings; wall expectation low-single-digit/within-variance — census is the deliverable. New `ScopeQueryTracker.getDeclarationFromScope` (cursor-free prefix chain-walk, confirmed against oxc-walker 0.7.0 source) + equivalence suite (209 fixtures, 7,952 records, zero mismatches) is the OSS-494 foundation. **Track D perf backlog: OSS-492 subs OSS-494 → OSS-495 → OSS-496 next; OSS-489 stage-2 transform batching deferred on ticket.** **In flight: PR #274 ([OSS-447](https://linear.app/kunai/issue/OSS-447) migration-family fixes, In Review)** — its Linear comment maps the residual families still blocking the `example_qwik_router_client` flip (key-prefix is the highest-leverage unfiled one). Code-hygiene track: Tracks A, B, C, E closed; track D (perf) active. **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite **1055** baseline (+5 equivalence tests pending absorb). Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) per the Branches/What-next sections below.
...
| Full suite passing (baseline) | **1055** |
| Last verified | 2026-06-11 on `main` (post PR #279; baseline absorbing the +2 new tests) |
...
|---|---|---|---|---|
| `main` | `7bdf117` (PR #279) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router **interactive e2e in dev** (counter + `server$` RPC) under `tsOptimizer`, at parity with SWC; OSS-470 + OSS-472 merged. Backlog: OSS-471 (app migration MOVE — parity, not Q14), OSS-473 (spa_init dev-path), OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3); OSS-450 Sub-D. Prod-preview `_run` Q14 at parity with SWC (bundler/runtime). Code-hygiene track: OSS-474/475/476/477/478/479/480/482 (PRs #232/#245/#234/#236/#238/#240/#242/#251) merged — Tracks A, B, C, E fully closed; D active (OSS-486..491 Done; backlog: OSS-489 stage-2 only). OSS-481 createOptimizer NAPI parity merged (PR #244). OSS-483 src reorg merged (PR #253) — phase/domain layout, `utils/` gone. |
...
| `perf/oss-493-walk-census-spike` | `7e13b52` (PR #281) | ✅ | 203/9 + baseline clean (+5 new) | OSS-493 census + `ScopeQueryTracker` spike — **GO for the OSS-492 walk-fusion arc**. Census table + cost assessment on the ticket comment; subclass + equivalence suite is the durable diff. |
...
- **2026-06-11** — PR #281 ([OSS-493](https://linear.app/kunai/issue/OSS-493), In Progress → review) opened from `perf/oss-493-walk-census-spike`. OSS-492 fusion-arc gate: walk census (2,477 walks/pass; −43.8% projected), `ScopeQueryTracker.getDeclarationFromScope` subclass + 209-fixture equivalence suite (zero mismatches), cost assessment (bookkeeping 16.3ms < savings ~28ms) — **GO** recorded on the ticket comment. Full 1055 baseline +5; convergence 203/9.
...
- Perf (hygiene track D, **active**): cumulative BENCH-01 3.06× → 2.54×, BENCH-02 4.56× → 3.28× (caps 1.15× / 1.5× still far off); CPU profile + per-step results in `BENCHMARKS.md` "Update 2026-06-10". Remaining: [OSS-492](https://linear.app/kunai/issue/OSS-492) walk-fusion umbrella (Sub-1 OSS-493 spike done, **GO**; next OSS-494 post-walk resolution → OSS-495 build-during-gather → OSS-496 extraction fusion); [OSS-489](https://linear.app/kunai/issue/OSS-489) stage-2 transform batching (deferred on ticket). Done: [OSS-487](https://linear.app/kunai/issue/OSS-487) micros (PR #269), [OSS-488](https://linear.app/kunai/issue/OSS-488) `transformAllJsx` gather half (PR #271), [OSS-489](https://linear.app/kunai/issue/OSS-489) session churn step 1 (PR #275). File a Linear ticket per workstream; append `BENCHMARKS.md` row before/after for wall-affecting steps.
