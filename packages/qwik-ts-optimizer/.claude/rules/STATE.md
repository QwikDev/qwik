...
Last updated: 2026-06-11 ([OSS-496](https://linear.app/kunai/issue/OSS-496) extraction fusion **Done** — PR #287 merged: Phase-1 extraction is a composable collector hosted in the canonical gather walk; the Phase-1/2 boundary in `transformOneModule` merged — one fused program walk per module — plus a sound marker prefilter (`sourceMayContainMarkers`) that lets marker-less non-JSX modules skip the walk entirely. **The [OSS-492](https://linear.app/kunai/issue/OSS-492) walk-fusion arc is complete** (OSS-493/494/495/496 all Done; umbrella closed with final cumulative census on the ticket). Census: total program walks **1,999 → 826 (−58.7% step; 2,477 → 826, −66.6% across the arc)** — trigger + jsx-only modules at 1 walk each, 565/783 passthrough modules at zero walks via the prefilter. Wall: BENCH-01 −1.7% min across 3 interleaved same-session pairs (variance edge, recorded honestly; an intermediate fusion-only build measured +5% before the prefilter + lazy-frame + zero-extraction-skip fixes) — rows in BENCHMARKS.md. **Track D perf backlog now: only OSS-489 stage-2 transform batching (deferred on ticket).** **In flight: PR #274 ([OSS-447](https://linear.app/kunai/issue/OSS-447) migration-family fixes, In Review)** — its Linear comment maps the residual families still blocking the `example_qwik_router_client` flip (key-prefix is the highest-leverage unfiled one). Code-hygiene track: Tracks A, B, C, E closed; track D (perf) paused with the fusion arc done. **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite baseline **1065** (+4: fused-extraction parity + marker-prefilter tests). Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) per the Branches/What-next sections below.
...
| Full suite passing (baseline) | **1065** |
| Last verified | 2026-06-11 on `main` (post PR #287; baseline auto-absorbed +4 new test IDs → 1065) |
...
|---|---|---|---|---|
| `main` | `e2fbb92` (post PR #287) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router **interactive e2e in dev** (counter + `server$` RPC) under `tsOptimizer`, at parity with SWC; OSS-470 + OSS-472 merged. Backlog: OSS-471 (app migration MOVE — parity, not Q14), OSS-473 (spa_init dev-path), OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3); OSS-450 Sub-D. Prod-preview `_run` Q14 at parity with SWC (bundler/runtime). Code-hygiene track: OSS-474/475/476/477/478/479/480/482 (PRs #232/#245/#234/#236/#238/#240/#242/#251) merged — Tracks A, B, C, E fully closed; D paused (OSS-486..491 + the OSS-492 fusion arc OSS-493/494/495/496 all Done; backlog: OSS-489 stage-2 only). OSS-481 createOptimizer NAPI parity merged (PR #244). OSS-483 src reorg merged (PR #253) — phase/domain layout, `utils/` gone. |
