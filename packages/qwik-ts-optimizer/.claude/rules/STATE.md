...
Last updated: 2026-06-10 (Track D perf baseline established — **PR #259**: BENCH-01 3.06× / BENCH-02 4.56× at `6204de8`, CPU profile recorded in `BENCHMARKS.md` — catalogued D1/D2/D3 are <8% combined; walk/parse consolidation is the real target. Prior: [OSS-485](https://linear.app/kunai/issue/OSS-485) signal-hoist rename whole-buffer overwrite fix — **merged, PR #257**: `applySignalHoistRenames` now re-overwrites only the recorded JSX-inserted chunks instead of the whole buffer, so original offsets stay valid for later parent-rewrite passes; the `repl-console.tsx` crash is gone and **BENCH-01 is measurable again** (3.08× / BENCH-02 4.41× vs caps 1.15× / 1.5×) — hygiene track D (perf) can now establish its baseline. Prior: [OSS-484](https://linear.app/kunai/issue/OSS-484) test-tree reorganization — merged, PR #255, track E closed.) Code-hygiene track: OSS-474/475/476/477/478/479/480/482/483/484 **Done** — Tracks A, B, C, E fully closed; track D (perf) is the only one open, now unblocked — roadmap in project memory. **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite **1027**. Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) and the prod-preview `_run` Q14 (at parity with SWC, bundler/runtime) per the Branches/What-next sections below.
...
| Full suite passing (baseline) | **1027** |
| Last verified | 2026-06-10 on `main` (post PR #257; baseline auto-update absorbed +2 regression tests) |
...
|---|---|---|---|---|
| `main` | `d7feb19` (PR #259) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router **interactive e2e in dev** (counter + `server$` RPC) under `tsOptimizer`, at parity with SWC; OSS-470 + OSS-472 merged. Backlog: OSS-471 (app migration MOVE — parity, not Q14), OSS-473 (spa_init dev-path), OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3); OSS-450 Sub-D. Prod-preview `_run` Q14 at parity with SWC (bundler/runtime). Code-hygiene track: OSS-474/475/476/477/478/479/480/482 (PRs #232/#245/#234/#236/#238/#240/#242/#251) merged — Tracks A, B, C, E fully closed; D open and unblocked (OSS-485 BENCH-01 crash fixed, PR #257). OSS-481 createOptimizer NAPI parity merged (PR #244). OSS-483 src reorg merged (PR #253) — phase/domain layout, `utils/` gone. |
