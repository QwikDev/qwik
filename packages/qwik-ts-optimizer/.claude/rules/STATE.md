...
Last updated: 2026-06-18 ([OSS-447](https://linear.app/kunai/issue/OSS-447) migration-family fixes **Done** — PR #274 merged: QRL ownership for moved helper bodies (`buildMovedQrlSupport` synthesizes the `q_<symbol>` declarations + `qrl`/marker-Qrl import deps), the **MIG-06** post-pass (`reexportMovedDeclDependencies` — un-exported `keep` dependencies of moved decls flip to `reexport`; movers referenced from a different segment demote to `reexport`; fixpoint-then-flip per SWC's `precompute_and_declare_auto_exports` order), and property-position usage semantics (`isNonReferenceIdentifier` stops `document.startViewTransition` counting as root usage of the `startViewTransition` decl). Target segment `QwikRouterMockProvider_component` went **19 → 1** residuals — but the `example_qwik_router_client` flip is **not** unblocked: it needs families beyond OSS-447 + OSS-448 (JSX key-prefix `0K_*` vs `/K_*`, over-eager `_wrapProp`, missing `_fnSignal` hoists, `_rawProps` over-consolidation, a capture-unpacking gap, SWC transitive-dependency migration + topological ordering), itemized on the ticket comment for follow-up filing. Convergence **203/9** unchanged (PR flipped **+8** unit/e2e tests, not a convergence test); full suite baseline **1073** (+8). **Track D perf backlog: only [OSS-489](https://linear.app/kunai/issue/OSS-489) stage-2 transform batching (deferred on ticket).** Milestone still standing: `vite-qwik-router` interactive end-to-end in dev under the TS optimizer (counter + `server$` RPC at parity with SWC). Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog ([OSS-448](https://linear.app/kunai/issue/OSS-448) still required for the router-client flip) + RCA follow-ups (OSS-465) per the Branches/What-next sections below.
...
| Last verified | 2026-06-18 on `main` (post PR #274; baseline auto-absorbed +8 new test IDs → 1073) |
...
|---|---|---|---|---|
| `main` | `aff15d8` (post PR #274) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router **interactive e2e in dev** (counter + `server$` RPC) under `tsOptimizer`, at parity with SWC; OSS-470 + OSS-472 merged. OSS-447 (migration QRL ownership + MIG-06) merged (PR #274). Backlog: OSS-448 (scope-aware import DCE — still required for `example_qwik_router_client` flip, alongside the key-prefix/`_wrapProp`/`_fnSignal`/`_rawProps`/transitive-dep families mapped on the OSS-447 comment); OSS-471 (app migration MOVE — parity, not Q14), OSS-473 (spa_init dev-path), OSS-465 RCA follow-ups; OSS-439 (F3); OSS-450 Sub-D. Prod-preview `_run` Q14 at parity with SWC (bundler/runtime). Code-hygiene track: Tracks A, B, C, E fully closed; D paused (OSS-486..491 + the OSS-492 fusion arc OSS-493/494/495/496 all Done; backlog: OSS-489 stage-2 only). |
| `feat/oss-497-tsdown-bundle` | (off `d0b07ff`) | local | 203/1073 + baseline clean | [OSS-497](https://linear.app/kunai/issue/OSS-497) (OSS-450 Sub-F, In Progress) — package the optimizer for external npm distribution. Swapped the `tsc` library build for a **tsdown** ESM bundle (`tsdown.config.ts`: one entry, esm, `platform:node`, `dts`, `fixedExtension:false` → `dist/index.{js,d.ts}`); all 8 runtime deps stay external by default (load-bearing for the native oxc-parser/transform `.node` bindings). `package.json` wired for publish (drop `private`, `files:[dist]`, `publishConfig.access=public`, `sideEffects:false`, npm metadata, `prepublishOnly`); `tsdown` moved to devDeps + lockfile synced (incidental `@emnapi` 1.9.2→1.11.1 transitive bump). Verified: build clean, deps confirmed external, real `transformModule` through the bundle extracts segments with 0 diagnostics, `npm pack` ships only `dist` + LICENSE, full regression gate green (203/1073). **End-to-end through qwik-bundler's real pipeline**: the `vite-qwik-router` fixture client build (`✓ 186ms`, full `q-*.js` segment set) and the full `--app` client+server build (`✓` 36 modules, server entry emitted) are both green under `experimental: ['tsOptimizer']`, loading the bundled `createOptimizer` via `import('qwik-optimizer-ts')`; the dev SSR server (`vite --mode ssr`) returns **HTTP 200** with valid Qwik container markup (`q:render="ssr-dev"`, `q:container="paused"`) and no errors (browser-interactive click/RPC not exercised — needs a browser harness). Licensed **MIT** to match Qwik core (LICENSE preserves the QwikDev + BuilderIO attribution per the derivative). ESM-only; registry-publish automation deferred. |
