...
Last updated: 2026-06-11 ([OSS-489](https://linear.app/kunai/issue/OSS-489) session-churn step 1 **Done** — PR #275: churn census (worst-case 487 session parses = 407 one-shot + 80 cap-waste) + the three above-the-floor fixes: combined `extractDestructuredFieldInfo` (both destructured-param projections from one parse), parse-memo cap 4 → 16, sound `_rawProps`+`.w(` textual prefilter on `consolidateRawPropsInWCalls`. Monorepo session parses 6,289 → 5,646 (−10.2%); wall within variance both shapes as expected (~5% parse self-time). Stage-2 transform batching deferred with rationale on the ticket (correctness-bearing edit ordering, bounded win) — pick up only with explicit appetite. **Track D perf backlog: `flattenDestructureUseCalls` (unfiled, deferred on OSS-486) + OSS-489 stage-2 (deferred on ticket).** **In flight: PR #274 ([OSS-447](https://linear.app/kunai/issue/OSS-447) migration-family fixes, In Review)** — its Linear comment maps the residual families still blocking the `example_qwik_router_client` flip (key-prefix is the highest-leverage unfiled one). Code-hygiene track: Tracks A, B, C, E closed; track D (perf) paused. **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite **1044**. Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) per the Branches/What-next sections below.
...
| Full suite passing (baseline) | **1044** |
| Last verified | 2026-06-11 on `main` (post PR #275; baseline absorbing the +4 new tests) |
...
|---|---|---|---|---|
| `main` | `6f18f9c` (PR #275) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router **interactive e2e in dev** (counter + `server$` RPC) under `tsOptimizer`, at parity with SWC; OSS-470 + OSS-472 merged. Backlog: OSS-471 (app migration MOVE — parity, not Q14), OSS-473 (spa_init dev-path), OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3); OSS-450 Sub-D. Prod-preview `_run` Q14 at parity with SWC (bundler/runtime). Code-hygiene track: OSS-474/475/476/477/478/479/480/482 (PRs #232/#245/#234/#236/#238/#240/#242/#251) merged — Tracks A, B, C, E fully closed; D paused (OSS-486 + OSS-487 Done; deferred census items remain). OSS-481 createOptimizer NAPI parity merged (PR #244). OSS-483 src reorg merged (PR #253) — phase/domain layout, `utils/` gone. |
| `parity/oss-447-helper-migration` | `2a87089` (PR #274) | ✅ | 203/9 + baseline clean | OSS-447 migration-family fixes (QRL ownership, MIG-06 dependency reexport, property-position usage semantics) — **PR #274 open, In Review**. Target segment 19 → 1 residuals; flip still blocked by families mapped on the ticket comment. |
...
- Perf (hygiene track D, **paused — OSS-486 Done**): cumulative BENCH-01 3.06× → 2.71×, BENCH-02 4.56× → ~4.3× (caps 1.15× / 1.5× still far off); CPU profile + per-step results in `BENCHMARKS.md` "Update 2026-06-10". Remaining: `flattenDestructureUseCalls` (unfiled, deferred on OSS-486) + [OSS-489](https://linear.app/kunai/issue/OSS-489) stage-2 transform batching (deferred on ticket). Done: [OSS-487](https://linear.app/kunai/issue/OSS-487) micros (PR #269), [OSS-488](https://linear.app/kunai/issue/OSS-488) `transformAllJsx` gather half (PR #271), [OSS-489](https://linear.app/kunai/issue/OSS-489) session churn step 1 (PR #275) (`flattenDestructureUseCalls`, `transformAllJsx`, emitted-text walkers — the canonical gather walk is the attachment point). File a Linear ticket per workstream; append `BENCHMARKS.md` row before/after.
