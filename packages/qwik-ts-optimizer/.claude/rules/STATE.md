...
Last updated: 2026-06-11 ([OSS-491](https://linear.app/kunai/issue/OSS-491) Phase-0.5 flatten prefilter **Done** — PR #279: sound `component$` textual prefilter on `flattenDestructureUseCalls` (mirrors the exact `callee.name` trigger; renamed imports never flattened, so the gate cannot change behavior) + lazy MagicString (materializes on first overwrite). Census: monorepo Phase-0.5 walks 1,391 → 485 (−65%), ctors 1,391 → 12 (−99%); wall within variance both shapes, as the ticket predicted. Closes the last unfiled OSS-486 census deferral. **Track D perf backlog: OSS-489 stage-2 transform batching only (deferred on ticket; pick up with explicit appetite). Unfiled larger candidate: fold Phase-1 `extractSegments` into the canonical gather walk (3 program walks → 2 per module) — multi-session, scope before committing.** **In flight: PR #274 ([OSS-447](https://linear.app/kunai/issue/OSS-447) migration-family fixes, In Review)** — its Linear comment maps the residual families still blocking the `example_qwik_router_client` flip (key-prefix is the highest-leverage unfiled one). Code-hygiene track: Tracks A, B, C, E closed; track D (perf) active. **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite **1055** (+2 prefilter pin tests). Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) per the Branches/What-next sections below.
