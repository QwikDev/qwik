...
Last updated: 2026-05-09 (post BENCHMARKS.md bootstrap; between workstreams; OSS-347 still the only remaining track v2 ticket)
...
| Last verified | 2026-05-09 on `main` (post PR #28 merge, head `62c0527`) |
...
|---|---|---|---|---|
| `main` | `62c0527` (post PR #28 merge) | ✅ | baseline | All track v1 + track v2 implementation work landed; OPTIMIZER.md / REGRESSION.md / types.ts JSDoc / extract.ts JSX fold-in / closure-node threading / closure-form const literals / portable benchmark harness / post-merge cleanup routine / BENCHMARKS.md perf-history doc all shipped |
...
**Between workstreams.** OSS-355 (portable benchmark) shipped; PR #28 bootstrapped [`BENCHMARKS.md`](../../BENCHMARKS.md) with 9 historical data points + Mermaid ratio-over-time charts. No immediate active item. Two perf follow-ups now available to file when picked up: BENCH-01 monorepo ratio is **2.66×** vs the 1.15× cap, BENCH-02 worst-case is **4.67×** vs the 1.5× cap (latest values from `BENCHMARKS.md`). Run with `BENCH=1 pnpm vitest run tests/benchmark/optimizer-benchmark.test.ts --no-file-parallelism` (with `$QWIK_HOME` set + the SWC binding built in `$QWIK_HOME/packages/optimizer/bindings/`). File a Linear ticket per perf workstream rather than batching them — each one will need its own profiling pass. Append a row to `BENCHMARKS.md` before/after each perf-targeted PR so the impact is visible.
