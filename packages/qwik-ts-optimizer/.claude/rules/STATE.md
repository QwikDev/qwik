...
Last updated: 2026-07-09 (PR #332 — [OSS-520](https://linear.app/kunai/issue/OSS-520) **Done**). Active workstream [OSS-499](https://linear.app/kunai/issue/OSS-499): validate **qwik-design-system** under qwik-bundler + the TS optimizer. **Router `<Link>` client-side navigation now WORKS end-to-end** (browser-verified, matches SWC) — the 3-layer chain is closed: layer 1 = absolute dev-file-path doubling ([OSS-519](https://linear.app/kunai/issue/OSS-519)), layers 2-3 = transitive dependency migration for `inlinedQrl` lib segments ([OSS-520](https://linear.app/kunai/issue/OSS-520), Subs 521/522/523: run migration wiring for inlinedQrl, exclude their captures from segment usage, MIG-06a transitive move). Convergence 203/9; full 1143 (+1 unit test). **Next: the layer working nav exposed — target-page `component$` widget hydration errors (Header/Playground `qrl is not defined`, Search `item is not defined`); plus the residual [OSS-517](https://linear.app/kunai/issue/OSS-517) `.length` runtime class.** OSS-456 parity + OSS-450 integration open in parallel.
...
| Last verified | 2026-07-09 on `main` (post PR #332 `5ef707f`; OSS-520 transitive inlinedQrl migration, +1 unit test) |
...
|---|---|---|---|---|
| `main` | `5ef707f` (post PR #332) | ✅ | baseline | Active workstream: [OSS-499](https://linear.app/kunai/issue/OSS-499) — qwik-design-system validation under qwik-bundler + tsOptimizer. SWC baseline green; **bug classes A–O + carousel const-alias (OSS-518) + absolute dev-file-path (OSS-519) + transitive inlinedQrl migration (OSS-520) fixed**; a `/components/*` sweep renders ~19 pages 200. **Router `<Link>` client-side navigation now WORKS end-to-end (browser-verified, matches SWC) — 3-layer chain closed (OSS-519 + OSS-520).** **Next: target-page widget hydration errors surfaced by working nav (Header/Playground `qrl`, Search `item` not defined); residual [OSS-517](https://linear.app/kunai/issue/OSS-517) `.length` runtime class.** **+** OSS-456 vite-qwik-router e2e at parity. OSS-498 rename → `qwik-ts-optimizer` In Progress until `npm publish`. Backlog: [OSS-512](https://linear.app/kunai/issue/OSS-512) (comment cleanup), OSS-448 (scope-aware import DCE), OSS-471, OSS-473, OSS-465 RCA follow-ups, OSS-439 (F3), OSS-450 Sub-D. Code-hygiene track D paused (OSS-489 stage-2). |
