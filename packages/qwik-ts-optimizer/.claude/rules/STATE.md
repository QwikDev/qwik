...
Last updated: 2026-06-25 (PR #302 / [OSS-501](https://linear.app/kunai/issue/OSS-501) **merged** — Bug B lib-idempotency corruption fixed). Active workstream [OSS-499](https://linear.app/kunai/issue/OSS-499): validate **qwik-design-system** under qwik-bundler + the TS optimizer (`experimental: ['tsOptimizer']`); SWC baseline green. **All 3 optimizer bug classes the tsOptimizer build surfaced are now fixed**: Bug A ([OSS-500](https://linear.app/kunai/issue/OSS-500), multi-line JSX attr whitespace, PR #297), Bug C ([OSS-502](https://linear.app/kunai/issue/OSS-502), `stripExports` dup-imports, PR #299), and **Bug B ([OSS-501](https://linear.app/kunai/issue/OSS-501), PR #302)** — a nested `inlinedQrl` used as a value inside another's captures array. The optimizer extracted the inner QRL + rewrote its call site inside the outer's `.w([...])` array → MagicString edit collision → dropped bytes. 3-phase fix: phase 1 `filterCaptureInlinedQrls` (don't extract capture-position QRLs), phase 2 build the nested `.w([...])` from the full paren-aware `explicitCaptures` (not identifier-only `captureNames`), phase 3 `hoist-inlined-qrl.ts` lifts the surviving inline body to `const _inlined_<name>` (new tool-surface convention, added to OPTIMIZER.md). All 4 lib modules (progress-root, slider-thumb, carousel-item, select-content) re-parse clean. Convergence **203/9** unchanged; full **1084 → 1087** (+3). **Next for OSS-499**: dist resynced into qds; re-run `QDS_TS_OPTIMIZER=1` docs build (was 44 → 39 after A+C; Bug B's 19 corrupted-lib errors should now clear) → then `vp test`/Playwright. OSS-456 parity continuation + OSS-450 integration umbrella open in parallel.
...
| Full suite passing (baseline) | **1087** |
| Last verified | 2026-06-25 on `main` (post PR #302; OSS-501 lib-idempotency fix, +3 tests) |
