...
Last updated: 2026-06-11 ([OSS-494](https://linear.app/kunai/issue/OSS-494) post-walk free-identifier resolution **Done** — PR #283 merged: the canonical gather walk's free-identifier projection now buffers `(closure, name, scopeKey)` during enter and resolves post-walk via `ScopeQueryTracker.getDeclarationFromScope` (memoized per pair; per-closure first-free-occurrence order preserved). Resolution-timing change only — walk count unchanged; the one semantic mechanism change in the [OSS-492](https://linear.app/kunai/issue/OSS-492) fusion arc is de-risked in isolation, dissolving the build-before-gather ordering constraint. **Track D perf backlog: OSS-495 (build tracker during the gather walk) → OSS-496 (extraction fusion) next; OSS-489 stage-2 transform batching deferred on ticket.** **In flight: PR #274 ([OSS-447](https://linear.app/kunai/issue/OSS-447) migration-family fixes, In Review)** — its Linear comment maps the residual families still blocking the `example_qwik_router_client` flip (key-prefix is the highest-leverage unfiled one). Code-hygiene track: Tracks A, B, C, E closed; track D (perf) active. **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite **1061** (+1 differential test). Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) per the Branches/What-next sections below.
...
| Full suite passing (baseline) | **1061** |
| Last verified | 2026-06-11 on `main` (post PR #283; baseline auto-update absorbing the +1 new differential test) |
...
|---|---|---|---|---|
| `main` | `859b64b` (PR #283) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router **interactive e2e in dev** (counter + `server$` RPC) under `tsOptimizer`, at parity with SWC; OSS-470 + OSS-472 merged. Backlog: OSS-471 (app migration MOVE — parity, not Q14), OSS-473 (spa_init dev-path), OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3); OSS-450 Sub-D. Prod-preview `_run` Q14 at parity with SWC (bundler/runtime). Code-hygiene track: OSS-474/475/476/477/478/479/480/482 (PRs #232/#245/#234/#236/#238/#240/#242/#251) merged — Tracks A, B, C, E fully closed; D active (OSS-486..491 + OSS-493/494 Done; backlog: OSS-492 subs OSS-495..496, OSS-489 stage-2). OSS-481 createOptimizer NAPI parity merged (PR #244). OSS-483 src reorg merged (PR #253) — phase/domain layout, `utils/` gone. |
...
| `perf/oss-495-build-during-gather` | `bb579f3` (PR #285) | ✅ | 203/9 + baseline clean | [OSS-495](https://linear.app/kunai/issue/OSS-495) (sub of OSS-492, **PR #285 open, In Review**) — standalone ScopeTracker build walk deleted; tracker builds during the gather walk (unfrozen → freeze → post-walk resolution). Census: walks 2,477 → 1,999 (−19.3%), trigger 3→2; wall within variance per BENCHMARKS.md. Prerequisite for OSS-496 extraction fusion. |
