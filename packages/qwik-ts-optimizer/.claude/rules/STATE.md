...
Last updated: 2026-07-08 (PRs #321 / #322 — [OSS-513](https://linear.app/kunai/issue/OSS-513) / [OSS-514](https://linear.app/kunai/issue/OSS-514) both **Done**). Active workstream [OSS-499](https://linear.app/kunai/issue/OSS-499): validate **qwik-design-system** under qwik-bundler + the TS optimizer. **qds docs build GREEN + dev-SSR now RENDERS** (homepage + `/motion/` + `/code-playground/` HTTP 200, zero SSR errors) after two more onion layers cleared — Bug L, over-eager `_wrapProp` on nested signal `.value` reads (`search.tsx` `.map is not a function`; [OSS-513](https://linear.app/kunai/issue/OSS-513), PR #321), and Bug M, a dropped `.w([captures])` on bare `$()` QRL refs under inline/hoist (`render.tsx` `_captures[0]` undefined; [OSS-514](https://linear.app/kunai/issue/OSS-514), PR #322). Convergence 203/9; full 1125 → 1131 (+6). Next: sweep more qds pages/interactions for the next layer (tested pages clean, coverage not exhaustive). OSS-456 parity + OSS-450 integration open in parallel.
...
| Full suite passing (baseline) | **1131** |
| Last verified | 2026-07-08 on `main` (post PRs #321/#322; OSS-513 nested-`.value` wrap +2, OSS-514 bare-`$()` capture wrap +4) |
...
|---|---|---|---|---|
| `main` | `498503a` (post PRs #321/#322) | ✅ | baseline | Active workstream: [OSS-499](https://linear.app/kunai/issue/OSS-499) — qwik-design-system validation under qwik-bundler + tsOptimizer. SWC baseline green; **bug classes A–M fixed**; docs build GREEN and **dev-SSR now RENDERS** (homepage + `/motion/` + `/code-playground/` 200, zero SSR errors) after Bug L (over-eager `_wrapProp` on nested `.value`, [OSS-513](https://linear.app/kunai/issue/OSS-513), PR #321) and Bug M (dropped `.w([captures])` on bare `$()` refs under inline/hoist, [OSS-514](https://linear.app/kunai/issue/OSS-514), PR #322). **Next: sweep more qds pages/interactions for the next onion layer (tested pages clean, coverage not exhaustive).** **+** OSS-456 vite-qwik-router interactive e2e in dev at parity. OSS-498 rename → `qwik-ts-optimizer` In Progress until `npm publish`. Backlog: [OSS-512](https://linear.app/kunai/issue/OSS-512) (codebase-wide comment cleanup), OSS-448 (scope-aware import DCE), OSS-471, OSS-473, OSS-465 RCA follow-ups, OSS-439 (F3), OSS-450 Sub-D. Code-hygiene track D paused (OSS-489 stage-2). |
