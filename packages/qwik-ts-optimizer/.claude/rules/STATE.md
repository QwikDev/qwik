...
Last updated: 2026-06-11 ([OSS-490](https://linear.app/kunai/issue/OSS-490) MagicString churn **Done** — PR #277: exact-range JSX write memo (`writeJsxCall` records overwrites; `sliceTransformed` consults before `MagicString.slice`'s chunk-list walk), lazy `TransformSession.edits` (MagicString on first access; unedited `toSource()` short-circuits), and a binary-searched skip-range index in `transformAllJsx`. First Track D step where predicted ≈ measured: BENCH-02 74 → 64ms (**−13.5%**), BENCH-01 −4.0% (both outside overlap), 30-iter harness −11.2%; after-profile: slice 8.1% → 2.6%, ctor 4.0% → 1.0%, isInSkipRange 2.5% → 0.2%. **Track D perf backlog: `flattenDestructureUseCalls` (unfiled, deferred on OSS-486) + OSS-489 stage-2 (deferred on ticket).** **In flight: PR #274 ([OSS-447](https://linear.app/kunai/issue/OSS-447) migration-family fixes, In Review)** — its Linear comment maps the residual families still blocking the `example_qwik_router_client` flip (key-prefix is the highest-leverage unfiled one). Code-hygiene track: Tracks A, B, C, E closed; track D (perf) active. **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite **1053** (+9 new unit tests). Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) per the Branches/What-next sections below.
...
| Full suite passing (baseline) | **1053** |
| Last verified | 2026-06-11 on `main` (post PR #277; baseline absorbing the +9 new tests) |
