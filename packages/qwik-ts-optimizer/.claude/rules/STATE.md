...
Last updated: 2026-06-22 ([OSS-498](https://linear.app/kunai/issue/OSS-498) rename + package README **merged**, PR #294 — In Progress until publish). The package is renamed `qwik-optimizer-ts` → **`qwik-ts-optimizer`** (name confirmed available) and ships `0.0.1` from the OSS-497 tsdown bundle; root README is now a proper package README (the test-suite README moved to `tests/README.md`). Verified: `npm pack` → `qwik-ts-optimizer-0.0.1.tgz` (dist + LICENSE + README + package.json), real `transformModule` through the bundle 0 diagnostics, full regression gate green. **Remaining: the maintainer runs `npm login` + `npm publish` (interactive OTP) — then flip OSS-498 Done.** Follow-ups (filed out-of-scope on OSS-498): qwik-bundler adapter must update its `import('qwik-optimizer-ts')` → new name + swap the `file:` dep for the published version; internal doc-rule-file name references. Convergence **203/9** unchanged; full baseline **1073** (packaging/docs-only). Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456) parity continuation ([OSS-448](https://linear.app/kunai/issue/OSS-448) + residual families) open in parallel; integration umbrella [OSS-450](https://linear.app/kunai/issue/OSS-450) at the publication step.
...
| Last verified | 2026-06-22 on `main` (post PR #294; OSS-498 rename + README — no test-count change) |
