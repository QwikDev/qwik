...
Last updated: 2026-06-08 (post PRs #225 + #226 — OSS-464 + OSS-469 Done, both merged to `main`). **🎉 Milestone: `vite-qwik-router` now RENDERS end-to-end under the TS optimizer (dev SSR HTTP 200, full content) — both runtime walls cleared.** Two fixes landed: OSS-464 (pre-transformed `_jsxDEV` handler extraction, PR #225) + OSS-469 (router-lib `_captures` import-strip, PR #226). TS now matches SWC behaviour on this fixture: both render in dev SSR, both build green (client + server), and both hit the **same** pre-existing prod-preview Q14 (in `writeAttrs`) — an environmental fixture/runtime issue at parity, not an optimizer bug. RCA at `RCA-vite-qwik-router.md`; systemic follow-ups under OSS-465. Convergence 203/212 unchanged (both fixes are functional, not convergence flips).
