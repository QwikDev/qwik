...
Last updated: 2026-06-10 ([OSS-484](https://linear.app/kunai/issue/OSS-484) test-tree reorganization — **merged, PR #255** (hygiene track E — **closed**): 69 test files re-homed to mirror the src taxonomy, 30 `oss-*` filenames renamed to behavior names, all tracker refs stripped from test names + comments; `.ci/baseline.json` migrated by mapping the old Linux-generated baseline through the move/rename map per REGRESSION.md's new deliberate-ID-migration procedure, 1:1 verified. Prior: [OSS-483](https://linear.app/kunai/issue/OSS-483) src reorganization — merged, PR #253.) Code-hygiene track: OSS-474/475/476/477/478/479/480/482/483/484 **Done** — Tracks A, B, C, E fully closed; track D (perf) is the only one open — roadmap in project memory. **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite **1025**. Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) and the prod-preview `_run` Q14 (at parity with SWC, bundler/runtime) per the Branches/What-next sections below.
...
| Full suite passing (baseline) | **1025** |
| Last verified | 2026-06-10 on `main` (post PR #255; test-tree migration 1:1-verified, baseline mapped) |
...
|---|---|---|---|---|
| `main` | `1e40d33` (PR #255) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router **interactive e2e in dev** (counter + `server$` RPC) under `tsOptimizer`, at parity with SWC; OSS-470 + OSS-472 merged. Backlog: OSS-471 (app migration MOVE — parity, not Q14), OSS-473 (spa_init dev-path), OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3); OSS-450 Sub-D. Prod-preview `_run` Q14 at parity with SWC (bundler/runtime). Code-hygiene track: OSS-474/475/476/477/478/479/480/482 (PRs #232/#245/#234/#236/#238/#240/#242/#251) merged — Tracks A, B, C fully closed; D/E open. OSS-481 createOptimizer NAPI parity merged (PR #244). OSS-483 src reorg merged (PR #253) — phase/domain layout, `utils/` gone. |
