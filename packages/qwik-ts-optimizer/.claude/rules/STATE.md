...
Last updated: 2026-06-09 ([OSS-470](https://linear.app/kunai/issue/OSS-470) reactive `_jsxDEV` emit — **merged, PR #228**). **🎉 Milestone: `vite-qwik-router` renders AND is interactive — the counter button increments (Playwright-verified, at parity with SWC).** Three fixes this arc, all merged: OSS-464 (`_jsxDEV` handler extraction, PR #225) + OSS-469 (router-lib `_captures`, PR #226) + **OSS-470 (reactive `_jsxDEV` emit — `q:p` captures, const-bag handlers, `_wrapProp`, flags; both client + server JSX paths; PR #228)**. OSS-470 made the pre-transformed `_jsxDEV`→`_jsxSorted` output match SWC byte-for-byte, wiring event handlers + signal reactivity. **One button still broken**: the `server$` "Test server" RPC fails with Qwik `Q14` ("serverQrl … does not have a chunk path"). Root cause now fully traced (next workstream, off `main`): a **migration-policy** divergence — TS reexports `const testServer$ = serverQrl(…)` while SWC *moves* it into its single consuming segment — driven by (1) capture over-attribution (the component wrongly captures the module-level `testServer$` referenced only inside its nested handler → `segmentUsage` shows 2 consumers → MIG-02 reexport instead of MIG-01 move) and (2) a broken MOVE execution for marker-QRL bindings. Convergence 203/212 unchanged (functional fix). RCA at `RCA-vite-qwik-router.md`; systemic follow-ups under OSS-465.
...
| Last verified | 2026-06-09 on `main` (post PR #228) |
...
|---|---|---|---|---|
| `main` | `79587dd` (PR #228) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router renders e2e (dev SSR) + builds green; OSS-464 + OSS-469 + OSS-470 merged. Next: `server$` Q14 (migration MOVE-vs-REEXPORT) off `main`. Open backlog: OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3 multi-session); OSS-450 Sub-D in qwik-bundler PR #12. |
