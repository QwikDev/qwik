...
Last updated: 2026-07-20 (**OSS-541 de-any umbrella CLOSED — both subs merged.** PR #363 ([OSS-542](https://linear.app/kunai/issue/OSS-542), `tests/optimizer` specs) and PR #364 ([OSS-543](https://linear.app/kunai/issue/OSS-543), the `src/testing/ast-compare.ts` differential oracle) both **merged → `main` (`cfa6922`)**; OSS-541/542/543 all **Done**. The oracle de-any (~164 `any` across 71 normalizers → guard-based narrowing via reused `isAstNode` + local `isRecord`/`asArray`, no `as` casts) was verified **bit-identical** (full pass/fail set unchanged, CI-green) — the acceptance criterion for a differential oracle — and dist was rebuilt + re-synced to the qds store copy (byte-identical; behavior-neutral change). Convergence **203/9** unchanged. **NEXT: OSS-456 parity, OSS-450 qwik-bundler integration, or the OSS-540 #4 qds head-to-head.**)
...
| Full suite passing (baseline) | **1208** all green (post-#364; passing set unchanged — the de-any refactors renamed/removed no test IDs) |
| Last verified | 2026-07-20 — post PR #364 merge (`cfa6922`); OSS-543 oracle de-any verified **bit-identical** (CI-green): 203/212 convergence + 1208 baseline all pass, 0 regressions |
