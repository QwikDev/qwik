...
Last updated: 2026-06-10 ([OSS-483](https://linear.app/kunai/issue/OSS-483) src reorganization — **merged, PR #253**: `src/optimizer/utils/` eliminated and all loose `src/optimizer/` root files re-homed into phase/domain directories (`prepare/`, `extraction/`, `analysis/`, `rewrite/`, `segment/`, `jsx/`, `diagnostics/`, `ast/`, `edit/`, `qwik/`); 54 pure `git mv` moves, line-neutral + test-neutral; rule-doc path refs resynced in-PR.) Code-hygiene track: OSS-474/475/476/477/478/479/480/482 (PRs #232/#245/#234/#236/#238/#240/#242/#251) **Done** — Tracks A, B, C fully closed; tracks D (perf) / E (test-file tracker-ref rename) open — roadmap in project memory. **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite **1025**. Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) and the prod-preview `_run` Q14 (at parity with SWC, bundler/runtime) per the Branches/What-next sections below.
