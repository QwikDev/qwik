...
Last updated: 2026-07-20 (**OSS-471 MERGED — `server$` single-consumer MOVE (SWC parity).** PR #367 ([OSS-471](https://linear.app/kunai/issue/OSS-471)) merged → `main` (`523ae5e`); OSS-471 **Done**, closing an OSS-456 sub. A module-level `const testServer$ = server$(…)` referenced only inside one nested `onClick$` now MOVES into its handler segment instead of re-exporting — four coupled fixes (capture over-attribution **gated to file-emitting strategies**; `escapeSymbol` match ×2; callee import source; per-marker PURE), each **byte-verified against SWC** via `createOptimizer().transformModules`. Convergence **203/9** unchanged; full **1209** (baseline auto-update absorbs the +1 strip-aware regression test; 2 router tests renamed 1:1 in `.ci/baseline.json`), 0 regressions. Dist rebuilt + re-synced to the qds store copy. OPTIMIZER.md → no update (bug-fix/type-internal; cited refs <50-line drift). **OSS-456 now has ONE open sub left: OSS-473 (router-lib dev chunk-URL 404). NEXT: OSS-473, OSS-448 parity, or OSS-450 qwik-bundler integration.**)
...
| Full suite passing (baseline) | **1209** (post-#367; baseline auto-update absorbs the +1 strip-aware regression test; the 2 renamed router tests mapped 1:1) |
| Last verified | 2026-07-20 — post PR #367 merge (`523ae5e`); OSS-471 `server$` MOVE **byte-verified vs SWC**, gate green: 203/212 convergence + full baseline all pass (+1), 0 regressions |
...
- **2026-07-20** — **PR #367 ([OSS-471](https://linear.app/kunai/issue/OSS-471)) merged → `main` (`523ae5e`); OSS-471 Done, closing an OSS-456 sub.** Single-consumer `const testServer$ = server$(…)` (referenced only in one nested `onClick$`) now **MOVES into its handler segment** instead of re-exporting — four coupled fixes: capture over-attribution exclusion **gated to file-emitting strategies** (inline/hoist keep the ref inline, so the `useStyle$(STYLES)` reexport survives); `escapeSymbol` match in `tryBuildMarkerDeclMove` + the parallel `movedMarkerSymbols` parent demotion; marker-callee import from its real source; per-marker PURE via `needsPureAnnotation`. Each byte-verified against SWC. Reachable (non-stripped) output matches SWC; the stripped variant registers via `_noopQrl` (an unreachable-path cosmetic var-name divergence is documented). Convergence **203/9** unchanged; full **1209** (+1 strip test; 2 router tests renamed 1:1 in `.ci/baseline.json` per REGRESSION.md deliberate-migration), 0 regressions. Two follow-up commits stripped every inline `//` comment per review. OPTIMIZER.md → no update (bug-fix/type-internal; cited refs <50-line drift).
- **2026-07-20** — **PR #364 ([OSS-543](https://linear.app/kunai/issue/OSS-543)) merged → `main` (`cfa6922`); OSS-541 de-any umbrella CLOSED.** The `src/testing/ast-compare.ts` oracle de-any (~164 `any` → guard-based narrowing, bit-identical, CI-green) landed; OSS-543 + OSS-541 → **Done**, branch deleted, dist rebuilt + re-synced to the qds store copy (byte-identical — behavior-neutral change, so no new runtime string to grep). A review-response follow-up dropped the one-line JSDoc on the trivial `asArray`/`asString` helpers (WHAT-narration). OPTIMIZER.md → no update (`ast-compare.ts` not in the trigger checklist). Both de-any subs now shipped; `tests/optimizer/**` + `src/testing/ast-compare.ts` are `any`-free.
