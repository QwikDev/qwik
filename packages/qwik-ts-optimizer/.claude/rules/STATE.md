...
Last updated: 2026-07-07 (PRs #313 + #314 — [OSS-506](https://linear.app/kunai/issue/OSS-506) / [OSS-507](https://linear.app/kunai/issue/OSS-507) + [OSS-508](https://linear.app/kunai/issue/OSS-508), all **Done**). Active workstream [OSS-499](https://linear.app/kunai/issue/OSS-499): validate **qwik-design-system** under qwik-bundler + the TS optimizer. **qds docs build GREEN** (bugs A–F). **Bug G serverQrl SSR crash now FIXED** (PR #313): the real root cause was **not** migration over-eagerness (PR #311's downstream fix) but a **const-fold gap** — `isServer`/`isBrowser` were never folded inside hoist/inline closure bodies (re-emitted from `ext.bodyText`, outside the parent MagicString that `replaceConstants` edits), so the router lib kept dead client branches → surviving `@qwik-router-config` import → eager route-tree pull → SSR circular dep. Fixed by `foldConstantsInBodyText`, plus Q3 `dropTopLevelModuleScopeCaptures`. **OSS-508** (PR #314) then fixed a pre-transformed `_jsxDEV` object-shorthand corruption from single-use const inlining. Convergence 203/9; full 1097 → 1111 (+14). **Remaining qds bug: carousel `.script.ts` `document is not defined` in SSR** — a separate optimizer eval-order divergence (module byte-identical TS vs SWC), needs its own OSS-499 sub-ticket. OSS-456 parity + OSS-450 integration open in parallel.
...
| Full suite passing (baseline) | **1111** |
| Last verified | 2026-07-07 on `main` (post PR #314; OSS-506/507 Bug G serverQrl fix +9, OSS-508 shorthand fix +5) |
...
|---|---|---|---|---|
| `main` | `b3bcd31` (post PR #314) | ✅ | baseline | Active workstream: [OSS-499](https://linear.app/kunai/issue/OSS-499) — qwik-design-system validation under qwik-bundler + tsOptimizer. SWC baseline green; **bug classes A–F fixed**; **Bug G serverQrl SSR crash FIXED** ([OSS-506](https://linear.app/kunai/issue/OSS-506)/[OSS-507](https://linear.app/kunai/issue/OSS-507), PR #313 — const-fold in hoist/inline bodies via `foldConstantsInBodyText` + Q3 capture-drop) and **[OSS-508](https://linear.app/kunai/issue/OSS-508)** `_jsxDEV` shorthand corruption fixed (PR #314). **qds docs build GREEN**; dev SSR renders past `highlight.server.ts`. **Remaining qds bug: carousel `.script.ts` `document is not defined`** — separate eval-order divergence, needs an OSS-499 sub-ticket. **+** OSS-456 vite-qwik-router interactive e2e in dev at parity. OSS-498 rename → `qwik-ts-optimizer` In Progress until `npm publish`. Backlog: OSS-448 (scope-aware import DCE), OSS-471, OSS-473, OSS-465 RCA follow-ups, OSS-439 (F3), OSS-450 Sub-D. Code-hygiene track D paused (OSS-489 stage-2). |
