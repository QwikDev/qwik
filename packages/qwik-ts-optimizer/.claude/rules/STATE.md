...
Last updated: 2026-06-25 (PR #305 / [OSS-503](https://linear.app/kunai/issue/OSS-503) **merged** — Bug D flatten-destructures object-shorthand fix). Active workstream [OSS-499](https://linear.app/kunai/issue/OSS-499): validate **qwik-design-system** under qwik-bundler + the TS optimizer (`experimental: ['tsOptimizer']`); SWC baseline green. **qds docs-build progression: 44 → 39 (Bug A+C) → 3 (Bug B/OSS-501) → segment-codegen stage (Bug D/OSS-503 cleared).** Fixed: A ([OSS-500](https://linear.app/kunai/issue/OSS-500), PR #297), C ([OSS-502](https://linear.app/kunai/issue/OSS-502), PR #299), B ([OSS-501](https://linear.app/kunai/issue/OSS-501), PR #302 — capture-position `inlinedQrl`), D ([OSS-503](https://linear.app/kunai/issue/OSS-503), PR #305 — `flatten-destructures` rewrote a `use*()`-destructured binding used as an object shorthand `{ isDisabled }` to a bare member expr `{ bindings.disabledSig }`; fix expands to `{ key: member }` via a new `isShorthandPropertyValue` predicate). **NEW Bug E (not yet filed)**: with D cleared the build reaches component segment codegen and fails on 3 segments (`file-upload-drop-area`, `checkbox-trigger`, `navbar-item-content`) with `Expected , or ) but found Identifier` (params/args corruption); none use the `use*()`-destructure pattern, so distinct + pre-existing. Convergence **203/9** unchanged; full **1087 → 1090** (+3). Next: dig into Bug E. OSS-456 parity continuation + OSS-450 integration umbrella open in parallel.
...
| Full suite passing (baseline) | **1090** |
| Last verified | 2026-06-25 on `main` (post PR #305; OSS-503 flatten-shorthand fix, +3 tests) |
