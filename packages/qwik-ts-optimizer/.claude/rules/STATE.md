...
Last updated: 2026-06-26 (PR #309 / [OSS-505](https://linear.app/kunai/issue/OSS-505) **merged** — Bug F: the raw-props consolidation gate counted *nested* `return`s as component shape, so a `useTask$(({ track }) => { …; if(!x) return; … })` handler got its tracker param rewritten to `(_rawProps)`, colliding with the captured outer-component `_rawProps`; fix counts only direct top-level `ReturnStatement`s). Active workstream [OSS-499](https://linear.app/kunai/issue/OSS-499): validate **qwik-design-system** under qwik-bundler + the TS optimizer (`experimental: ['tsOptimizer']`). **qds docs build now GREEN end-to-end** (exit 0, 1480 modules) — all six emit/parse bug classes fixed: A ([OSS-500](https://linear.app/kunai/issue/OSS-500)/#297), B ([OSS-501](https://linear.app/kunai/issue/OSS-501)/#302), C ([OSS-502](https://linear.app/kunai/issue/OSS-502)/#299), D ([OSS-503](https://linear.app/kunai/issue/OSS-503)/#305), E ([OSS-504](https://linear.app/kunai/issue/OSS-504)/#307), F ([OSS-505](https://linear.app/kunai/issue/OSS-505)/#309). **But build green ≠ app runs: running the dev SSR surfaced runtime Bug G (not filed) — every page 500s with `serverQrl is not a function`, a circular-dependency module-init-order crash in the `server$`/router-lib SSR path; TS-only (SWC renders 200), NOT import-order (tested end-to-end). Now the active focus — deep diagnosis + Vite harness recipe in `[[project_qds_validation_oss499]]`.** Convergence **203/9** unchanged; full **1092 → 1094** (+2).
...
| Full suite passing (baseline) | **1094** |
| Last verified | 2026-06-26 on `main` (post PR #309; OSS-505 Bug F fix, +2 tests) |
...
|---|---|---|---|---|
| `main` | `a0a2c87` (post PR #309) | ✅ | baseline | Active workstream: [OSS-499](https://linear.app/kunai/issue/OSS-499) — qwik-design-system validation under qwik-bundler + tsOptimizer. SWC baseline green; **6 emit/parse bug classes fixed (A–F)**: A ([OSS-500](https://linear.app/kunai/issue/OSS-500), PR #297), C ([OSS-502](https://linear.app/kunai/issue/OSS-502), PR #299), B ([OSS-501](https://linear.app/kunai/issue/OSS-501), PR #302), D ([OSS-503](https://linear.app/kunai/issue/OSS-503), PR #305), E ([OSS-504](https://linear.app/kunai/issue/OSS-504), PR #307), **F ([OSS-505](https://linear.app/kunai/issue/OSS-505), PR #309 — raw-props gate counts only top-level returns)**. **qds docs build now GREEN end-to-end** (44 → 39 → 3 → segment-parse → segment-codegen → `_rawProps` double-decl → green). **NEW runtime gap Bug G (not filed, ACTIVE FOCUS)**: dev SSR `serverQrl is not a function` on every page — a circular-dependency module-init-order crash in the `server$`/router-lib SSR path; TS-only (SWC renders 200), ruled out import-order. **+** OSS-456 — vite-qwik-router interactive e2e in dev under `tsOptimizer`, at parity with SWC. OSS-498 package rename → `qwik-ts-optimizer` — **In Progress until the maintainer runs `npm publish`**. Backlog: OSS-448 (scope-aware import DCE — `example_qwik_router_client` flip); OSS-471, OSS-473, OSS-465 RCA follow-ups; OSS-439 (F3); OSS-450 Sub-D. Code-hygiene track D paused (OSS-489 stage-2 backlog). |
