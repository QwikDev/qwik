...
Last updated: 2026-07-16 (**PR #361 ([OSS-541](https://linear.app/kunai/issue/OSS-541) umbrella) merged → `main` (`e685f23`)** — test-infrastructure de-any: `jsx-transform.test.ts` + `signal-analysis.test.ts` de-anied, shared typed helper module `tests/optimizer/helpers/parse-nodes.ts` extracted. Labeled refactor, test-only, 0 src touched; gate green (convergence **203/9** unchanged, full **1208** baseline all pass, 0 regressions). Remaining de-any tracked under **[OSS-541](https://linear.app/kunai/issue/OSS-541)** + subs [OSS-542](https://linear.app/kunai/issue/OSS-542) (tests/optimizer specs + oxc-walker pattern) / [OSS-543](https://linear.app/kunai/issue/OSS-543) (`src/testing/ast-compare.ts` oracle narrowing — corrected from the old "mechanical" mis-billing), all Backlog. **OSS-537 hygiene sweep COMPLETE** (OSS-538/539/540, PRs #357/#358/#359, all Done); **OSS-540 #4** (`_jsxDEV` component-param `_fnSignal` var→const bag) still awaits its qds head-to-head as the definitive validation. OSS-499 qds validation CLEAN (OSS-536 fixed+merged, 29-route re-sweep 0 TS-only divergences). **NEXT: OSS-456 parity, OSS-450 qwik-bundler integration, resume OSS-499 with the OSS-540 #4 qds head-to-head, or pick up OSS-542/543 de-any.**)
...
| Full suite passing (baseline) | **1208** all green (post-#361; passing set unchanged — the de-any refactor renamed/removed no test IDs) |
| Last verified | 2026-07-16 — post PR #361 merge (`e685f23`): 203/212 convergence + 1208 baseline all pass, 0 regressions |
