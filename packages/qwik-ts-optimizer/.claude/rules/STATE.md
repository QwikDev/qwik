...
| Metric | Value |
|---|---|
| Convergence failing | **30 / 212** |
| Convergence passing | **182 / 212** (85.8%) |
| Full suite failing | 53 / 698 |
| Full suite passing | 645 / 698 |
| Last verified | 2026-05-12 on `main` (post PR #48 merge, head `7616e94`) |
...
|---|---|---|---|---|
| `main` | `7616e94` (post PR #48 merge) | ✅ | baseline | F8a / OSS-361 closed; F1b / OSS-360 closed; refactor track v2 + F4 closed; new `simplify.ts` module ports first slice of SWC's `simplify::simplifier` pass. |
...
| F8 — diverse semantic bugs | PARTIAL | 4 remaining tests | F8a (`example_getter_generation`) closed via PR #48 ([OSS-361](https://linear.app/kunai/issue/OSS-361)) — new `simplify.ts` module; 4 remaining tests each need their own fix |
...
- **F8 remaining individual fixes** (4 tests, each its own bug). F8a closed via PR #48. Remaining order from CONVERGENCE_FAILURES.md: `should_transform_three_nested_loops_handler_captures_outer_only` (capture-analysis bug — production fix), `should_wrap_prop_from_destructured_array` (`_fnSignal` dedup + destructure flattening), `example_qwik_react` (migration — `_auto_filterProps`), `example_component_with_event_listeners_inside_loop` (hoist depth in loops).
