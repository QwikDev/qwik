...
| Last verified | 2026-05-09 on `main` (post PR #44 merge, head `45c66ec`) |
...
|---|---|---|---|---|
| `main` | `45c66ec` (post PR #44 merge) | ✅ | baseline | Refactor track v2 + F4 closed; production walkers fully typed. |
...
| _(no active workstream branch)_ | — | — | — | F4 closed. Next: F1b (mutual-recursion component migration) is the smallest and most natural follow-on; F8 individual fixes; F2 cluster rebase; or perf follow-ups. See "What to do next". |
...
- **F4-adjacent edge cases**. The compareAst additions PR #42 landed (`stripFrameworkHelperImports`, broader `isReorderableDeclaration`) are general — they may unblock other tests too. Worth scanning failure list to see if any previously-blocked tests now flip cleanly with a small targeted optimizer fix.
