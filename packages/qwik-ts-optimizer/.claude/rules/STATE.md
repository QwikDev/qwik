...
Last updated: 2026-06-08 ([OSS-470](https://linear.app/kunai/issue/OSS-470) reactive `_jsxDEV` emit on branch `fix/oss-470-jsxdev-reactive-emit`). **🎉 Milestone: `vite-qwik-router` renders AND is interactive — the counter button increments (Playwright-verified, at parity with SWC).** Three fixes this arc: OSS-464 (`_jsxDEV` handler extraction, PR #225, merged) + OSS-469 (router-lib `_captures`, PR #226, merged) + **OSS-470 (reactive `_jsxDEV` emit — `q:p` captures, const-bag handlers, `_wrapProp`, flags; both client + server JSX paths; PR pending)**. OSS-470 made the pre-transformed `_jsxDEV`→`_jsxSorted` output match SWC byte-for-byte, which wires event handlers + signal reactivity. **One button still broken**: the `server$` "Test server" RPC fails with Qwik `Q14` ("serverQrl … does not have a chunk path") — its handler now fires, but the `server$` chunk doesn't resolve. Root cause is a **migration-policy** divergence (TS reexports `const testServer$ = serverQrl(…)`; SWC *moves* it into the consuming segment), NOT the JSX emit — separate follow-up. Convergence 203/212 unchanged (functional fix). RCA at `RCA-vite-qwik-router.md`; systemic follow-ups under OSS-465.
...
| Full suite passing (baseline) | **998** |
| Last verified | 2026-06-08 on `main` (post PRs #225 + #226) |
...
|---|---|---|---|---|
| `fix/oss-470-jsxdev-reactive-emit` | (this branch) | ⏳ PR pending | 203 conv / 999 full + 2 new | OSS-470 — reactive `_jsxDEV` emit (q:p, const-bag, _wrapProp, flags). Counter button now increments (Playwright-verified). Touches `jsx-call-transform.ts`, `segment-generation.ts`, `segment-codegen.ts`, `rewrite/inline-body.ts`, `rewrite/index.ts`, `loop-hoisting.ts`. Next: `server$` Q14 (migration MOVE-vs-REEXPORT). |
| `main` | `1f7ae59` (PR #226) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router renders e2e (dev SSR) + builds green; OSS-464 + OSS-469 merged. Open backlog: OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3 multi-session); OSS-450 Sub-D in qwik-bundler PR #12. |
