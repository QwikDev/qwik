...
Last updated: 2026-06-10 ([OSS-486](https://linear.app/kunai/issue/OSS-486) walk/parse consolidation **In Progress** — steps 1+2 merged (PRs #261, #263): group-1 walk fusion (`computeClosureFreeIdentifiers`, walks −35%) + group-2 session threading (canonical wrapper + parse memo, parses −47%); **first Track D wall movement** — BENCH-01 3.06× → **2.96×**, 30-iter worst-case harness −9% cumulative from baseline. **Next step: census group 3** — fold per-module gather walks into the canonical walk; census table + per-step rationale in OSS-486 comments. OSS-487 (micros, Backlog): `countJsxKeyConsumption` item closed by #263; remaining = `computeSegmentUsage` quadratic + regex hoists.) Code-hygiene track: Tracks A, B, C, E closed; track D (perf) **active**. **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite **1035**. Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) per the Branches/What-next sections below.
...
| Full suite passing (baseline) | **1035** |
| Last verified | 2026-06-10 on `main` (post PR #263; baseline absorbed +6 net new tests) |
...
|---|---|---|---|---|
| `main` | `6460906` (PR #263) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router **interactive e2e in dev** (counter + `server$` RPC) under `tsOptimizer`, at parity with SWC; OSS-470 + OSS-472 merged. Backlog: OSS-471 (app migration MOVE — parity, not Q14), OSS-473 (spa_init dev-path), OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3); OSS-450 Sub-D. Prod-preview `_run` Q14 at parity with SWC (bundler/runtime). Code-hygiene track: OSS-474/475/476/477/478/479/480/482 (PRs #232/#245/#234/#236/#238/#240/#242/#251) merged — Tracks A, B, C, E fully closed; D open and unblocked (OSS-485 BENCH-01 crash fixed, PR #257). OSS-481 createOptimizer NAPI parity merged (PR #244). OSS-483 src reorg merged (PR #253) — phase/domain layout, `utils/` gone. |
