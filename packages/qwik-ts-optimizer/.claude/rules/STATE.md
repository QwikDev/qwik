...
Last updated: 2026-06-25 (PR #297 / [OSS-500](https://linear.app/kunai/issue/OSS-500) **merged** — Bug A whitespace fold). New active workstream [OSS-499](https://linear.app/kunai/issue/OSS-499): validate **qwik-design-system** (`/Users/scottweaver/Projects/qwik-design-system`, branch `codex/vite-8-new-optimizer`) end-to-end under qwik-bundler + the TS optimizer (`experimental: ['tsOptimizer']`). Integration wired (dep fixed to `qwik-ts-optimizer: file:../../TS-Optimizer`, `QDS_TS_OPTIMIZER=1` toggle); **SWC baseline build green** so the bundler wiring is sound. The tsOptimizer build surfaced 3 distinct optimizer bug classes: **Bug A ([OSS-500](https://linear.app/kunai/issue/OSS-500)) fixed here** — multi-line JSX attribute strings emitted raw newlines → "Unterminated string"; `jsx-props.ts` now folds whitespace to a valid single-line string (probed for parity), gated on the slice containing `\n`/`\r`/`\t`, +8 tests. Two remain (Backlog under OSS-499): **Bug B ([OSS-501](https://linear.app/kunai/issue/OSS-501))** corrupted re-emission of pre-optimized `@qds.dev/*` libs (idempotency; deterministic, 4 modules), **Bug C ([OSS-502](https://linear.app/kunai/issue/OSS-502))** duplicate imports on the rolldown `program`-input path. Also landed in #297: a CODING_BEST_PRACTICES.md "Comments" section (no WHAT/SWC-reference comments in code; tests self-document via names). Convergence **203/9** unchanged; full baseline **1073 → 1081** (+8 Bug A tests). OSS-456 parity continuation + OSS-450 integration umbrella open in parallel.
...
| Full suite passing (baseline) | **1081** |
| Last verified | 2026-06-25 on `main` (post PR #297; OSS-500 Bug A multi-line JSX attr whitespace fold, +8 tests) |
