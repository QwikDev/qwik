...
Last updated: 2026-06-10 ([OSS-486](https://linear.app/kunai/issue/OSS-486) walk/parse consolidation **In Progress** — step 1 (group-1 walk fusion) **merged, PR #261**: per-closure `getUndeclaredIdentifiersInFunction` calls in capture analysis / C02 / event-capture promotion replaced by one module-wide `computeClosureFreeIdentifiers` map (two program walks, node-identity keyed); walk invocations −52%/−35%, wall-neutral — the wall targets are group 2 (per-segment session threading, ~6.5k parses) and group 3 (per-module walk fusion), census on the ticket. Prior: Track D baseline established (PR #259, BENCH-01 3.06× / BENCH-02 4.56×) post-[OSS-485](https://linear.app/kunai/issue/OSS-485) crash fix (PR #257).) Code-hygiene track: Tracks A, B, C, E closed; track D (perf) **active** — OSS-486 (walk/parse, In Progress) + OSS-487 (micros, Backlog). **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite **1029**. Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) per the Branches/What-next sections below.
...
| Full suite passing (baseline) | **1029** |
| Last verified | 2026-06-10 on `main` (post PR #261; baseline auto-update absorbed +2 differential tests) |
...
|---|---|---|---|---|
| `main` | `1fbdc07` (PR #261) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router **interactive e2e in dev** (counter + `server$` RPC) under `tsOptimizer`, at parity with SWC; OSS-470 + OSS-472 merged. Backlog: OSS-471 (app migration MOVE — parity, not Q14), OSS-473 (spa_init dev-path), OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3); OSS-450 Sub-D. Prod-preview `_run` Q14 at parity with SWC (bundler/runtime). Code-hygiene track: OSS-474/475/476/477/478/479/480/482 (PRs #232/#245/#234/#236/#238/#240/#242/#251) merged — Tracks A, B, C, E fully closed; D open and unblocked (OSS-485 BENCH-01 crash fixed, PR #257). OSS-481 createOptimizer NAPI parity merged (PR #244). OSS-483 src reorg merged (PR #253) — phase/domain layout, `utils/` gone. |
