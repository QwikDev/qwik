...
Last updated: 2026-06-11 ([OSS-495](https://linear.app/kunai/issue/OSS-495) build-during-gather **Done** — PR #285 merged: the standalone ScopeTracker build walk is deleted; the canonical gather walk builds the tracker as it traverses (unfrozen → freeze on return → buffered free-identifier resolution). Census: total program walks **2,477 → 1,999 (−19.3%)**, trigger modules 3→2 (478 build walks gone), classification identical to the OSS-493 baseline; wall within variance per BENCHMARKS.md, matching the spike's ≈neutral projection — the census is the deliverable. OPTIMIZER.md two-traversal description corrected in the same PR. **Track D perf backlog: OSS-496 (fuse Phase-1 extraction into the gather walk, the 2→1 step closing the [OSS-492](https://linear.app/kunai/issue/OSS-492) arc) next; OSS-489 stage-2 transform batching deferred on ticket.** **In flight: PR #274 ([OSS-447](https://linear.app/kunai/issue/OSS-447) migration-family fixes, In Review)** — its Linear comment maps the residual families still blocking the `example_qwik_router_client` flip (key-prefix is the highest-leverage unfiled one). Code-hygiene track: Tracks A, B, C, E closed; track D (perf) active. **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite **1061** unchanged. Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) per the Branches/What-next sections below.
...
| Last verified | 2026-06-11 on `main` (post PR #285; no new test IDs — baseline unchanged at 1061) |
...
|---|---|---|---|---|
| `main` | `fad6fdd` (PR #285) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router **interactive e2e in dev** (counter + `server$` RPC) under `tsOptimizer`, at parity with SWC; OSS-470 + OSS-472 merged. Backlog: OSS-471 (app migration MOVE — parity, not Q14), OSS-473 (spa_init dev-path), OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3); OSS-450 Sub-D. Prod-preview `_run` Q14 at parity with SWC (bundler/runtime). Code-hygiene track: OSS-474/475/476/477/478/479/480/482 (PRs #232/#245/#234/#236/#238/#240/#242/#251) merged — Tracks A, B, C, E fully closed; D active (OSS-486..491 + OSS-493/494/495 Done; backlog: OSS-492 sub OSS-496, OSS-489 stage-2). OSS-481 createOptimizer NAPI parity merged (PR #244). OSS-483 src reorg merged (PR #253) — phase/domain layout, `utils/` gone. |
