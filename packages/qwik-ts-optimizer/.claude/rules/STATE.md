...
Last updated: 2026-06-10 ([OSS-486](https://linear.app/kunai/issue/OSS-486) walk/parse consolidation **Done** — full arc merged (PRs #261, #263, #265, #267): group-1 walk fusion + group-2 session threading + group-3 canonical gather walk (`analysis/module-gather-walk.ts`) + `profile-deep.test.ts` rebuilt as a working profiling harness. Cumulative: monorepo walks −44% from the pre-track census, parses −47%, **BENCH-01 3.06× → 2.71×** (TS wall 1674 → 1519 ms, two wall wins outside variance); all six acceptance boxes ticked, closing comment on the ticket. Track D remaining backlog: [OSS-487](https://linear.app/kunai/issue/OSS-487) micros (`computeSegmentUsage` quadratic + regex hoists) + the deliberately-deferred census items documented on the ticket. **Next pickup: per "What to do next"** — Candidate A (OSS-447 + OSS-448 parity) or Candidate B (OSS-450 bundler integration).) Code-hygiene track: Tracks A, B, C, E closed; track D (perf) **paused at a good stopping point**. **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite **1038**. Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) per the Branches/What-next sections below.
...
| Full suite passing (baseline) | **1038** |
| Last verified | 2026-06-10 on `main` (post PR #267; baseline absorbing the +1 harness flip) |
...
|---|---|---|---|---|
| `main` | `405f71c` (PR #267) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router **interactive e2e in dev** (counter + `server$` RPC) under `tsOptimizer`, at parity with SWC; OSS-470 + OSS-472 merged. Backlog: OSS-471 (app migration MOVE — parity, not Q14), OSS-473 (spa_init dev-path), OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3); OSS-450 Sub-D. Prod-preview `_run` Q14 at parity with SWC (bundler/runtime). Code-hygiene track: OSS-474/475/476/477/478/479/480/482 (PRs #232/#245/#234/#236/#238/#240/#242/#251) merged — Tracks A, B, C, E fully closed; D open and unblocked (OSS-485 BENCH-01 crash fixed, PR #257). OSS-481 createOptimizer NAPI parity merged (PR #244). OSS-483 src reorg merged (PR #253) — phase/domain layout, `utils/` gone. |
...
- Perf (hygiene track D, **paused — OSS-486 Done**): cumulative BENCH-01 3.06× → 2.71×, BENCH-02 4.56× → ~4.3× (caps 1.15× / 1.5× still far off); CPU profile + per-step results in `BENCHMARKS.md` "Update 2026-06-10". Remaining: [OSS-487](https://linear.app/kunai/issue/OSS-487) micros (Backlog) + census items deferred with rationale on OSS-486 (`flattenDestructureUseCalls`, `transformAllJsx`, emitted-text walkers — the canonical gather walk is the attachment point). File a Linear ticket per workstream; append `BENCHMARKS.md` row before/after.
