...
Last updated: 2026-07-08 (PR #324 — [OSS-515](https://linear.app/kunai/issue/OSS-515) **Done**). Active workstream [OSS-499](https://linear.app/kunai/issue/OSS-499): validate **qwik-design-system** under qwik-bundler + the TS optimizer. qds docs build GREEN + dev-SSR renders homepage / `/motion/` / `/code-playground/`; **`/components/*` pages now advance past `PropsPlayground`** after Bug N — a `q:p`-delivered handler capturing a destructured prop wasn't raw-props-consolidated, leaving the `q:p` value bare (`showAll is not defined` on every component page; [OSS-515](https://linear.app/kunai/issue/OSS-515), PR #324). Convergence 203/9; full 1131 → 1133 (+2). **Next: Bug O ([OSS-516](https://linear.app/kunai/issue/OSS-516), Backlog) — the `_jsxDEV` path never hoists reactive exprs to `_fnSignal`, over-subscribing hosts (192 `chore on already-streamed host` warnings; `/components/select/` `reading 'length'` crash). Partial fix on WIP branch `wip/oss-516-fnsignal-jsxdev` (scalar hoisting done, warnings 192→90); complex `.map()` reactive children the remaining blocker.** OSS-456 parity + OSS-450 integration open in parallel.
...
| Full suite passing (baseline) | **1133** |
| Last verified | 2026-07-08 on `main` (post PR #324 `82a3785`; OSS-515 q:p raw-props consolidation +2) |
...
|---|---|---|---|---|
| `main` | `82a3785` (post PR #324) | ✅ | baseline | Active workstream: [OSS-499](https://linear.app/kunai/issue/OSS-499) — qwik-design-system validation under qwik-bundler + tsOptimizer. SWC baseline green; **bug classes A–N fixed**; docs build GREEN and dev-SSR renders (homepage / `/motion/` / `/code-playground/` 200); **`/components/*` pages advance past `PropsPlayground`** after Bug N (`q:p`-delivered destructured-prop capture not raw-props-consolidated, [OSS-515](https://linear.app/kunai/issue/OSS-515), PR #324). **Next: Bug O ([OSS-516](https://linear.app/kunai/issue/OSS-516), Backlog) — `_jsxDEV` path missing `_fnSignal` hoisting → host over-subscription (192 warnings, `/components/select/` crash); partial fix on WIP `wip/oss-516-fnsignal-jsxdev` (192→90 warnings), complex `.map()` children the blocker.** **+** OSS-456 vite-qwik-router interactive e2e in dev at parity. OSS-498 rename → `qwik-ts-optimizer` In Progress until `npm publish`. Backlog: [OSS-512](https://linear.app/kunai/issue/OSS-512) (comment cleanup), OSS-448 (scope-aware import DCE), OSS-471, OSS-473, OSS-465 RCA follow-ups, OSS-439 (F3), OSS-450 Sub-D. Code-hygiene track D paused (OSS-489 stage-2). |
