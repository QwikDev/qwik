...
Last updated: 2026-06-10 ([OSS-487](https://linear.app/kunai/issue/OSS-487) Track D catalogued micros **Done** — PR #269: the segment-usage projection quadratic (inherited from `computeSegmentUsage` by the canonical gather walk) replaced with a sorted range-stack sweep; `wireMigration` per-move-decision program walk narrowed to the decl's enclosing top-level statement; DCE `ifBracelessPattern` + sCall-placement `\b`-tester regex hoists. The `countJsxKeyConsumption` item was already closed by OSS-486 group 2. Same-session A/B vs `main`: BENCH-02 −6% min wall (80 → 75 ms, ratio bands outside overlap — matches the profiled 4.2% + 2.3% share of the two fixed sites); BENCH-01 −3%, variance edge, not claimed. **Track D perf backlog is now the deliberately-deferred census items on OSS-486 only.** **Next pickup: per "What to do next"** — Candidate A (OSS-447 + OSS-448 parity) or Candidate B (OSS-450 bundler integration).) Code-hygiene track: Tracks A, B, C, E closed; track D (perf) **paused at a good stopping point**. **Milestone still standing: `vite-qwik-router` is interactive end-to-end in dev under the TS optimizer** (counter + `server$` RPC at parity with SWC; OSS-470 PR #228 + OSS-472 PR #230). Convergence **203/9** unchanged; full suite **1040**. Active workstream [OSS-456](https://linear.app/kunai/issue/OSS-456); parity backlog + RCA follow-ups (OSS-465) per the Branches/What-next sections below.
...
| Full suite passing (baseline) | **1040** |
| Last verified | 2026-06-10 on `main` (post PR #269; baseline absorbing the +2 new tests) |
...
|---|---|---|---|---|
| `main` | `087323b` (PR #269) | ✅ | baseline | Active workstream: OSS-456 — vite-qwik-router **interactive e2e in dev** (counter + `server$` RPC) under `tsOptimizer`, at parity with SWC; OSS-470 + OSS-472 merged. Backlog: OSS-471 (app migration MOVE — parity, not Q14), OSS-473 (spa_init dev-path), OSS-465 RCA follow-ups; OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3); OSS-450 Sub-D. Prod-preview `_run` Q14 at parity with SWC (bundler/runtime). Code-hygiene track: OSS-474/475/476/477/478/479/480/482 (PRs #232/#245/#234/#236/#238/#240/#242/#251) merged — Tracks A, B, C, E fully closed; D paused (OSS-486 + OSS-487 Done; deferred census items remain). OSS-481 createOptimizer NAPI parity merged (PR #244). OSS-483 src reorg merged (PR #253) — phase/domain layout, `utils/` gone. |
...
- **2026-06-10** — PR #269 ([OSS-487](https://linear.app/kunai/issue/OSS-487), **Done**) merged to `main` (`087323b`). Track D catalogued micros: the gather-walk segment-usage projection's per-visit extraction scan (the quadratic inherited from `computeSegmentUsage`) replaced by a sorted range-stack sweep, bounded-behavior-pinned by a Proxy-counted 200-extraction unit test (~7k range reads vs ~600k for the scan); `wireMigration` walks the decl's enclosing top-level statement instead of the whole program; `ifBracelessPattern` + sCall `\b`-tester regex hoists. Same-session A/B vs `main`: BENCH-02 −6% min wall (outside overlap, matches the profiled 4.2% + 2.3% share); BENCH-01 −3% (variance edge, not claimed). Full 1040 (+2); convergence 203/9. Remaining perf backlog: deferred census items on OSS-486 only.
- **2026-06-10** — PR #267 ([OSS-486](https://linear.app/kunai/issue/OSS-486) close-out, **Done**) merged to `main` (`405f71c`). `profile-deep.test.ts` rebuilt as a working profiling harness: deterministic 1-warmup + `PROFILE_ITERATIONS` workload driver for `--cpu-prof` attachment; `$QWIK_HOME` worst-case file with largest-fixture fallback so it registers and passes on CI (failing → passing flip, totals guard preserved). Last acceptance box on OSS-486 — ticket flipped Done with all six boxes ticked + closing-arc comment (cumulative: walks −44%, parses −47%, BENCH-01 3.06× → 2.71×). Full 1038 (+1); convergence 203/9. Track D paused; remaining perf backlog = OSS-487 + deferred census items.
- **2026-06-10** — PR #265 ([OSS-486](https://linear.app/kunai/issue/OSS-486) step 3, In Progress) merged to `main` (`ad08ad6`). Group-3 gather-walk fusion: new canonical gather walk (`analysis/module-gather-walk.ts`) folds lexical scopes, loop map, scope entries, segment usage, and passive-conflict detection into the group-1 free-identifier walk (7 per-module program walks → 2; segment usage moved Phase 3 → Phase 2, diagnostics emission kept at the Phase-4 site for order); peer-tool jsx-call-transform 4 walks → 2. Replaced walks retained as differential oracles, pinned by a per-projection corpus parity test. Monorepo walks −14.7% (19,836 → 16,920); **second Track D wall win**: BENCH-01 2.96× → 2.71× (TS −6.8%, outside same-session overlap), BENCH-02 within variance. Full 1037 (+2); convergence 203/9. Next: OSS-486 close-out — `profile-deep.test.ts` repair; deferred census items documented on the ticket.
- **2026-06-10** — PR #263 ([OSS-486](https://linear.app/kunai/issue/OSS-486) step 2, In Progress) merged to `main` (`6460906`). Group-2 session threading: canonical `TransformSession` wrapper + last-4 parse memo keyed by exact wrapped source (staleness impossible by key construction); direct-parse sites converted; `countJsxKeyConsumption` walks the Phase-1 closure node (closes that OSS-487 item). New `tolerateErrors` split pinned by unit test after `example_drop_side_effects` caught the recovered-program difference. Parses −50%/−47%; **first Track D wall win**: BENCH-01 2.96× (below baseline band), 30-iter harness −9% cumulative. Full 1035 (+6); convergence 203/9. **Next: census group 3** — fold the per-module gather walks (`computeSegmentUsage`, `collectAllScopeEntries`, `buildExtractionLoopMap`, `buildClosureLexicalScopes`, `detectPassivePreventdefaultConflicts`, peer-tool jsx-call-transform ×4, hand-rolled walkers) into the canonical walk; census table on the OSS-486 ticket comment.
- **2026-06-10** — PR #261 ([OSS-486](https://linear.app/kunai/issue/OSS-486) step 1, In Progress) merged to `main` (`1fbdc07`). Group-1 walk fusion: new `analysis/closure-free-identifiers.ts` computes free identifiers for all extraction closures in two whole-program walks (frozen ScopeTracker tree + depth-aware resolution, keyed by closure node identity); capture analysis, C02 diagnostics, and event-handler capture promotion consume slices of the shared map. Walk invocations −52% worst-case / −35% mono; wall-neutral (recorded honestly in BENCHMARKS.md — eliminated walks were subtree-sized). Differential test sweeps every function node in every fixture input for exact parity. Convergence 203/9; full 1029 (+2). Next: group 2 (session threading, the parse win).
- **2026-06-10** — PR #259 (Track D pickup, docs) merged to `main` (`d7feb19`). Perf baseline established post-OSS-485: BENCH-01 546/1674ms **3.06×** (TS wall +7% vs 2026-05-13, above the historical band — accumulated feature-work cost), BENCH-02 20/89ms **4.56×** (within band); min-of-3-runs methodology. Inclusive CPU profile recorded for both shapes in `BENCHMARKS.md`: walking 40–48% on-stack, parsing 13–17%, `generateSegments` ~36%; catalogued D1/D2/D3 combine to <8% → not needle-movers; perf workstream re-aimed at walk/parse consolidation (ticket to follow).
- **2026-06-10** — PR #257 ([OSS-485](https://linear.app/kunai/issue/OSS-485), Done) merged to `main` (`6204de8`). `applySignalHoistRenames` no longer commits its `_hf` renumbering as a whole-buffer `overwrite(0, len)` — the JSX walk records each inserted chunk via `writeJsxCall` (now the exit context’s only MagicString write path) and the rename re-overwrites just those ranges, keeping original offsets valid for later parent-rewrite passes (peer-tool `jsx()` rewrite, moved-decl slices). Fixes the `repl-console.tsx` crash BENCH-01 caught; benchmark measurable again (3.08× / 4.41× vs caps 1.15× / 1.5×) — Track D baseline unblocked. +2 regression tests (full 1027); convergence 203/9 unchanged; OPTIMIZER.md drifted line ref resynced in-PR.
- **2026-06-10** — PR #255 ([OSS-484](https://linear.app/kunai/issue/OSS-484), Done, hygiene track E — **track closed**) merged to `main` (`1e40d33`). Test-tree reorganization: 69 files re-homed to mirror the src phase/domain taxonomy, 30 `oss-*` filenames → behavior names, zero tracker refs left in test names/comments. First use of the deliberate-ID-migration procedure (now in REGRESSION.md): baseline derived by mapping the old Linux baseline (NOT regenerated locally — macOS registers 2 QWIK_HOME-gated benchmark skips that don't exist on CI; first CI run tripped the totals guard exactly so). 1:1 verified: passing 1025/1025, failing set identical, convergence untouched. Remaining hygiene track: D (perf).
- **2026-06-10** — PR #253 ([OSS-483](https://linear.app/kunai/issue/OSS-483), Done) merged to `main` (`c549408`). Source-tree reorganization: `src/optimizer/utils/` eliminated, all loose root files re-homed into phase/domain directories (`prepare/`/`extraction/`/`analysis/`/`rewrite/`/`segment/`/`jsx/` + `ast/`/`edit/`/`qwik/`/`diagnostics/`); 54 `git mv` moves with line-neutral import rewrites (OPTIMIZER.md line anchors unaffected), test-neutral (203/9, full 1025). Landing rule now stated in OPTIMIZER.md's code map: no `utils/` — a helper belongs to the domain it serves. Test-file re-homing deliberately deferred (baseline gate keys on test IDs; track-E-shaped follow-up).
- **2026-06-10** — PR #251 ([OSS-482](https://linear.app/kunai/issue/OSS-482), Done, hygiene track B4 — **Track B closed**) merged to `main` (`fa90510`). Unified the duplicated q:p JSX-walk collector into shared `jsx/qp-walk.ts` (resolver-callback design keeps segment-codegen's `elementQpParams` fallback byte-identical) + shared `eventHandlerPropName` with the passive-set delta documented rather than silently unified; `_`/`_1` check → `hasUnderscorePlaceholderParams`; inline hoist decl → `formatWCall`. +12 unit tests; test-neutral (203/9, full 1025 post-baseline); OPTIMIZER.md segment-codegen refs resynced in-PR (incl. pre-existing >50-line drift). Remaining hygiene tracks: D (perf), E (test-file tracker-ref rename).
## What to do next
...
- Perf (hygiene track D, **paused — OSS-486 Done**): cumulative BENCH-01 3.06× → 2.71×, BENCH-02 4.56× → ~4.3× (caps 1.15× / 1.5× still far off); CPU profile + per-step results in `BENCHMARKS.md` "Update 2026-06-10". Remaining: census items deferred with rationale on OSS-486 ([OSS-487](https://linear.app/kunai/issue/OSS-487) micros Done via PR #269) (`flattenDestructureUseCalls`, `transformAllJsx`, emitted-text walkers — the canonical gather walk is the attachment point). File a Linear ticket per workstream; append `BENCHMARKS.md` row before/after.
