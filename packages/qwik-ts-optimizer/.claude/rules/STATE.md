...
| Last verified | 2026-05-09 on `main` (post PR #30 merge, head `52bbc30`) |
...
- **Validated on real PRs**: every refactor-track PR through PR #24 has run the gate green — PR #10 (OSS-339), PR #11 (OSS-340), PR #12 (OSS-344), PR #13 (OSS-345), PR #14 (OSS-346), PR #17 (OSS-349 docs-only), PR #18 (OSS-350), PR #20 (OSS-351), PR #22 (OSS-353), PR #23 (OSS-354), PR #24 (OSS-355). Doc/planning-only PRs (PR #15 OSS-348, PR #16 REGRESSION, PR #19 + PR #21 + PR #25 + PR #27 + PR #29 STATE refreshes, PR #26 METHODOLOGIES post-merge routine, PR #28 BENCHMARKS.md bootstrap, PR #30 OSS-347 SPEC) also ran clean since they don't change source.
...
|---|---|---|---|---|
| `main` | `52bbc30` (post PR #30 merge) | ✅ | baseline | All track v1 + track v2 implementation work landed; OPTIMIZER.md / REGRESSION.md / types.ts JSDoc / extract.ts JSX fold-in / closure-node threading / closure-form const literals / portable benchmark harness / post-merge cleanup routine / BENCHMARKS.md perf-history doc / OSS-347 segment-generation-refactor SPEC all shipped |
| `refactor/segment-gen-prep-inline` | (about to be created) | — | baseline | OSS-356 / Sub-A — Prep + inline-strategy extraction. ~1.5h, low blast radius. |
...
| [OSS-356](https://linear.app/kunai/issue/OSS-356) | Sub-A — Extract `Prep` + inline-strategy from `generateAllSegmentModules` | `refactor/segment-gen-prep-inline` (about to start) | **In Progress** |
| [OSS-357](https://linear.app/kunai/issue/OSS-357) | Sub-B — Extract migration-wiring + nested call-sites | (depends on Sub-A merge) | Backlog |
| [OSS-358](https://linear.app/kunai/issue/OSS-358) | Sub-C — Extract `buildDefaultStrategySegment` sequencer; final orchestrator shape | (depends on Sub-A AND Sub-B merge) | Backlog |
...
**Active: [OSS-356](https://linear.app/kunai/issue/OSS-356) (Sub-A of OSS-347).** Branch as `refactor/segment-gen-prep-inline`. Read `OPTIMIZER.md`'s "Quick reference — code map" + the `.planning/specs/segment-generation-refactor.md` SPEC for the orchestrator's current shape and the seams identified. Extract `computeSegmentGenerationPrep` + `buildInlineStrategySegment` + a shared `consolidateRawPropsCaptures` helper. Run convergence + full-suite + `failure-families.test.ts` against `main` baseline before each commit; commit message follows the four-question refactor format from `METHODOLOGIES.md`. Auto-merge isn't available for this PR (it modifies source code, not just STATE.md).

**Sub-B (OSS-357) and Sub-C (OSS-358) follow strictly after Sub-A merges.** Each has its own scope documented in the SPEC and its Linear ticket. Don't start B before A merges; don't start C before B merges.
