...
Last updated: 2026-06-08 (OSS-464 fix on branch `fix/oss-464-jsxdev-handler-extraction`). **Milestone: the `vite-qwik-router` route-level runtime crash (`testServer$ is not defined`) is FIXED — pre-transformed `_jsxDEV` event handlers now extract correctly; client + server builds green, route SSR no longer crashes on the handler.** Runtime now advances to a **new wall in the router *library*** (`_captures is not defined` in `useQwikRouter`'s `useTask$`) — a separate OSS-456 router-lib-processing bug, not OSS-464. RCA at `RCA-vite-qwik-router.md`; systemic follow-ups under OSS-465. Convergence 203/212 unchanged (OSS-464 is a functional fix, not a convergence flip).
...
**Active workstream**: [OSS-456](https://linear.app/kunai/issue/OSS-456) qwik-router lib processing parity (FAST-TRACK). **`vite-qwik-router` BUILDS green (client + server)** under `experimental: ['tsOptimizer']` — 8 sub-bugs fixed: OSS-457, OSS-459 (PR #211); OSS-458 origin/extension (PR #215); OSS-460 duplicate core import (PR #217); OSS-461 inlinedQrl rawProps + DCE nested-fold (PR #219); OSS-462 stranded PURE annotation (PR #221); OSS-463 reexported `server$` binding (PR #222); **[OSS-464](https://linear.app/kunai/issue/OSS-464) pre-transformed `_jsxDEV` handler extraction (branch `fix/oss-464-jsxdev-handler-extraction`, PR pending)**. The route-level `testServer$ is not defined` runtime crash is resolved. **Runtime now fails further in** — `_captures is not defined` in the router *library*'s `useQwikRouter` `useTask$` (the documented next wall, unmasked by the OSS-464 fix; untouched by it).
...
**OSS-464 — DONE (PR pending).** Root cause confirmed (TS-optimizer gap; SWC handles it): in the bundler pipeline esbuild pre-transforms JSX to `_jsxDEV(tag, { onClick$: () => … })` before the optimizer, and the optimizer never extracted `$`-suffixed handlers from the object-property form (only from raw JSX attributes), so the handler stayed inline and its `server$`-bound `const` got mis-attributed and dropped. Fix: a Phase-1 `Property` extraction branch (`extract.ts`) flagged `isJsxObjectProp`, routed to the bare-value call-site rewrite in both `buildNestedCallSites` (segment path) and `rewrite/inline-body.ts` (inline/hoist path); plus a SWC-matching `children`-key skip in the JSX-bag naming push. Verified end-to-end against the real fixture (client + server build green; `testServer$` crash gone). Seeds the OSS-466 `_jsxDEV` test tier (`tests/optimizer/jsxdev-handler-extraction.test.ts`).

**Next wall (re-characterized 2026-06-08, ticket pending)**: with the route fixed, SSR now reaches the router *library*'s `useQwikRouter` `useTask$` segment (`@qwik.dev/router/lib/index.qwik.mjs`, `useQwikRouter_useTask_XpalYii770E`) and throws `_captures is not defined` at runtime — the segment references `_captures` without it being in scope/imported. This is the same segment STATE.md previously flagged for a brace-count *build* error under the client strip config; under the real dev SSR (server, hoist, `stripEventHandlers: true`) it surfaces as a runtime missing-`_captures`. Separate OSS-456 router-lib-processing bug; needs its own ticket. Repro recipe in project memory `[[reference-router-build-repro]]`.
...
|---|---|---|---|---|
| `fix/oss-464-jsxdev-handler-extraction` | (this branch) | ⏳ PR pending | 203 conv / 996 full + 2 new | OSS-464 — pre-transformed `_jsxDEV` handler extraction. Route-level `testServer$` crash fixed; client+server build green. Touches `extract.ts`, `rewrite/inline-body.ts`, `transform/segment-generation.ts` + OPTIMIZER.md note. |
| `main` | `4c6cd74` (PR #223) | ✅ | baseline | Active workstream: OSS-456. Open backlog: OSS-465 RCA follow-ups (incl. the router-lib `_captures` next wall); OSS-447 + OSS-448 (block `example_qwik_router_client` flip); OSS-439 (F3 multi-session); OSS-450 Sub-D in qwik-bundler PR #12. |
...
- **2026-06-08** — branch `fix/oss-464-jsxdev-handler-extraction` ([OSS-464](https://linear.app/kunai/issue/OSS-464), PR pending): extract `$`-suffixed event handlers from pre-transformed `_jsxDEV(tag, { onClick$: () => … })` object-property form (a new Phase-1 `Property` branch flagged `isJsxObjectProp`, routed to the bare-value call-site rewrite in both the segment-codegen and inline/hoist paths) + a SWC-matching `children`-key naming skip. Fixes the route-level `testServer$ is not defined` crash; verified end-to-end (client + server build green). Convergence 203/212 unchanged; +2 functional tests (`jsxdev-handler-extraction.test.ts`, the OSS-466 `_jsxDEV` tier seed). Unmasks the next wall: `_captures is not defined` in the router lib `useQwikRouter` `useTask$`.
