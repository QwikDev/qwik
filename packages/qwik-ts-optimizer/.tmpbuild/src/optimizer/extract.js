...
import { collectImports, collectCustomInlined, getCalleeName, isMarkerCall, isSyncMarker, getCtxKind, getCtxName, } from './marker-detection.js';
 ...
 * contains JSX nodes.
 */
function determineExtension(argNode, sourceExt) {
    let hasJsx = false;
    walk(argNode, {
        enter(node) {
...
function resolveCanonicalCalleeName(calleeName, imports) {
 ...
 */
function getDirectWrapperContextName(node, parent, imports, customInlined) {
    if (parent?.type !== 'CallExpression')
        return null;
    if (!Array.isArray(parent.arguments) || !parent.arguments.some((arg) => arg === node)) {
        return null;
    }
...
function computeQrlCallee(calleeName) {
    if (calleeName === '$')
        return '';
    if (calleeName === 'sync$')
        return '_qrlSync';
...
function computeLineCol(source, offset) {
    let line = 1;
    let col = 1;
...
function collectSegmentImports(bodyNode, imports) {
...
function isComponentTag(tagNode) {
 ...
 */
export function extractSegments(source, relPath, scope, transpileJsx, preParsedProgram) {
    const program = preParsedProgram ?? parseSync(relPath, source, { experimentalRawTransfer: true }).program;
    ...
    walk(program, {
        enter(node, parent) {
            ...
            if (node.type === 'ExportDefaultDeclaration') {
                ...
                const hasName = (decl?.type === 'FunctionDeclaration' && decl.id) ||
            ...
            if (node.type === 'CallExpression' &&
                ...
                const nameValue = arg1?.type === 'StringLiteral'
                    ...
                    : (arg1?.type === 'Literal' && typeof arg1.value === 'string')
                        ? arg1.value
                        : null;
                const isNullBody = arg0 && (arg0.type === 'NullLiteral' ||
                ...
                if (arg0 && nameValue && !isNullBody) {
                    ...
                    let explicitCapturesText = null;
                    const inlinedCaptureNames = [];
                ...
                if (pushCount > 0)
                    pushedNodes.set(node, pushCount);
                return;
            }
 ...
 */
function disambiguateExtractions(extractions, fileStem, relPath, scope) {
