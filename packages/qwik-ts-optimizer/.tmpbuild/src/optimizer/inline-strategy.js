 ...
 */
export function getSentinelCounter(index) {
 ...
 * ```
 */
export function buildNoopQrlDeclaration(symbolName) {
 ...
 * ```
 */
export function buildNoopQrlDevDeclaration(symbolName, devMeta) {
    return (`const q_${symbolName} = /*#__PURE__*/ _noopQrlDEV("${symbolName}", {\n` +
        `    file: "${devMeta.file}",\n` +
        `    lo: ${devMeta.lo},\n` +
        `    hi: ${devMeta.hi},\n` +
 ...
 * ```
 */
export function buildStrippedNoopQrl(symbolName, strippedIndex) {
 ...
 * ```
 */
export function buildStrippedNoopQrlDev(symbolName, strippedIndex, devMeta) {
    const counter = getSentinelCounter(strippedIndex);
    return (`const q_qrl_${counter} = /*#__PURE__*/ _noopQrlDEV("${symbolName}", {\n` +
        `    file: "${devMeta.file}",\n` +
        `    lo: ${devMeta.lo},\n` +
        `    hi: ${devMeta.hi},\n` +
        `    displayName: "${devMeta.displayName}"\n` +
 ...
 * ```
 */
export function buildSCall(varName, bodyText) {
 ...
 * ```
 */
export function buildHoistConstDecl(symbolName, bodyText) {
 ...
 * ```
 */
export function buildHoistSCall(qrlVarName, symbolName) {
    return `${qrlVarName}.s(${symbolName});`;
}
//# sourceMappingURL=inline-strategy.js.map