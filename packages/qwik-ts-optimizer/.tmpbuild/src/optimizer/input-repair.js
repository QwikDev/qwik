 ...
 */
export function repairInput(source, filename) {
    const initial = parseSync(filename, source, { experimentalRawTransfer: true });
 ...
 */
function tryRemoveUnmatchedParens(source, filename) {
    let openCount = 0;
    let closeCount = 0;
    ...
    for (let i = closePositions.length - 1; i >= 0; i--) {
        ...
        const result = parseSync(filename, candidate, { experimentalRawTransfer: true });
 ...
 */
function tryWrapJsxTextArrows(source, filename) {
    ...
    const result = parseSync(filename, repaired, { experimentalRawTransfer: true });
    if (result.program.body.length > 0)
        return repaired;
    return null;
}
 ...
 */
function findJsxTextRegionsWithGt(source) {
    const regions = [];
    ...
    for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
        ...
        const isSkippable = trimmed.length === 0 ||
            trimmed.startsWith('<') ||
            trimmed.startsWith('{') ||
            trimmed.startsWith('//') ||
            trimmed === '>' ||
...
//# sourceMappingURL=input-repair.js.map