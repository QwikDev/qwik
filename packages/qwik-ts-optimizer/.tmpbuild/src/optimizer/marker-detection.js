...
function isQwikCoreSource(source) {
    return QWIK_CORE_PREFIXES.some((prefix) => source === prefix || source.startsWith(prefix + '/'));
}
...
export function collectCustomInlined(program) {
...
export function getCalleeName(callExpr) {
 ...
 */
export function isMarkerCall(callExpr, imports, customInlined) {
...
export function isBare$(callExpr) {
    return getCalleeName(callExpr) === '$';
}
/** sync$ is a marker but does NOT extract a segment. */
export function isSyncMarker(calleeName) {
    return calleeName === 'sync$';
}
export function getCtxKind(_calleeName, isJsxEventAttr, isJsxNonEventAttr = false) {
    if (isJsxEventAttr)
        return 'eventHandler';
...
export function getCtxName(calleeName, isJsxEventAttr, jsxAttrName) {
    return (isJsxEventAttr && jsxAttrName) ? jsxAttrName : calleeName;
}
//# sourceMappingURL=marker-detection.js.map