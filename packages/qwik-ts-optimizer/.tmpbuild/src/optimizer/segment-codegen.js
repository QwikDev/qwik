 ...
 */
import { createRegExp, exactly, oneOrMore, maybe, wordChar, wordBoundary, whitespace, charNotIn, global } from 'magic-regexp';
...
const qwikDisableDirective = createRegExp(exactly('/*').and(whitespace.times.any()).and('@qwik-disable-next-line')
    .and(oneOrMore(whitespace)).and(oneOrMore(wordChar))
    .and(whitespace.times.any()).and('*/').and(whitespace.times.any()).and(maybe(exactly('\n'))), [global]);
const qrlConstName = createRegExp(exactly('const').and(oneOrMore(whitespace)).and(exactly('q_').and(oneOrMore(charNotIn(' \t\n\r'))).grouped()));
const qrlSuffixPattern = createRegExp(wordBoundary.and(oneOrMore(wordChar).and('Qrl').grouped()).and(wordBoundary), [global]);
const funcSignaturePattern = createRegExp(whitespace.times.any().and('function').and(whitespace.times.any()).and(wordChar.times.any()).and(whitespace.times.any()).grouped()
    .and('(').and(charNotIn(')').times.any().grouped()).and(')').at.lineStart());
// ── Shared helper: insert an import before the first '//' separator ──
function insertImportBeforeSeparator(parts, importStmt) {
...
function partsHaveImport(parts, symbol) {
    return parts.some(p => p.includes(`{ ${symbol} }`) || p.includes(`{ ${symbol},`) ||
 ...
 */
function replacePropsFieldReferences(bodyText, fieldMap) {
    const wrappedSource = `(${bodyText})`;
    let parseResult;
    try {
        parseResult = parseSync('__rpf__.tsx', wrappedSource, { experimentalRawTransfer: true });
    }
    catch {
        return bodyText;
    }
 ...
 */
export function injectCapturesUnpacking(bodyText, captureNames) {
...
function injectIntoBlockBody(bodyText, line) {
 ...
 */
function findEnclosingArrowBodyForCapture(text, pos, capturedVarName) {
    let i = pos - 1;
    while (i >= 1) {
        if (text[i] !== '(' && text[i] !== '{') {
            i--;
 ...
 */
function findVarDeclarationEnd(text, startPos, varName) {
 ...
 */
function collectBodyIdentifiers(bodyText) {
    const ids = new Set();
    try {
        const wrapped = `(${bodyText})`;
        const parsed = parseSync('segment.tsx', wrapped, { experimentalRawTransfer: true });
        let funcNode = null;
        walk(parsed.program, {
            enter(node) {
 ...
 */
function buildSegmentImports(extraction, capturedNames, importContext) {
    const parts = [];
    ...
    for (const imp of extraction.segmentImports) {
        ...
        const spec = { localName: imp.localName, importedName: imp.importedName };
        if (existing) {
 ...
 */
function addCaptureAndMigrationImports(parts, captureInfo) {
...
function addNestedQrlDeclarations(parts, nestedQrlDecls) {
    ...
    if (nestedQrlDecls.some(d => d.includes('qrl(') && !d.includes('_noopQrl(') && !d.includes('qrlDEV(')))
 ...
 */
function rewriteNestedCallSitesInline(bodyText, nestedCallSites, bodyOffset) {
...
function injectHoistDeclarations(bodyText, hoistDeclarations) {
    ...
    for (const hoist of hoistDeclarations) {
        ...
        if (charBefore === '(') {
            ...
            while (closeIdx < bodyText.length && depth > 0) {
                if (bodyText[closeIdx] === '(')
                    depth++;
                else if (bodyText[closeIdx] === ')')
                    depth--;
                closeIdx++;
            }
            closeIdx--;
...
function injectComponentScopeWDecls(bodyText, decls) {
...
function inlineEnumReferences(bodyText, enumValueMap) {
...
function applyRawPropsIfComponent(bodyText, extraction, parts) {
 ...
 */
function stripDiagnosticsAndDirectives(bodyText) {
    ...
    bodyText = bodyText.replace(/<(\w+)([^>]*?)>/g, (_match, tagName, attrsStr) => {
        ...
        if (elementPassive.size > 0) {
            cleaned = cleaned.replace(/\s*preventdefault:(\w+)/g, (pdFull, eventName) => {
 ...
 */
function transformSegmentJsx(bodyText, parts, jsxOptions, nestedCallSites, captureInfo) {
    // Fast check: skip parseSync for segments without actual JSX
    if (!(/(?:<[A-Z_a-z\/]|JSX)/.test(bodyText)))
        return { bodyText };
    try {
        const wrappedBody = `(${bodyText})`;
        const bodyParse = parseSync('segment.tsx', wrappedBody, { experimentalRawTransfer: true });
        ...
        const jsxResult = transformAllJsx(wrappedBody, bodyS, bodyParse.program, jsxOptions.importedNames, undefined, jsxOptions.devOptions, jsxOptions.keyCounterStart, true, qpOverrides, qrlsWithCaptures, jsxOptions.paramNames, jsxOptions.relPath, undefined, segConstIdents);
...
function buildQrlsWithCapturesSet(nestedCallSites) {
    if (!nestedCallSites)
        return undefined;
...
function transformSyncCalls(bodyText, parts) {
...
function ensureCoreImports(bodyText, parts) {
 ...
 */
function recollectPostTransformImports(bodyText, parts, importContext, importsBySource, capturedNames, nestedCallSites) {
...
function addSameFileImport(parts, id, importContext) {
...
function addQrlCalleeImports(parts, bodyText, nestedCallSites, _importContext) {
    if (!nestedCallSites) {
 ...
 */
function normalizeSeparators(parts) {
 ...
 */
export function generateSegmentCode(extraction, nestedQrlDecls, captureInfo, jsxOptions, nestedCallSites, importContext, enumValueMap) {
    const capturedNames = new Set(captureInfo ? captureInfo.captureNames : []);
    let segmentKeyCounterValue;
 ...
 */
export function removeDeadConstLiterals(bodyText) {
    const wrapper = `const __dce__ = ${bodyText}`;
    let parsed;
    try {
        parsed = parseSync('__dce__.tsx', wrapper, { experimentalRawTransfer: true });
    }
    catch {
        return bodyText;
    }
    if (!parsed?.program?.body?.[0])
        return bodyText;
 ...
 */
export function rewriteFunctionSignature(bodyText, paramNames) {
    const paramList = paramNames.join(', ');
    const arrowIdx = findArrowIndex(bodyText);
    if (arrowIdx !== -1) {
        let parenEnd = arrowIdx - 1;
        while (parenEnd >= 0 && /\s/.test(bodyText[parenEnd]))
            parenEnd--;
        if (bodyText[parenEnd] === ')') {
            let depth = 1;
            let parenStart = parenEnd - 1;
            while (parenStart >= 0 && depth > 0) {
                if (bodyText[parenStart] === ')')
                    depth++;
                else if (bodyText[parenStart] === '(')
                    depth--;
                parenStart--;
            }
            parenStart++;
        ...
        while (identStart > 0 && /\w/.test(bodyText[identStart - 1]))
            identStart--;
...
//# sourceMappingURL=segment-codegen.js.map