 ...
 */
function forEachChildNode(node, visitor) {
...
export function isSignalValueAccess(node) {
    return (node != null &&
 ...
 */
export function isStoreFieldAccess(node, importedNames, localNames) {
    ...
    if (localNames && !localNames.has(objName))
        return false;
...
function containsJsx(node) {
    ...
    if (Array.isArray(node))
        return node.some(containsJsx);
    if (node.type === 'JSXElement' || node.type === 'JSXFragment')
        return true;
    for (const key of Object.keys(node)) {
        if (key === 'type')
            continue;
        if (containsJsx(node[key]))
            return true;
    }
    return false;
}
 ...
 */
function containsUnknownCall(node, importedNames) {
    if (node == null)
        return false;
    ...
    forEachChildNode(node, (child) => {
        if (!found && containsUnknownCall(child, importedNames))
            found = true;
    });
    return found;
}
...
function containsImportedReference(node, importedNames) {
    if (node == null)
        return false;
    ...
    let found = false;
    forEachChildNode(node, (child) => {
        if (!found && containsImportedReference(child, importedNames))
            found = true;
    });
    return found;
}
...
function getMemberChainRoot(node) {
...
function isDeepStoreAccess(node, importedNames, localNames) {
    if (node.type !== 'MemberExpression')
        return false;
    ...
    if (GLOBAL_NAMES.has(root))
        return false;
    if (localNames && !localNames.has(root))
        return false;
    return true;
}
/**
 ...
 */
function collectIdentifiersFromExpr(node, importedNames, seen, roots) {
    if (node == null)
        return;
    if (node.type === 'Identifier') {
 ...
 */
function collectReactiveRoots(node, importedNames, localNames) {
    const roots = [];
    const seen = new Set();
    function walk(n) {
        if (n == null)
 ...
 */
function collectAllDeps(node, importedNames) {
    const reactiveRoots = [];
    const bareIdents = [];
    const seen = new Set();
    function walk(n) {
        ...
        for (const key of Object.keys(n)) {
            ...
            if (key === 'key' && (n.type === 'Property' || n.type === 'ObjectProperty'))
                continue;
            if (key === 'property' && (n.type === 'MemberExpression' || n.type === 'StaticMemberExpression') && !n.computed)
                continue;
            const val = n[key];
            if (val && typeof val === 'object') {
                if (Array.isArray(val)) {
 ...
 */
function generateFnSignal(exprNode, source, roots) {
    const rootToParam = new Map();
    ...
    const replacements = [];
    function collectReplacements(n, parentKey, parentNode) {
        if (n == null)
            return;
 ...
 */
function stripTernaryConditionParens(text) {
 ...
 */
function normalizeStringQuotes(text) {
    let result = '';
    for (let i = 0; i < text.length; i++) {
        const ch = text[i];
        if (ch === "'") {
 ...
 */
export function analyzeSignalExpression(exprNode, source, importedNames, localNames) {
    if (exprNode == null)
        return { type: 'none' };
 ...
 */
function tryBuildFnSignal(exprNode, source, importedNames, localNames) {
    const roots = collectReactiveRoots(exprNode, importedNames, localNames);
    if (roots.length === 0)
        return { type: 'none' };
...
function analyzeCallExpression(exprNode, source, importedNames, localNames) {
...
function analyzeMemberExpression(exprNode, source, importedNames, localNames) {
...
export class SignalHoister {
     ...
     */
    buildRenameMap() {
        if (this.hoistedFunctions.length <= 1)
            return null;
