...
import { detectLoopContext, generateParamPadding, } from "./loop-hoisting.js";
import { transformSync as oxcTransformSync } from "oxc-transform";
import { emitC02, emitC05, emitPreventdefaultPassiveCheck, classifyDeclarationType, parseDisableDirectives, filterSuppressedDiagnostics, } from "./diagnostics.js";
import { computeOutputExtension, computeParentModulePath, computeRelPath, getExtension, } from "./path-utils.js";
const exportConstAssign = createRegExp(exactly('export').and(oneOrMore(whitespace)).and('const').and(oneOrMore(whitespace))
    .and(oneOrMore(wordChar)).and(whitespace.times.any()).and('=').and(whitespace.times.any()));
const exportConstLine = createRegExp(exactly('export const ').and(oneOrMore(wordChar)).and(' = ').at.lineStart(), [multiline]);
...
const ifBracedBoolLiteral = createRegExp(wordBoundary.and('if').and(whitespace.times.any()).and('(').and(whitespace.times.any())
    .and(anyOf('true', 'false').grouped()).and(whitespace.times.any()).and(')').and(whitespace.times.any()).and('{'), [global]);
const trueAndOp = createRegExp(wordBoundary.and('true').and(whitespace.times.any()).and('&&').and(whitespace.times.any()), [global]);
const falseOrOp = createRegExp(wordBoundary.and('false').and(whitespace.times.any()).and('||').and(whitespace.times.any()), [global]);
const falseAndOp = createRegExp(wordBoundary.and('false').and(whitespace.times.any()).and('&&').and(whitespace.times.any()), [global]);
const elseClause = createRegExp(whitespace.times.any().and('else').and(whitespace.times.any()).and('{').at.lineStart());
const nsImportPattern = createRegExp(exactly('*').and(oneOrMore(whitespace)).and('as').and(oneOrMore(whitespace)).and(oneOrMore(wordChar).grouped()));
const dceGuard = createRegExp(wordBoundary.and(anyOf(exactly('if').and(whitespace.times.any()).and('(').and(whitespace.times.any())
    .and(anyOf('true', 'false', '!true', '!false')).and(wordBoundary), exactly('true').and(whitespace.times.any()).and('&&'), exactly('false').and(whitespace.times.any()).and('||'), exactly('false').and(whitespace.times.any()).and('&&'))));
/**
 ...
 */
function applySegmentConstReplacement(code, filename, isServer) {
 ...
 */
function injectUseHmr(segmentCode, devFile) {
...
function applySegmentDCE(code) {
    ...
    result = result.replace(notTrueLiteral, (match, offset) => {
        if (isInsideString(result, offset))
            return match;
        return "false";
    });
 ...
 */
function simplifyFalseAndExpressions(code) {
 ...
 */
function applySegmentSideEffectSimplification(code, filename) {
                ...
                for (const declarator of node.declarations) {
                    if (declarator.id?.type === "Identifier" && declarator.init) {
                        ...
                        if (node.declarations?.length > 1)
                            continue; // skip multi-declarator
                        varDecls.push({
 ...
 */
function extractBinaryOperandIdentifiers(text) {
    const ids = [];
    const seen = new Set();
 ...
 */
function removeUnusedImports(code, filename, transpileJsx) {
    ...
    const unreferencedSpecs = importSpecs.filter((spec) => {
        ...
        if (isQwikImport && !transpileJsx) {
            ...
            if (quoteChar === "'") {
                ...
                const hasNonDollarSpec = (spec.node.specifiers ?? []).some((s) => {
 ...
 */
function buildPassthroughModule(repairedCode, relPath, origPath, cachedProgram) {
    const program = cachedProgram ??
        parseSync(relPath, repairedCode, { experimentalRawTransfer: true })
            .program;
    ...
    const bodyReferencedNames = new Set();
    ...
    if (lastImportEnd >= 0) {
        ...
        while (bodyStart < repairedCode.length &&
 ...
 */
function buildEnclosingExtractionMap(extractions) {
    const map = new Map();
 ...
 */
function postProcessSegmentCode(code, opts) {
    let result = code;
    const filename = opts.canonicalFilename + opts.extension;
    // Strip TS types when transpileTs is enabled
    if (opts.shouldTranspileTs) {
        const hasTsSyntax = /(?::\s*[A-Z_$\w{[(]|<\w+>(?!\s*[),;])|<\w+,|\bas\s+\w|interface\s+\w|type\s+\w|enum\s+\w|\w+\s*!\.|\w+\s*!\[|<[A-Z]\w*(?:\s*,\s*\w+)*\s*>\s*\()/.test(result);
        if (hasTsSyntax) {
            const tsStripOptions = {
                typescript: { onlyRemoveTypeImports: false },
            };
            if (!opts.shouldTranspileJsx) {
                tsStripOptions.jsx = "preserve";
            }
            const sourceExt = opts.sourceExtensions.get(opts.symbolName) ?? opts.extension;
            const tsStripped = oxcTransformSync(opts.canonicalFilename + sourceExt, result, tsStripOptions);
            if (tsStripped.code) {
 ...
 */
export function transformModule(options) {
    const allModules = [];
    const diagnostics = [];
    ...
    for (const input of options.input) {
        ...
        const needsJsxTransform = options.transpileJsx !== false && (ext === ".tsx" || ext === ".jsx");
        if (extractions.length === 0 && !needsJsxTransform) {
            allModules.push(buildPassthroughModule(repairedCode, relPath, input.path, repairResult.program));
            continue;
        }
        // Phase 2: Collect imports and analyze captures
        const program = repairResult.program ??
            parseSync(relPath, repairedCode, { experimentalRawTransfer: true })
                .program;
        const originalImports = collectImports(program);
        const importedNames = new Set(originalImports.keys());
        ...
        const closureNodes = new Map(); // symbolName -> closureNode
        const bodyScopeIds = new Map(); // symbolName -> scope ids from body
        const bodyPrograms = new Map(); // symbolName -> parsed program (for reuse)
        ...
        for (const extraction of extractions) {
            ...
            if (extraction.isInlinedQrl) {
                if (extraction.explicitCaptures) {
                    ...
                    const identCaptures = items.filter((s) => /^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(s) &&
                        s !== "true" &&
                        s !== "false" &&
                        s !== "null" &&
                        s !== "undefined");
        ...
        const globalDeclPositions = new Map();
        ...
        const extractionLoopMap = new Map();
        {
            ...
            for (const extraction of extractions) {
                ...
                const allCaptures = undeclaredIds.filter((name) => allScopeIds.has(name) && !importedNames.has(name));
                const uniqueCaptures = [...new Set(allCaptures)].sort((a, b) => (declPositions.get(a) ?? 0) - (declPositions.get(b) ?? 0));
                ...
                if (!enclosingLoops || enclosingLoops.length === 0) {
                    ...
                    const loopLocalSet = new Set(immediateLoop.iterVars);
        ...
        // Unify parameter slots for multiple event handlers on the same element
        {
            ...
            const handlersByParent = new Map();
            for (const ext of extractions) {
            ...
            for (const [, handlers] of handlersByParent) {
                ...
                for (const [, group] of elementGroups) {
                    ...
                    if (anyInLoop) {
                        allLoopLocals.sort((a, b) => (globalDeclPositions.get(a) ?? 0) -
                    ...
                    for (const h of group) {
                        const handlerCaptures = new Set();
                        for (let i = 2; i < h.paramNames.length; i++) {
                            handlerCaptures.add(h.paramNames[i]);
                        }
                        if (handlerCaptures.size === 0)
                            continue; // no captures, keep (_, _1) only
        ...
        const elementQpParamsMap = new Map();
        ...
        // We need to re-derive it. Let's scan all event handlers with paramNames.
        {
            ...
            const handlersByParent2 = new Map();
            for (const ext of extractions) {
                if (ext.ctxKind !== "eventHandler")
                    continue;
                if (ext.paramNames.length < 2 ||
            ...
            for (const [, handlers] of handlersByParent2) {
                ...
                for (const [, group] of elementGroups2) {
                    ...
                    if (anyInLoop2) {
                        allVars.sort((a, b) => (globalDeclPositions.get(a) ?? 0) -
                            (globalDeclPositions.get(b) ?? 0));
                    }
                    else {
        ...
        detectC02Diagnostics(extractions, closureNodes, enclosingExtMap, bodyPrograms, importedNames, program, repairedCode, relPath, diagnostics);
        ...
        for (const ext of extractions) {
            ...
            for (let i = 2; i < ext.paramNames.length; i++) {
                ...
                if (/^_\d*$/.test(p))
                    continue; // skip padding (_2, _3, etc.)
                parentUsage.add(p);
            }
        }
        ...
        const entryStrategy = options.entryStrategy ?? { type: "smart" };
        ...
        const preRenameSymbolName = new Map();
        const emitMode = options.mode ?? "prod";
        if (emitMode === "prod") {
            for (const ext of extractions) {
                if (ext.isInlinedQrl)
                    continue; // inlinedQrl has its own naming
        ...
        const hasLocalInlinedQrl = extractions.some((e) => e.isInlinedQrl && !relPath.includes("node_modules"));
        ...
        const parentResult = rewriteParentModule(repairedCode, relPath, extractions, originalImports, migrationDecisions, moduleLevelDecls, shouldTranspileJsx && (ext === ".tsx" || ext === ".jsx")
            ? { enableJsx: true, importedNames, enableSignals: true }
            : undefined, emitMode, devFile, isInlineStrategy
            ? {
                inline: true,
                entryType: entryStrategy.type,
            ...
            : options.stripCtxName ||
                ...
                : undefined, options.stripExports, options.isServer, options.explicitExtensions, options.transpileTs, options.minify, qrlOutputExt, program);
        ...
        detectC05Diagnostics(program, originalImports, repairedCode, relPath, diagnostics);
        if (ext === ".tsx" || ext === ".jsx") {
        ...
        const sameFileExportNames = new Set();
        const defaultExportedNames = new Set();
        const renamedExports = new Map();
        for (const node of program.body) {
                ...
                if (node.specifiers) {
                    for (const spec of node.specifiers) {
                        const exportedName = spec.exported?.type === "Identifier"
                            ...
                            : spec.exported?.value;
                        if (exportedName)
                            sameFileExportNames.add(exportedName);
        ...
        const importAttributesMap = new Map();
        for (const node of program.body) {
            if (node.type !== "ImportDeclaration")
                continue;
        ...
        const moduleImportsForContext = [];
        const addedModuleImports = new Set();
        ...
        const constLiteralsMap = new Map();
        if (!isInlineStrategy) {
            for (const ext of updatedExtractions) {
        ...
        for (const ext of updatedExtractions) {
            if (ext.isSync)
                continue; // sync$ is inlined, no separate segment module
            // Check if this segment is stripped
            const stripped = isStrippedSegment(ext.ctxName, ext.ctxKind, options.stripCtxName, options.stripEventHandlers);
            ...
            if (isInlineStrategy) {
                ...
                const entryField = resolveEntryField(entryStrategy.type, ext.symbolName, ext.ctxName, null, "manual" in entryStrategy
            ...
            const childQrlVarNames = new Map();
            const nestedQrlDecls = children.map((child) => {
                const childStripped = isStrippedSegment(child.ctxName, child.ctxKind, options.stripCtxName, options.stripEventHandlers);
                if (childStripped) {
                ...
                if (isDevMode && devFile) {
                    const devExt = options.explicitExtensions
                        ? (qrlOutputExt ?? ".js")
                        : undefined;
                    return buildQrlDevDeclaration(child.symbolName, child.canonicalFilename, devFile, child.loc[0], child.loc[1], child.displayName, devExt);
                }
            ...
            if (ext.parent === null && !ext.isInlinedQrl) {
                ...
                const migratedVarNames = new Set();
                for (const decision of migrationDecisions) {
            ...
            for (const child of children) {
                ...
                if (isJsxAttr) {
                    ...
                    if (child.isComponentEvent) {
                        ...
                        if (eventNameForPassive.startsWith("on") &&
                        ...
                        if (displayNamePath.includes("_q_ep_") ||
                    ...
                    const hasLoopCrossCaptures = child.captures &&
                    ...
                    const loopLocalParams = [];
                    if (child.paramNames.length >= 2 &&
                        child.paramNames[0] === "_" &&
                        child.paramNames[1] === "_1") {
            ...
            const segmentResult = stripped
                ...
                : generateSegmentCode(ext, nestedQrlDecls.length > 0 ? nestedQrlDecls : undefined, effectiveCaptureInfo, (() => {
                    const srcExt = sourceExtensions.get(ext.symbolName) ?? ext.extension;
                    return (shouldTranspileJsx &&
                    ...
                    : undefined, nestedCallSites.length > 0 ? nestedCallSites : undefined, importContext, enumValueMap.size > 0 ? enumValueMap : undefined);
            ...
            let parentComponentSymbol = null;
            ...
            const entryField = resolveEntryField(entryStrategy.type, ext.symbolName, ext.ctxName, parentComponentSymbol, "manual" in entryStrategy
                ? entryStrategy.manual
                : undefined);
            const segmentAnalysis = {
                origin: ext.origin,
 ...
 */
function detectC02Diagnostics(extractions, closureNodes, enclosingExtMap, bodyPrograms, importedNames, program, source, file, diagnostics) {
    for (const extraction of extractions) {
        const closureNode = closureNodes.get(extraction.symbolName);
        if (!closureNode)
            continue;
 ...
 */
function detectC05Diagnostics(program, originalImports, source, file, diagnostics) {
    ...
    for (const exportName of moduleExportNames) {
        ...
        for (const site of callSites) {
            const [startLine, startCol] = computeLineColFromOffset(source, site.start);
            const [endLine, endCol] = computeLineColFromOffset(source, site.end);
            diagnostics.push(emitC05(exportName, qrlName, file, {
                lo: site.start,
                hi: site.end,
                startLine,
                startCol,
 ...
 */
function findCallSites(program, funcName) {
    const sites = [];
    walk(program, {
        enter(node) {
 ...
 */
function computeLineColFromOffset(source, offset) {
    let line = 1;
    let col = 1;
...
function findIdentifierReferenceSite(closureNode, identName) {
    let site = null;
    walk(closureNode, {
        enter(node, parent) {
            if (site || node.type !== 'Identifier' || node.name !== identName)
                return;
            if (parent?.type === 'VariableDeclarator' && parent.id === node)
                return;
            ...
            if (parent?.type === 'MemberExpression' && parent.property === node && !parent.computed)
                return;
            if (parent?.type === 'StaticMemberExpression' && parent.property === node)
                return;
 ...
 */
function detectPassivePreventdefaultConflicts(program, file, source, diagnostics) {
    walk(program, {
        enter(node) {
 ...
 */
function collectBindingNamesFromNode(node, names) {
    if (!node)
        return;
    switch (node.type) {
        case "Identifier":
