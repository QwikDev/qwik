 ...
 */
export function compareAst(expected, actual, filename) {
 ...
 */
function normalizeImportOrder(program) {
 ...
 */
function normalizeImportAliases(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
    // Collect all aliased import specifiers
    const aliasMap = new Map(); // local -> imported
...
function collectDeclNames(node, names) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        for (const item of node)
 ...
 */
function isReorderableDeclaration(stmt) {
    if (stmt?.type !== 'VariableDeclaration' || stmt.kind !== 'const')
        return false;
    if (!stmt.declarations || stmt.declarations.length !== 1)
        return false;
    ...
    if (decl.init?.type === 'Literal' && typeof decl.init.value === 'string' && name.startsWith('_hf'))
        return true;
 ...
 */
function normalizeQrlDeclarationOrder(program) {
    sortReorderableBlock(program?.body);
    // Recurse into function/arrow bodies to sort .w() hoisting declarations
    walkBodies(program, (body) => sortReorderableBlock(body));
}
/**
 * Sort contiguous blocks of reorderable statements within a body array.
 */
function sortReorderableBlock(body) {
    if (!body || !Array.isArray(body))
        return;
    let i = 0;
    while (i < body.length) {
        if (!isReorderableDeclaration(body[i])) {
            i++;
            continue;
        }
        const blockStart = i;
        while (i < body.length && isReorderableDeclaration(body[i])) {
            i++;
        }
        if (i - blockStart <= 1)
            continue;
        const block = body.slice(blockStart, i);
        block.sort((a, b) => JSON.stringify(a).localeCompare(JSON.stringify(b)));
 ...
 */
function walkBodies(node, cb) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        node.forEach(n => walkBodies(n, cb));
        return;
    }
 ...
 */
function canonicalizeQrlVarNames(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
...
function stripPositions(node, ancestors = []) {
    if (Array.isArray(node))
 ...
 */
function sortSpecifiersWithinImports(program) {
    if (!program?.body)
        return;
    for (const stmt of program.body) {
        if (stmt.type === 'ImportDeclaration' && stmt.specifiers?.length > 1) {
            stmt.specifiers.sort((a, b) => {
 ...
 */
function sortIndependentExpressionStatements(program) {
    if (!program?.body)
        return;
    let i = 0;
    while (i < program.body.length) {
        if (!isIndependentExprStatement(program.body[i])) {
            i++;
            continue;
        }
        const start = i;
        while (i < program.body.length && isIndependentExprStatement(program.body[i]))
            i++;
        if (i - start <= 1)
            continue;
...
function isIndependentExprStatement(stmt) {
 ...
 */
function sortIndependentTopLevelStatements(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
    let i = 0;
    while (i < program.body.length) {
        if (!isIndependentTopLevel(program.body[i])) {
            i++;
            continue;
        }
        const start = i;
        while (i < program.body.length && isIndependentTopLevel(program.body[i]))
            i++;
        if (i - start <= 1)
            continue;
        const block = program.body.slice(start, i);
...
function isIndependentTopLevel(stmt) {
 ...
 */
function normalizeVoidZero(program) {
    walkAndReplace(program, (node) => {
 ...
 */
function normalizeBooleanLiterals(program) {
    walkAndReplace(program, (node) => {
 ...
 */
function stripDirectives(program) {
    if (!program?.body)
        return;
 ...
 */
function normalizeArrowBodies(node) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        for (const item of node)
 ...
 */
function renumberHoistedFunctions(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
 ...
 */
function normalizeEnumIIFE(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
    for (let i = 0; i < program.body.length; i++) {
        const stmt = program.body[i];
        ...
        if (stmt.type === 'VariableDeclaration' &&
            ...
            let callExpr = init;
            if (callExpr.type === 'ParenthesizedExpression')
            ...
            if (!callee.params || callee.params.length !== 1)
                continue;
 ...
 */
function normalizeJsxCalleeNames(node) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        for (const item of node)
...
function mergeJsxSplitProps(node) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        for (const item of node)
    ...
    if (!constProps)
        return;
    if (constProps.type === 'Literal' && constProps.value === null)
        return;
 ...
 */
function normalizeWrapProp(node) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
...
function unwrapWrapProp(node) {
    if (!node || node.type !== 'CallExpression')
        return node;
    const callee = node.callee;
    if (!callee || callee.type !== 'Identifier' || callee.name !== '_wrapProp')
        return node;
    const args = node.arguments;
    if (!args)
        return node;
 ...
 */
function inlineFnSignalSimple(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
    // Collect all _hf function declarations: name -> { params, body }
    const hfDecls = new Map();
    for (const stmt of program.body) {
        if (stmt?.type !== 'VariableDeclaration')
            continue;
        for (const decl of stmt.declarations || []) {
            ...
            if (fn.type !== 'ArrowFunctionExpression')
                continue;
            const paramNames = (fn.params || []).map((p) => p.name).filter(Boolean);
    ...
    function substituteParams(node, paramMap) {
        if (!node || typeof node !== 'object')
            return node;
        if (Array.isArray(node))
            return node.map(n => substituteParams(n, paramMap));
    ...
    function processNode(node) {
        ...
        if (node.type !== 'CallExpression')
            return node;
        if (node.callee?.type !== 'Identifier' || node.callee.name !== '_fnSignal')
            return node;
        if (!node.arguments || node.arguments.length < 2)
            return node;
        const hfRef = node.arguments[0];
        const argsArray = node.arguments[1];
        if (hfRef?.type !== 'Identifier' || !hfDecls.has(hfRef.name))
            return node;
 ...
 */
function normalizeJsxFlags(node) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        for (const item of node)
 ...
 */
function stripQpProperties(node) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        for (const item of node)
 ...
 */
function canonicalizeCaptureBindings(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
    ...
    function processBody(body) {
        ...
        for (const stmt of body) {
            if (stmt?.type !== 'VariableDeclaration')
                continue;
            for (const decl of stmt.declarations || []) {
                if (!decl.init || !isCapturesAccess(decl.init))
                    continue;
                if (decl.id?.type === 'Identifier') {
    ...
    function isCapturesAccess(node) {
        // Match _captures[N]
...
function mergeGetVarConstProps(node) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        for (const item of node)
    ...
    for (let i = 0; i < props.length; i++) {
        ...
        if (aCall?.type !== 'CallExpression' || aCall.callee?.type !== 'Identifier')
            continue;
        ...
        for (let j = 0; j < props.length; j++) {
            if (j === i || consumed.has(j))
                continue;
            const b = props[j];
            if (b.type !== 'SpreadElement')
                continue;
            const bCall = b.argument;
            if (bCall?.type !== 'CallExpression' || bCall.callee?.type !== 'Identifier')
                continue;
    ...
    if (consumed.size > 0) {
        node.properties = props.filter((_, i) => !consumed.has(i));
    }
}
/**
 ...
 */
function normalizeDevModePositions(node) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        for (const item of node)
    ...
    if (node.type === 'ObjectExpression' && Array.isArray(node.properties)) {
        const hasLo = node.properties.some((p) => p.key?.name === 'lo' || p.key?.value === 'lo');
        const hasHi = node.properties.some((p) => p.key?.name === 'hi' || p.key?.value === 'hi');
        const hasFile = node.properties.some((p) => p.key?.name === 'file' || p.key?.value === 'file');
        ...
        const hasLine = node.properties.some((p) => (p.key?.name || p.key?.value) === 'lineNumber');
        const hasCol = node.properties.some((p) => (p.key?.name || p.key?.value) === 'columnNumber');
        const hasFileName = node.properties.some((p) => (p.key?.name || p.key?.value) === 'fileName');
 ...
 */
function normalizeDevQrlCalls(node) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        for (const item of node)
 ...
 */
function stripJsxSourceInfo(node) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        for (const item of node)
 ...
 */
function stripUseHmrCalls(program) {
    function processBody(body) {
        if (!Array.isArray(body))
            return;
        for (let i = body.length - 1; i >= 0; i--) {
            ...
            if (expr?.type !== 'CallExpression')
                continue;
            if (expr.callee?.type !== 'Identifier' || expr.callee.name !== '_useHmr')
                continue;
            body.splice(i, 1);
        }
    }
 ...
 */
function mergeDuplicateObjectProperties(node) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        for (const item of node)
    ...
    if (node.type === 'ObjectExpression' && Array.isArray(node.properties)) {
        ...
        if (toRemove.size > 0) {
            node.properties = node.properties.filter((_, i) => !toRemove.has(i));
        }
    }
}
/**
 ...
 */
function sortObjectProperties(node) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        for (const item of node)
 ...
 */
function stripUnusedCallBindings(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
    // Collect all identifier names referenced in the program
    function collectRefs(node, refs, skipDecl) {
        if (!node || typeof node !== 'object')
            return;
        if (Array.isArray(node)) {
            node.forEach(n => collectRefs(n, refs, skipDecl));
            return;
        }
 ...
 */
function canonicalizeFnSignalArgs(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
    ...
    for (const stmt of program.body) {
        ...
        for (const decl of stmt.declarations || []) {
            ...
            const name = decl.id.name;
            if (/^_hf\d+$/.test(name) && decl.init) {
                const params = (decl.init.params || []).map((p) => p.name).filter(Boolean);
    ...
    function processFnSignalCalls(node) {
        if (!node || typeof node !== 'object')
            return;
        if (Array.isArray(node)) {
            for (const item of node)
        ...
        const hfRef = node.arguments[0];
        const argsArray = node.arguments[1];
        if (hfRef?.type !== 'Identifier' || !/^_hf\d+$/.test(hfRef.name))
            return;
        if (argsArray?.type !== 'ArrayExpression' || !Array.isArray(argsArray.elements))
            return;
        const hfName = hfRef.name;
        const hfInfo = hfDecls.get(hfName);
        if (!hfInfo || hfInfo.paramNames.length !== argsArray.elements.length)
            return;
        ...
        const paramMap = new Map(); // old param name -> new param name
...
function deduplicateImports(program) {
    if (!program?.body)
        return;
    ...
    for (let i = 0; i < program.body.length; i++) {
        ...
        const hasDefault = stmt.specifiers?.some((s) => s.type === 'ImportDefaultSpecifier');
 ...
 */
function inlineSegmentBodyIntoSCall(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
    ...
    for (let i = 0; i < program.body.length; i++) {
        ...
        if (expr.callee.object?.type !== 'Identifier')
            continue;
        if (expr.callee.property?.type !== 'Identifier' || expr.callee.property.name !== 's')
            continue;
        if (!expr.callee.object.name?.startsWith('q_'))
            continue;
    ...
    if (toRemoveIndices.size > 0) {
        program.body = program.body.filter((_, i) => !toRemoveIndices.has(i));
    }
}
/**
 ...
 */
function normalizeAutoExports(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
    ...
    for (const stmt of program.body) {
        if (stmt?.type !== 'ImportDeclaration' || !stmt.specifiers)
            continue;
        for (const spec of stmt.specifiers) {
 ...
 */
function stripUnusedImports(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
 ...
 */
function stripIsServerGuards(program) {
 ...
 */
function stripPureExpressionStatements(program) {
...
function stripUnusedLocalDeclarations(program) {
 ...
 */
function stripNoopLabeledStatements(node) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
 ...
 */
function stripOrphanedSideEffectCalls(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
        ...
        for (let i = program.body.length - 1; i >= 0; i--) {
            ...
            const allImportOnly = [...usedIdents].every(name => {
                ...
                for (let j = 0; j < program.body.length; j++) {
                    ...
                    if (other?.type === 'ImportDeclaration')
                        continue; // skip import stmts
                    const refs = new Set();
                    collectAllIdents(other, refs);
                    if (refs.has(name))
                        return false; // referenced elsewhere
                }
                return true;
            });
...
function stripUnusedModuleLevelDeclarations(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
    let changed = true;
    while (changed) {
        ...
        for (let i = program.body.length - 1; i >= 0; i--) {
            ...
            if (stmt?.type === 'VariableDeclaration') {
                const allUnused = (stmt.declarations || []).every((decl) => {
                    const names = new Map();
...
function stripUnusedDeclsInBlock(block, params) {
    ...
    while (changed) {
        ...
        for (let i = block.body.length - 1; i >= 0; i--) {
            ...
            if (stmt?.type === 'VariableDeclaration') {
                ...
                const hasContent = tryBody.some((s) => s?.type !== 'EmptyStatement');
                if (!hasContent) {
                    block.body.splice(i, 1);
                    changed = true;
                }
...
function collectDeclaredNames(pattern, stmtIndex, map) {
    if (!pattern)
        return;
...
function collectAllIdents(node, set) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        node.forEach(n => collectAllIdents(n, set));
        return;
    }
    if (node.type === 'Identifier' && node.name) {
        set.add(node.name);
...
function stripDotSBodies(program) {
    ...
    for (const stmt of program.body) {
        ...
        if (!callee || callee.type !== 'MemberExpression')
            continue;
        if (callee.property?.type !== 'Identifier' || callee.property.name !== 's')
            continue;
 ...
 */
function stripMigratedDeclarations(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
...
function isExported(stmt, program) {
    // Check if the statement is wrapped in an ExportNamedDeclaration
    for (const s of program.body) {
        if (s?.type === 'ExportNamedDeclaration' && s.declaration === stmt)
            return true;
    }
    return false;
}
function collectPatternNames(pattern, nameSet, nameArr) {
    if (!pattern)
        return;
    if (pattern.type === 'Identifier') {
        nameSet.add(pattern.name);
        if (nameArr)
            nameArr.push(pattern.name);
    }
    else if (pattern.type === 'ArrayPattern') {
 ...
 */
function inlineDestructuredBindings(program) {
    function processFunctionBody(body, params) {
        if (!Array.isArray(body))
            return;
        ...
        for (let i = body.length - 1; i >= 0; i--) {
            ...
            const paramNames = new Set(params.map((p) => p.name).filter(Boolean));
    ...
    function replaceIdentifiers(node, bindings) {
        if (!node || typeof node !== 'object')
            return node;
        if (Array.isArray(node))
            return node.map(n => replaceIdentifiers(n, bindings));
 ...
 */
function destructureRawPropsParam(program) {
    if (!program?.body || !Array.isArray(program.body))
        return;
    for (const stmt of program.body) {
        // Find: export const NAME = (params...) => body
        const fn = getExportedSegmentFunction(stmt);
        if (!fn || !fn.params || fn.params.length < 3)
            continue;
        ...
        for (let pi = 2; pi < fn.params.length; pi++) {
            ...
            for (const ref of refs) {
                ...
                replaceNodeInParent(ref._grandparent, ref._parentKey, ref._parentIndex, { type: 'Identifier', name: fieldName });
            }
        }
    }
}
function getExportedSegmentFunction(stmt) {
    if (stmt?.type !== 'ExportNamedDeclaration')
        return null;
...
function collectIdentRefs(node, name, refs, parent, parentKey, parentIndex, grandparent) {
    if (!node || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        for (let i = 0; i < node.length; i++) {
...
function replaceNodeInParent(container, key, index, replacement) {
    if (!container || !key)
        return;
 ...
 */
function expandRawPropsCaptures(program) {
    function processFunctionBody(fn) {
        const body = fn.body;
        if (!body)
            return;
     ...
     */
    function expandMemberAccessesInBody(body, varName) {
        // Collect all references
        const refs = [];
        let hasNonMemberRef = false;
        function scan(node, parent, key, index) {
            if (!node || typeof node !== 'object')
                return;
            if (Array.isArray(node)) {
                for (let i = 0; i < node.length; i++)
                    scan(node[i], node, String(i), i);
                return;
            }
 ...
 */
function stripCapturesDeclarations(program) {
    function processBody(body) {
        if (!Array.isArray(body))
            return;
        for (let i = body.length - 1; i >= 0; i--) {
            ...
            if (!decls || decls.length === 0)
                continue;
            const allCaptures = decls.every((d) => {
                if (!d.init)
                    return false;
                // Match _captures[N]
...
function walkAndReplace(node, replacer) {
    if (node === null || typeof node !== 'object')
        return;
    if (Array.isArray(node)) {
        for (let i = 0; i < node.length; i++) {
