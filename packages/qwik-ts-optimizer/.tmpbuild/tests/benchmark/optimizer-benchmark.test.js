...
const swcBinding = require('/Users/jackshelton/dev/open-source/qwik/packages/optimizer/bindings/qwik.darwin-arm64.node');
// ---------------------------------------------------------------------------
...
function discoverFiles() {
    ...
    const output = execSync(`find ${QWIK_PACKAGES_DIR} -type f \\( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" \\) ! -path "*/node_modules/*" ! -path "*/dist/*" ! -path "*/.turbo/*"`, { encoding: 'utf-8', maxBuffer: 50 * 1024 * 1024 });
...
function buildSwcOptions(input) {
    return {
        input,
...
// ---------------------------------------------------------------------------
async function measureMinAsync(fn, runs) {
    let min = Infinity;
...
function measureMinSync(fn, runs) {
    let min = Infinity;
    for (let i = 0; i < runs; i++) {
        const start = performance.now();
        fn();
...
describeFn('BENCH-01: full monorepo benchmark', () => {
    it('TS optimizer is within 1.15x of SWC wall time for full monorepo', async () => {
        ...
        const swcTime = await measureMinAsync(() => swcBinding.transform_modules(swcOptions), MEASURED_RUNS);
        const tsTime = measureMinSync(() => transformModule({
            input: tsInput,
            srcDir: QWIK_PACKAGES_DIR,
            rootDir: QWIK_PACKAGES_DIR,
            entryStrategy: { type: 'segment' },
...
describeFn('BENCH-02: worst-case single file benchmark', () => {
    it('TS optimizer is within 1.5x of SWC for worst-case single file', async () => {
        ...
        const tsInput = [{ path: filePath, code }];
...
//# sourceMappingURL=optimizer-benchmark.test.js.map