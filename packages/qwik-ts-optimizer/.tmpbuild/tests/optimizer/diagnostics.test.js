...
import { emitC02, emitC03, emitC05, emitPreventdefaultPassiveCheck, parseDisableDirectives, filterSuppressedDiagnostics, } from '../../src/optimizer/diagnostics.js';
...
describe('emitC02', () => {
    it('produces error with code C02 for a function reference', () => {
        ...
        expect(diag.message).toBe("This code depends on the local function 'hola', but only plain serializable data can be used here. Move 'hola' inside the callback, or pass the data it needs instead.");
    ...
    it('produces error with code C02 for a class reference', () => {
        ...
        expect(diag.category).toBe('error');
        expect(diag.code).toBe('C02');
        expect(diag.message).toBe("This code depends on the local class 'Thing', but only plain serializable data can be used here. Move 'Thing' inside the callback, or pass the data it needs instead.");
        expect(diag.highlights).toBeNull();
        expect(diag.scope).toBe('optimizer');
    });
...
describe('emitC03', () => {
    it('produces error with code C03 listing captured identifiers', () => {
        ...
        expect(diag.message).toBe("This value depends on local variables (qrl), so it needs to be written as a function here.");
    ...
    it('comma-separates multiple captured identifiers', () => {
        ...
        expect(diag.message).toBe("This value depends on local variables (a, b, c), so it needs to be written as a function here.");
    });
...
describe('filterSuppressedDiagnostics', () => {
    ...
    it('handles diagnostics with null highlights (uses no line matching)', () => {
        ...
        const diags = [emitC02('hola', 'test.tsx', false)];
        const directives = new Map();
...
describe('emitC05', () => {
    it('produces error with code C05 for missing Qrl export', () => {
        ...
        expect(diag.message).toBe("Found 'useMemo$', but this file is missing the matching export 'useMemoQrl'. Export 'useMemoQrl' from the same file, or stop calling 'useMemo$' directly.");
        expect(diag.highlights).toEqual([
...
describe('emitPreventdefaultPassiveCheck', () => {
    it('produces warning for passive + preventdefault conflict', () => {
        ...
        expect(diag.message).toBe('This listener uses both passive:click and preventdefault:click. Passive listeners cannot call preventDefault(), so preventdefault:click will be ignored.');
        expect(diag.scope).toBe('optimizer');
    });
});
// ---------------------------------------------------------------------------
...
`;
        ...
        expect(messages).toContainEqual("This code depends on the local function 'hola', but only plain serializable data can be used here. Move 'hola' inside the callback, or pass the data it needs instead.");
        expect(messages).toContainEqual("This code depends on the local class 'Thing', but only plain serializable data can be used here. Move 'Thing' inside the callback, or pass the data it needs instead.");
    });
...
\t\t`;
        ...
        const passiveDiags = result.diagnostics.filter((d) => d.code === 'preventdefault-passive-check');
