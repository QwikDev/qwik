...
describe('resolveEntryField', () => {
    ...
    describe('component strategy', () => {
        it('returns parent component symbol for non-component segments', () => {
            expect(resolveEntryField('component', 'App_component_useTask_xyz', 'useTask', 'App_component_abc123', undefined)).toBe('App_component_abc123');
        });
        it('returns null for component segments themselves', () => {
            expect(resolveEntryField('component', 'App_component_abc123', 'component', null, undefined)).toBeNull();
        });
        it('returns null when no parent component exists', () => {
            expect(resolveEntryField('component', 'someHandler_xyz', 'eventHandler', null, undefined)).toBeNull();
        });
    });
    ...
    describe('manual strategy', () => {
        it('returns mapped value when symbol is in manual map', () => {
            ...
            expect(resolveEntryField('smart', 'App_component_abc123', 'component', null, manual)).toBe('vendor');
        });
        it('returns null when symbol is not in manual map', () => {
            const manual = { 'Other_component_xyz': 'vendor' };
            expect(resolveEntryField('smart', 'App_component_abc123', 'component', null, manual)).toBeNull();
        });
    });
...
//# sourceMappingURL=entry-strategy.test.js.map