...
describe('segment-only convergence debug', () => {
    it('analyzes all 210 snapshot tests for segment-only failures', () => {
        ...
        for (const snapFile of allFiles) {
            ...
            const filename = parsed.segments[0]?.metadata?.origin ||
            ...
            for (const expectedSeg of parsed.segments) {
                if (!expectedSeg.metadata)
                    continue;
                const actualSeg = result.modules.find((m) => m.segment && m.segment.name === expectedSeg.metadata.name);
                if (!actualSeg) {
        ...
        console.log(`Parent passes but >= 1 segment fails: ${segmentOnlyFailures.length}\n`);
        ...
        for (let i = 0; i < Math.min(20, segmentOnlyFailures.length); i++) {
            ...
            for (const failure of tr.segmentFailures) {
                console.log(`     - ${failure.segmentName}: ${failure.category}${failure.metadataFields ? ` (${failure.metadataFields.join(', ')})` : ''}`);
            }
        }
        ...
        if (topCategory === 'metadata_mismatch') {
            const topField = Object.entries(metadataFieldCounts).sort((a, b) => b[1] - a[1])[0];
            console.log(`Fix the "${topField[0]}" metadata field (appears in ${topField[1]} failures)`);
            console.log(`This single fix would flip at most ${topField[1]} segment-only tests.`);
        }
        else if (topCategory) {
            console.log(`Fix "${topCategory}" (${topCount} failures)`);
            console.log(`This single fix would flip at most ${topCount} segment-only tests.`);
        }
        // Always pass the test (it's informational)
        expect(true).toBe(true);
    });
});
//# sourceMappingURL=segment-only-debug.test.js.map