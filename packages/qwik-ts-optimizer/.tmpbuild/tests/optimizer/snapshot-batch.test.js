        ...
        for (const snapName of fullMatchSnapshots) {
            ...
            it(`matches ${snapName}`, () => {
                ...
                if (parsed.parentModules.length > 0) {
                    ...
                    expect(astResult.match, `Parent module AST mismatch for ${snapName}.\n` +
                        `Expected parse error: ${astResult.expectedParseError}\n` +
                        `Actual parse error: ${astResult.actualParseError}`).toBe(true);
                }
                ...
                for (const expectedSeg of parsed.segments) {
                    ...
                    if (actualSeg) {
                        ...
                        expect(astResult.match, `Segment ${expectedSeg.metadata.name} AST mismatch for ${snapName}.\n` +
                            `Expected parse error: ${astResult.expectedParseError}\n` +
                            `Actual parse error: ${astResult.actualParseError}`).toBe(true);
                    }
...
//# sourceMappingURL=snapshot-batch.test.js.map