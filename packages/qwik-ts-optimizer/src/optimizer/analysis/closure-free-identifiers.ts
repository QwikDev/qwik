...
/**
 * For each closure in `closureNodes`, the identifiers referenced inside it that
 * don't resolve to a declaration in its own subtree — ordered by first
 * reference, deduplicated, one entry per tracked node (possibly empty). Keyed by
 * node identity so the map survives the prod symbol rename.
 */
