...
/**
 * Whether `ext.calleeName` matches a registered context name
 * (e.g., `'server'` matches `server$`).
 */
...
/**
 * Whether the extraction's context is a JSX event handler (`onClick$`) or a
 * JSX-prop binding — both share the same QRL-as-attribute output shape.
 */
...
/**
 * Whether the parameter list begins with `(_, _1, ...)` — the convention for
 * loop callbacks where the first two slots are positional placeholders for the
 * loop's index and item; real captures begin at index 2.
 */
...
/**
 * Whether `ctxName` is a pre-extraction component segment — the source-level
 * `component$(...)` marker or its `componentQrl(...)` sibling. For the
 * post-extraction `'component'` form, see {@link isAnyComponentCtx}.
 */
 ...
 * `component$`/`componentQrl` markers or the post-extraction `'component'`
 * form. Three-arm version of {@link isComponentCtx}.
 */
...
/**
 * Whether a segment should be stripped to a `null` body: its ctxName starts
 * with a `stripCtxName` prefix, or `stripEventHandlers` is set and ctxKind is
 * `"eventHandler"`. Stripped segments emit `export const {symbolName} = null;`.
 */
 ...
 * **never** stripped, regardless of `stripCtxName`/`stripEventHandlers`:
 * stripping one whose ctxName happens to match a strip prefix (e.g. a router
 * lib's `serverQrl` dispatcher vs `stripCtxName: ['server']`) would collapse
 * its QRL to a chunkless `_noopQrl` and break `server$` RPC at runtime.
 */
