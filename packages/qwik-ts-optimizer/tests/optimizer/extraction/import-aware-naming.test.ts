/**
 * Regression tests — a single-Identifier-import marker arg
 * uses the import path for segment naming.
 *
 ...
 * import binding. TS pre-fix always used the surrounding context
 ...
 *
 * Bonus: this fix also closes the **F-deferred** item from the F5
...
`;
  ...
  it('relative path resolves: `./style.css` and `style.css` hash identically', () => {
    ...
    // form matches what SWC hashes — verified via the seed match against
    // SWC. Sanity check: identical hashes for the same module
