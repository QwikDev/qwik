/**
 * Regression tests — JSX dev-info `fileName:` honors the
 * user-supplied `input.devPath` override (part of the F5 strip-marker work).
 *
 * Pre-fix: JSX dev-info `fileName:` was hardcoded to use `relPath`
 ...
 *
 * The fix threads a new `userDevPath` field through `RewriteContext` +
