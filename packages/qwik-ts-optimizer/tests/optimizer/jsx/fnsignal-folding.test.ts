/**
 * Regression tests for `_fnSignal` lambda body constant folding.
 *
 ...
 * blocking the flip — sibling fixes cover bugs 4-6).
 */
...
`;
  ...
  it('does not regress _str format when the simplifiable test arm is a literal', () => {
    ...
    // pre-fix behavior; both lambda and `_str` carry `3` unchanged.
    const input = `
import { component$ } from '@qwik.dev/core';
