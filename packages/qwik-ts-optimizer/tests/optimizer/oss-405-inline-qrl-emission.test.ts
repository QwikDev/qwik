/**
 * Regression tests for OSS-405 — peer-tool `inlinedQrl(...)` re-emission
 ...
 *      anchor existed. Fix: when the OSS-404 anchor lookup fails, fall back
...
describe('OSS-405 Fix A: skip _captures injection on inlinedQrl bodies under inline', () => {
...
`;
  ...
  it('STILL injects _captures unpacking on regular $() captures under inline strategy', () => {
    ...
    // promotion pipeline (OSS-406) must continue to get `_captures[N]` /
    // `_rawProps.X` injection under inline strategy — the gate added in
    // OSS-405 must not regress that path.
    //
    // Shape mirrors OSS-406's `example_optimization_issue_3542`-style
...
`;
    ...
    // (proves the OSS-405 gate didn't accidentally short-circuit
    // the non-inlinedQrl injection path).
    expect(code).toContain('_captures[0]');
  });
});

describe('OSS-405 Fix B: jsx(...) → _jsxSorted(...) rewrite under inline strategy', () => {
  it('rewrites peer-tool jsx() calls inside .s(body) to _jsxSorted(...) form', () => {
    ...
    // never sees it. OSS-405 wires the same transform into the inline
    // body path.
    const input = `
...
describe('OSS-405 Fix C: sCall placement when no Qrl-form export anchor exists', () => {
  it('inserts sCalls after the last module decl any sCall body references', () => {
    // The OSS-404 F1c logic anchors sCalls relative to the LAST `Qrl(`-form
...
describe('OSS-405 Fix D: _auto_ reexport for module decl used inside inlinedQrl body under inline', () => {
