/**
 * Regression tests for OSS-407 — Hoist strategy + regCtxName emission.
 *
 * Pickup audit found 3 distinct coupled bugs vs the ticket's documented
 * 4 (bug E `_auto_STYLES` reexport and bug B's `.w(...)` call-site
 * wrap were auto-fixed by PR #138's migration filter for inline/hoist).
 *
 ...
 *      the referenced decl; grouped sCalls retain the OSS-404/OSS-405
...
describe('OSS-407 Fix A: direct emit for bare-identifier hoist body matching module-level decl', () => {
...
describe('OSS-407 Fix B: filter migrated names from body-side capture unpacking', () => {
...
`;
    ...
    // (B.2) Migration emits the `_auto_` reexport (PR #138 enabled this for
    // hoist; we keep the assertion as a sanity check that A+B+C cohabit).
...
`;
    ...
    // `_rawProps` capture (OSS-406 raw-props consolidation under inline).
    expect(code).toContain('_captures[0]');
  });
});

describe('OSS-407 Fix C: per-sCall placement when sCall references a forward-declared decl', () => {
  it('places `q_X.s(STYLES)` AFTER `const STYLES = ...` to avoid TDZ', () => {
    ...
    // which is declared AFTER the marker export. Default OSS-404 placement
...
`;
  ...
  it('keeps non-forward-dep sCalls in their OSS-404 anchor-relative position', () => {
...
`;
    ...
    // sits before the export per the OSS-404 anchor-relative default.
