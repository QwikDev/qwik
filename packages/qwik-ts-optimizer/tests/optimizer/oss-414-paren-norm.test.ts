/**
 * Regression tests for OSS-414 — `_hf<n>_str` paren normalization for
 ...
 * safety. For the BARE form `<div some={some}/>`, OSS-412's
 ...
 *
 * Companion to convergence's `example_props_optimization` (sibling of
 * OSS-411/412/413 — paren-norm closes bug 6 of 6 surfaced in the OSS-409
 * audit, but a separate sibling OSS-415 covers segment-body default
 * folding that is also needed for the target to flip).
 *
...
describe('OSS-414: _hf<n>_str paren normalization for object-property values', () => {
...
`;
  ...
  it('keeps bare-form _hf<n>_str unchanged (was already correct via OSS-412)', () => {
    // Bare-form benefits from OSS-412's ParenthesizedExpression unwrap; this
    // pin ensures OSS-414's paren-strip didn't inadvertently change it.
    const input = `
import { component$ } from '@qwik.dev/core';
export const C = component$(({some = 1+2}) => {
