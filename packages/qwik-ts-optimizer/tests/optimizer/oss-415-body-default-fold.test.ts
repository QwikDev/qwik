/**
 * Regression tests for OSS-415 — segment-body constant folding for
 ...
 * the runtime ships to clients. OSS-411 added folding to the lambda body
 * (`_hf<n>` arrow), but NOT to the segment-body source.
 *
 ...
 * blocker for the test to flip from failing). Sibling of
 * OSS-409/411/412/413/414.
 *
...
describe('OSS-415: segment-body default folding (post-JSX-transform pass)', () => {
...
`;
    ...
    // But the lambda body (`_hf<n> = ...`) is folded — pre-existing OSS-411 behavior.
    expect(code).toMatch(/_hf0\s*=\s*\(p0\)\s*=>\s*p0\.some\s*\?\?\s*3/);
    // And the console.log body emit is folded — the OSS-415 behavior.
    // (`applyIdentifierReplacements` defensively wraps the access in parens
    // — `console.log((_rawProps.some ?? 3))` — and OSS-414's paren-strip
