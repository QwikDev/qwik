/**
 * Regression tests for OSS-427 — eliminate spurious marker re-imports in
 * extracted segments (Sub-C of the OSS-424 F5 umbrella).
 *
 * Pre-OSS-427: when an extraction's body was the argument of a marker call
 * (`$(() => { ... })`, `client$(() => { ... })`, `useTask$(() => { ... })`),
 ...
 *
 * Root cause: the OSS-368 Identifier-tagging in `extract.ts:495` walked
 ...
 * extractions). Sub-C does not flip a test on its own (those fixtures
...
describe('OSS-427: marker callee no longer leaks into extracted segment imports', () => {
  it('inner $() segment does not re-import the bare $ marker', () => {
    // The inner `$()` extraction body is `() => { /* a */ }`. Pre-OSS-427:
...
`;
  ...
  it('extracted useTask$ segment does not re-import useTask$', () => {
    ...
    // re-importing useTask$ into its segment file pre-OSS-427.
    const input = `
