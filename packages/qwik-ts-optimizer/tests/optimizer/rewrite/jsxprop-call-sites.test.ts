/**
 * Regression tests for jSXProp ctxKind call-site rewriting
 * (`example_immutable_analysis` parity).
 *
 * Since the ctxKind harmonisation, Component-prop `$`-suffix attrs
 ...
 *
 * Doesn't flip `example_immutable_analysis` on its own — destructure
 * inlining, TS-annotation strip, and subsequently-surfaced bugs are
 * separate fixes.
 */
...
`);
      ...
      // Pin the classification — the call-site predicate change must not
