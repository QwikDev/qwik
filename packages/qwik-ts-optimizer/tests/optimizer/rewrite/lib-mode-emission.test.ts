/**
 * Regression tests for the lib-mode emission shape (part of the
 * `example_lib_mode` parity fix).
 *
 ...
 *
 * What this file does NOT cover: preserving original `$`-suffix imports
 * (a separate slice, covered by lib-mode-imports.test.ts) or aggressive
 * const-text inlining (a no-op in practice — the existing inline
 * pipeline + simplifier already handle function-local const-text
 * bindings). The target convergence test `example_lib_mode` needs the
 * import-surface slice too.
 */
