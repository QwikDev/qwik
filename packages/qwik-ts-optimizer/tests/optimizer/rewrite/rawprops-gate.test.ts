/**
 * Regression tests for two `_rawProps` bugs — tighten the consolidation
...
`;
    ...
    // Defaulted access carries the `1+2` default. Three valid shapes:
    // inline `_rawProps.some ?? 1+2`; hoisted into a `_hf<n>_str`
    // such as `"p0.some??1+2"`; with body folding, the inline-body emit is
...
`;
    ...
    // SWC's NullishCoalescing emission for defaulted fields. The
    // inline-body fold collapses `?? 1+2` to `?? 3`; the bare-literal
