 ...
 *
 * Five root causes for `example_parsed_inlined_qrls` (Inline + Prod,
 * idempotency case), on top of the pre-existing basic `q_X.s(body)`
 * re-emission:
 *
 ...
 *      keep auto-keys (cf. the qwik_react_inline fixture).
...
`;
  ...
  it('keeps auto-keys on nested jsx() calls under COMPONENT-tag parents', () => {
    ...
    // down — matches the `qwik_react_inline` shape with Host /
