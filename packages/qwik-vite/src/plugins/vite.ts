...
/**
 * Qwik libraries ship pre-built `.qwik.mjs` code, so the server build can leave them external: core
 * and the router share their runtime state with the copies such a library loads. Two things still
 * keep a library in the server bundle: it imports a v1 package name, which only the bundler's
 * aliases resolve, or the user lists in `ssr.noExternal`. The dev server transforms every Qwik
 * library instead: its SSR needs the segment URLs of the library's QRLs, and its core is the
 * development build, which cannot share state with the production build Node would load. On the
 * client every Qwik library is excluded from dep optimization, so the optimizer can split its QRLs
 * into segments.
 */
async function checkExternals() {
  ...
  /** Qwik libraries that use v1 package names, so they must stay in the server bundle. */
  const bundledDeps: string[] = [];
  let rootDir: string;
  const coreNames = ['@qwik.dev/core', '@builder.io/qwik'];
  // v1 package names only resolve through the bundler's aliases
  const bundledNames = ['@builder.io/qwik', '@builder.io/qwik-city'];
  const bundledImport = /["']@builder\.io\/qwik(?:-city)?(?:\/[^"']*)?["']/;
  ...
  /**
   * Whether the installed package imports a v1 package name. A runtime dependency declares it; a
   * devDependency only counts when the published files import it. A package too large to scan is
   * bundled, which is always safe.
   */
      ...
      async handler(config, env) {
        ...
        for (const dep of candidates) {
          if (await isQwikDep(dep, root)) {
            qwikDeps.push(dep);
            if (isDevServer || (await needsServerBundle(dep, root))) {
      ...
      async handler(source, importer, options) {
        ...
        if (result?.external) {
          ...
          if ((await isQwikDep(packageName, dir)) && (await needsServerBundle(packageName, dir))) {
            ...
            throw new Error(
              `\n==============\n` +
                `${packageName} is being treated as an external dependency, but it imports a v1 Qwik package name, which only resolves inside the server bundle.\n` +
