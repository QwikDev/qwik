...
export interface CondensedGraph {
  /** Bundle name -> index of its strongly-connected component. */
  componentOf: Map<string, number>;
  /** Members of each component, indexed by component. */
  components: string[][];
  /** Condensation adjacency: component -> its successor components (no self-loops). */
  successors: Set<number>[];
}

/**
 * Tarjan strongly-connected-component condensation of a bundle's static-import graph. The resulting
 * component graph is a DAG, which turns cyclic-graph problems (dependency totals, transitive
 * reduction) into well-posed acyclic ones.
 */
