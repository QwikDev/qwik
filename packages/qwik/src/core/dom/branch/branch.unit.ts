...
type BranchRenderFn = (ctx: ContainerContext) => ValueOrPromise<readonly Node[]>;
...
describe('branches', () => {
      ...
      registerSubscriberToOwner(
        new BranchSubscription(
          new Branch(
            range,
            () => visible.value,
            createQRL<BranchRenderFn>('chunk', 'renderThen', null, () => {
              thenResolved = true;
              return Promise.resolve({
                renderThen: () => [],
              });
            }),
            createQRL<BranchRenderFn>('chunk', 'renderElse', null, () => {
              elseResolved = true;
              return Promise.resolve({
                renderElse: () => [elseNode],
              });
  ...
  it('keeps the current branch while async conditions run sequentially', async () => {
    const scheduler = new Scheduler(noopSchedule);
    ...
    const branch = createOwned(() =>
      createBranch(
        { scheduler } as ContainerContext,
        range,
        () => (version.value === 0 ? true : version.value === 1 ? stale.promise : current.promise),
        () => [oldNode],
        () => [currentNode]
      )
    );
    ...
    branch.run();
    expect(replacements).toEqual([[oldNode]]);

    version.value = 1;
    const staleRun = scheduler.flushInteraction();
    ...
    version.value = 2;
    const currentRun = scheduler.flushInteraction();
    await Promise.resolve();
    expect(replacements).toEqual([[oldNode]]);
    current.resolve(false);
    await currentRun;
    expect(replacements).toEqual([[oldNode], [currentNode]]);
  });
