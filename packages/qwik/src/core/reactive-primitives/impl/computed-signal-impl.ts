 ...
 * the compute function turns out to be async (the optimizer converts async fns to generators, which
 * resolve to a promise when driven), the signal lazily switches on the async engine — jobs,
 * `loading`, `error`, polling — giving it the same API as an AsyncSignal. Auto-tracking keeps
 * working across await points because the invoke context is restored while driving the generator.
 */
  ...
  implements BackRef
{
   ...
   * only track via its explicit track(). Computeds auto-track every read instead, using a dedicated
   * context that generator driving restores across await points.
   */
