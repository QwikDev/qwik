import type { OutOfOrderRevealBoundary } from '../control-flow/pending-utils';
import { QPendingResolved } from '../shared/utils/markers';
import type { SSRContainer, SSROutOfOrderSegment } from './ssr-types';

/** Finalizes a deferred segment and emits its swap scripts; shared by Pending and Catch. */
export async function finalizeAndSwapOutOfOrderSegment(
  ...
  ssr.write(`<template ${QPendingResolved}="${boundaryId}"${revealBoundary?.attrs ?? ''}>`);
  ...
  if (__EXPERIMENTAL__.catchBoundary && errorSwapIds?.length) {
