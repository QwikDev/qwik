...
export const applyQwikComponentBody = (
  ...
  const componentChore = ssr.$scheduler$.schedule(
    ChoreType.COMPONENT,
    host,
    componentQrl,
    srcProps
  );
  if (isPromise(componentChore.$promise$)) {
    return componentChore.$promise$.then(() => componentChore.$returnValue$);
  }
  return componentChore.$returnValue$;
};
