...
export async function csrRenderInto<Props>(
  ...
  const compiled = await compileRoot<CsrRenderComponent<Props>>('csr', root);

  const scheduler = options?.scheduler ?? new Scheduler(noopSchedule);
  const renderResult = await renderCsr(container, compiled.root, {
    scheduler,
    props: options?.props,
...
export async function ssrRender<Props>(
  ...
  const compiled = await compileRootPair<Props>(root);
  const scheduler = options?.scheduler ?? new Scheduler(noopSchedule);
...
async function compileRoot<TRoot extends CsrRenderComponent<any> | SsrRenderComponent<any>>(
  target: RenderTarget,
  root: RenderRoot<any>
): Promise<CompiledRoot<TRoot>> {
  const id = compiledModuleId++;
  const input = await createCompiledInput(root);
...
async function compileRootPair<Props>(root: RenderRoot<Props>): Promise<{
  server: CompiledRoot<SsrRenderComponent<Props>>;
  client: CompiledRoot<CsrRenderComponent<Props>>;
}> {
  const id = compiledModuleId++;
  const input = await createCompiledInput(root);
  const inputPath = createRenderInputPath(input.sourceFile, id);
  const [server, client] = await Promise.all([
    importCompiledRoot<SsrRenderComponent<Props>>(
      'ssr',
      id,
      inputPath,
      input.code,
      input.rootExportName
    ),
    importCompiledRoot<CsrRenderComponent<Props>>(
      'csr',
      id,
      inputPath,
      input.code,
...
async function writeMissingServerSegmentsToClientDir(
  server: CompiledRoot<SsrRenderComponent<any>>,
  client: CompiledRoot<CsrRenderComponent<any>>
): Promise<void> {
...
async function createCompiledInput(root: RenderRoot<any>): Promise<CompiledInput> {
  const location = getRenderCallLocation();
  ...
  const componentName = getRenderRootName(sourceFile, jsxPosition, root.name);
  ...
  const exports = selectReachableDeclarations(declarations, componentName).map(
...
function selectReachableDeclarations(
  declarations: readonly ComponentDeclarationSource[],
  rootName: string
  ...
  for (let i = 0; i < queue.length; i++) {
    const declaration = byName.get(queue[i]);
    if (declaration === undefined) {
      continue;
    }
    const file = ts.createSourceFile(
      `${declaration.name}.tsx`,
      declaration.source,
      ts.ScriptTarget.Latest,
      true,
      ts.ScriptKind.TSX
    ...
    const visit = (node: ts.Node): void => {
      if (ts.isIdentifier(node) && byName.has(node.text) && !reachable.has(node.text)) {
        reachable.add(node.text);
  ...
  return declarations.filter((declaration) => reachable.has(declaration.name));
}

function getRenderRootName(
  ...
  const visit = (node: ts.Node): void => {
    ...
    if (ts.isCallExpression(node) && ts.isIdentifier(node.arguments[0])) {
  ...
  visit(sourceFile);
  if (!name) {
    throw new Error('Render helper expects a named root binding.');
  }
  return name;
}
...
function getRenderCallLocation(): JsxLocation {
  const location = getJsxStackLocation(new Error().stack ?? '');
  if (location === null) {
    throw new Error('Render helper could not locate the calling test.');
  }
  return location;
}
...
function getJsxStackLocation(stack: string): JsxLocation | null {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      fileName.endsWith('/core/test-utils.ts') ||
...
async function importCompiledRoot<TRoot extends CsrRenderComponent<any> | SsrRenderComponent<any>>(
  target: RenderTarget,
  id: number,
  inputPath: string,
  code: string,
...
function resolveSourceFile(fileName: string): string {
  ...
  const packagesPrefix = join(root, 'packages') + '/';
  if (fileName.startsWith(packagesPrefix)) {
    fileName = fileName.slice(packagesPrefix.length);
  ...
  const packagePath = join(root, 'packages', fileName);
  ...
  if (srcIndex !== -1) {
    for (let skip = 1; srcIndex + skip + 1 < parts.length; skip++) {
      const candidate = join(
        root,
        'packages',
        ...parts.slice(0, srcIndex + 1),
        ...parts.slice(srcIndex + 1 + skip)
      );
...
async function debugRender(
  ...
  modules: readonly TransformModule[],
  html: string,
  options: RenderOptions<any> | undefined,
