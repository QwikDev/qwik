...
describe.each([
  ...
  it('should auto-track dependencies read after an await in async computed', async () => {
    const Counter = component$(() => {
      const count = useSignal(1);
      const doubleCount = useComputed$(async () => {
        await Promise.resolve();
        // read after the await: tracked because the compute context survives awaits
        return count.value * 2;
      });
      return <button onClick$={() => count.value++}>{doubleCount.value}</button>;
    });
