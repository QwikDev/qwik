...
export class BranchSubscription implements BranchSubscriber {
  ...
  run(): ValueOrPromise<void> {
    ...
    return maybeThen(conditionFn, (conditionFn) => {
      const trackedConditionFn = runWithCollector(this, conditionFn);
...
export function createBranch(
  ...
  return new BranchSubscription(branch);
}
...
export class SSRBranch {
  ...
  run(): ValueOrPromise<string> {
    const subscription = new SSRBranchSubscription(this);

    return maybeThen(this.conditionQrl.resolved ?? this.conditionQrl.resolve(), (conditionFn) => {
      ...
      return maybeThen(trackedCondition, (trackedCondition) => {
        ...
        return maybeThen(
          rendererQrl?.resolved ?? (rendererQrl ? rendererQrl.resolve() : undefined),
          (renderer) => {
            if (renderer === undefined) {
              return '';
            }
