  ...
  it('hides the content-host and reveals the fallback-host for the given id', () => {
    const document = setup(
      `<div q:container="resumed" q:render="dom-ssr">
        <div q:cc="1" style="display:contents"><p id="content">partial</p></div>
        <div q:cf="1" style="display:none"><p id="fb">fallback</p></div>
      </div>`
    );
    (globalThis as any).qErr(1);
    expect(displayOf(document.querySelector('[q\\:cc="1"]'))).toBe('none');
    expect(displayOf(document.querySelector('[q\\:cf="1"]'))).toBe('contents');
  });

  it('only swaps the matching boundary id', () => {
    const document = setup(
      `<div q:container="resumed" q:render="dom-ssr">
        <div q:cc="1" style="display:contents"></div>
        <div q:cf="1" style="display:none"></div>
        <div q:cc="2" style="display:contents"></div>
        <div q:cf="2" style="display:none"></div>
      </div>`
    );
    (globalThis as any).qErr(2);
    expect(displayOf(document.querySelector('[q\\:cc="1"]'))).toBe('contents');
    expect(displayOf(document.querySelector('[q\\:cf="1"]'))).toBe('none');
    expect(displayOf(document.querySelector('[q\\:cc="2"]'))).toBe('none');
    expect(displayOf(document.querySelector('[q\\:cf="2"]'))).toBe('contents');
  });

  it('strips document and window handlers from the hidden content', () => {
    const document = setup(
      `<div q:container="resumed" q:render="dom-ssr">
        <div q:cc="1" style="display:contents">
