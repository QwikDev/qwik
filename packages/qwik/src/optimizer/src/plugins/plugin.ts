  ...
  const buildStart = async (_ctx: Rollup.PluginContext) => {
    ...
    if (generatePreManifest) {
      ...
      for (const output of result.modules) {
        ...
        debug(`buildStart() add transformedOutput`, key, output.segment?.displayName);
    ...
    if (ext in TRANSFORM_EXTS || TRANSFORM_REGEX.test(pathId)) {
      ...
      for (const mod of newOutput.modules) {
        if (mod !== module) {
          ...
          debug(`transform(${count})`, `segment ${key}`, mod.segment);
...
export interface NormalizedQwikPluginOptions
  extends Omit<Required<QwikPluginOptions>, 'experimental'> {
