...
/**
 * NOTE
 * the following code is copied from Vite and unmodified
...
function buildErrorMessage(err: RollupError, args: string[] = [], includeStack = true): string {
  if (err.plugin) args.push(`  Plugin: ${kleur.magenta(err.plugin)}`);
  const loc = err.loc ? `:${err.loc.line}:${err.loc.column}` : '';
  if (err.id) args.push(`  File: ${kleur.cyan(err.id)}${loc}`);
  if (err.frame) args.push(kleur.yellow(pad(err.frame)));
...
export const VITE_ERROR_OVERLAY_STYLES = `
  ...
  vite-error-overlay::part(message):before {
    width: 70px;
    height: 70px;
    display: block;
    content: '';
    background-image: url("data:image/svg+xml,%3Csvg width='70px' height='70px' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M12 8V12M12 16H12.01M7.8 21H16.2C17.8802 21 18.7202 21 19.362 20.673C19.9265 20.3854 20.3854 19.9265 20.673 19.362C21 18.7202 21 17.8802 21 16.2V7.8C21 6.11984 21 5.27976 20.673 4.63803C20.3854 4.07354 19.9265 3.6146 19.362 3.32698C18.7202 3 17.8802 3 16.2 3H7.8C6.11984 3 5.27976 3 4.63803 3.32698C4.07354 3.6146 3.6146 4.07354 3.32698 4.63803C3 5.27976 3 6.11984 3 7.8V16.2C3 17.8802 3 18.7202 3.32698 19.362C3.6146 19.9265 4.07354 20.3854 4.63803 20.673C5.27976 21 6.11984 21 7.8 21Z' stroke='rgba(255,255,255,0.7)' stroke-width='0.8' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E");
    position: absolute;
    top: -7px;
    left: -7px;
  }
