 ...
 * @public
 */
export async function renderToStream(rootNode: any, opts: RenderToStreamOptions): Promise<RenderToStreamResult> {
 ...
 * @public
 */
export async function renderToString(rootNode: any, opts: RenderToStringOptions = {}): Promise<RenderToStringResult> {
  const createDocTimer = createTimer();
  const doc = _createDocument(opts) as RenderDocument;
  const createDocTime = createDocTimer();
  const renderDocTimer = createTimer();
