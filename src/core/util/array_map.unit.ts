...
describe('array_utils', () => {
  ...
  describe('KeyValueArray', () => {
    ...
    it('should create value if not found', () => {
      const map: KeyValueArray<number> = [] as any;
      expect(keyValueArrayGet(map, 'A')).toEqual(undefined);
      expect(keyValueArrayGet(map, 'A', () => 2)).toEqual(2);
      expect(keyValueArrayGet(map, 'A', () => 0)).toEqual(2);
    });
  });
});
