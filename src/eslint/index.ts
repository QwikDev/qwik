...
const noClosedOverVariables: Rule.RuleModule = {
  ...
  create(context) {
    ...
    function isQHook(node: Rule.Node) {
      return isCallExpressionName(node, 'qHook');
    }

    function isQComponent(node: Rule.Node) {
      return isCallExpressionName(node, 'qComponent');
    }
    ...
    function traverseScopesVariables(
      ...
      cb: (variable: Scope.Variable, identifier: ESTree.Identifier) => void
    ) {
      let scope = null;
      while ((scope = queue.pop())) {
        queue.push(...scope.childScopes);
        for (const { identifier, resolved: variable } of scope.references) {
          if (variable !== null) {
            cb(variable, identifier);
          }
        }
      }
    }
    ...
    function findClosedOverVariables(
      ...
      if (currentQHookScope && currentQHookScope.parentNode) {
        ...
        traverseScopesVariables(queue, (variable, identifier) => {
          const { name } = variable;
