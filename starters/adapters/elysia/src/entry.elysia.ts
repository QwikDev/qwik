 ...
 * WHAT IS THIS FILE?
 *
 * It's the entry point for the Elysia server when building for production.
 *
...
const handleQwikRequest = async (request: Request) => {
...
new Elysia()
  .all("/", ({ request }) => handleQwikRequest(request))
  .all("/*", ({ request }) => handleQwikRequest(request))
  .listen(port);
