...
export const Nested = component$((props: { level: number }) => {
    ...
    return <> {
        props.level >= MAX_DEPTH && children.length
            ? id
            : <ul id={`${id}`} key={id} style="box-sizing: border-box; border: 1px solid #ccc; padding: 1rem; list-style: none">
                {id}
                {
...
export const UseId = component$(() => {
    ...
    useClientEffect$(() => {
        ...
        resultSignal.value = (totalIdsSignal.value == validIdsSignal.value && collsionsSignal.value == 0)
            ? "Passed"
            : "Failed";
    });

    return <>
        <h1>useId() Collision Test <span id="result" style={{ visibility: resultSignal.value ? "visible" : "hidden" }}>{resultSignal.value}</span></h1>
        Verify there are no id collisions from useId(). This tests generates a random depth and number of children each time to inject some randomness and create greater coverage over time. This test does not take into consideration microfrontends, but does use q:base to generate the base hash.
        <p><b>Total IDs:</b> <span id="totalIds">{totalIdsSignal.value}</span></p>
        <p><b>Valid IDs:</b> <span id="validIds">{totalIdsSignal.value}</span></p>
        <p><b>Collisions:</b> <span id="collisions">{collsionsSignal.value}</span></p>
