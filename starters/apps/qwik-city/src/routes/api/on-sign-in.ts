...
export const post: EndpointHandler = async (ev: any) => {
  ...
  return new Response(status.msg, { 
    status: status.code,
    headers: result == 'New Login' ? {
      'Set-Cookie': `${AUTHTOKEN_NAME}=${generateToken()}; Secure; HttpOnly; Max-Age=${60 * 5}`
    } : {}
  });
}
...
export const handleAuth = (submission: FormData, cookie: Cookie): AuthResult => {
  ...
  if (token !== undefined) { return 'Already Logged In' }
  if (username == 'admin' && password == 'password') { return 'New Login'}
